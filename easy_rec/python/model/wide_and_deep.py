...
from easy_rec.python.layers import dnn, input_layer
from easy_rec.python.model.rank_model import RankModel
from easy_rec.python.protos.wide_and_deep_pb2 import (  # NOQA
    WideAndDeep as WideAndDeepConfig,
)
...
class WideAndDeep(RankModel):
    def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
        super(WideAndDeep, self).__init__(model_config, feature_configs, features, labels, is_training)
        assert (
            model_config.WhichOneof('model') == 'wide_and_deep'
    ...
    def build_predict_graph(self):
        ...
        logging.info('input deep features dimension: %d' % self._deep_features.get_shape()[-1])
        ...
        logging.info('output deep features dimension: %d' % deep_fea.get_shape()[-1])
        ...
        if has_final:
            ...
            final_layer = dnn.DNN(self._model_config.final_dnn, self._l2_reg, 'final_dnn', self._is_training)
            all_fea = final_layer(all_fea)
            output = tf.layers.dense(all_fea, self._num_class, kernel_regularizer=self._l2_reg, name='output')
        else:
            deep_out = tf.layers.dense(deep_fea, self._num_class, kernel_regularizer=self._l2_reg, name='deep_out')
