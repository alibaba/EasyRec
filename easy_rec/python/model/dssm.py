...
class DSSM(EasyRecModel):
    def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
        super(DSSM, self).__init__(model_config, feature_configs, features, labels, is_training)
        self._loss_type = self._model_config.loss_type
        self._num_class = self._model_config.num_class
        assert (
            self._model_config.WhichOneof('model') == 'dssm'
    ...
    def _list_wise_sim(self, user_emb, item_emb):
        ...
        if hard_neg_indices is not None:
            ...
            pos_item_emb, neg_item_emb, hard_neg_item_emb = tf.split(item_emb, [batch_size, -1, noclk_size], axis=0)
        else:
            pos_item_emb = item_emb[:batch_size]
            neg_item_emb = item_emb[batch_size:]

        pos_user_item_sim = tf.reduce_sum(tf.multiply(user_emb, pos_item_emb), axis=1, keep_dims=True)
        ...
        if hard_neg_indices is not None:
            ...
            hard_neg_mask = tf.scatter_nd(
                hard_neg_indices, tf.ones_like(hard_neg_user_item_sim, dtype=tf.bool), shape=neg_sim_shape
            )
            hard_neg_user_item_sim = tf.scatter_nd(hard_neg_indices, hard_neg_user_item_sim, shape=neg_sim_shape)
            neg_user_item_sim = tf.where(hard_neg_mask, x=hard_neg_user_item_sim, y=neg_user_item_sim)
    ...
    def build_predict_graph(self):
        ...
        user_dnn = dnn.DNN(self.user_tower.dnn, self._l2_reg, 'user_dnn', self._is_training)
        ...
        item_dnn = dnn.DNN(self.item_tower.dnn, self._l2_reg, 'item_dnn', self._is_training)
    ...
    def _build_list_wise_loss_graph(self):
        if self._loss_type == LossType.SOFTMAX_CROSS_ENTROPY:
            ...
            self._loss_dict['cross_entropy_loss'] = -tf.reduce_mean(tf.log(hit_prob + 1e-12))
    ...
    def _build_point_wise_loss_graph(self):
        ...
        self._loss_dict[loss_name] = loss_builder.build(
            self._loss_type, label=label, pred=pred, loss_weight=self._sample_weight
        )
    ...
    def _build_list_wise_metric_graph(self, eval_config):
        ...
        for metric in eval_config.metrics_set:
            if metric.WhichOneof('metric') == 'recall_at_topk':
                ...
                metric_dict['recall_at_top%d' % metric.recall_at_topk.topk] = metrics.recall_at_k(
    ...
    def _build_point_wise_metric_graph(self, eval_config):
        ...
        for metric in eval_config.metrics_set:
            if metric.WhichOneof('metric') == 'auc':
                ...
                assert self._loss_type == LossType.CLASSIFICATION
                metric_dict['recall_at_topk%d' % metric.recall_at_topk.topk] = metrics.recall_at_k(
                    label, self._prediction_dict['probs'], metric.recall_at_topk.topk
                )
            elif metric.WhichOneof('metric') == 'mean_absolute_error':
                assert self._loss_type == LossType.L2_LOSS
                metric_dict['mean_absolute_error'] = metrics.mean_absolute_error(label, self._prediction_dict['y'])
            elif metric.WhichOneof('metric') == 'accuracy':
                assert self._loss_type == LossType.CLASSIFICATION
                metric_dict['accuracy'] = metrics.accuracy(label, self._prediction_dict['probs'])
            else:
                ValueError('invalid metric type: %s' % str(metric))
        return metric_dict
