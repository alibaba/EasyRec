...
if __name__ == '__main__':
    ...
    parser.add_argument('--saved_model_dir', type=str, default=None, help='saved model dir')
    parser.add_argument('--output_dir', type=str, default=None, help='output dir')
    parser.add_argument('--redis_url', type=str, default='127.0.0.1:6379', help='redis url')
    parser.add_argument('--redis_passwd', type=str, default='', help='redis password')
    ...
    if meta_graph_version == '':
        export_ts = [x for x in args.saved_model_dir.split('/') if x != '' and x is not None]
    ...
    with tf.Session() as sess:
        ...
        tf.saved_model.simple_save(sess, output_dir, inputs=inputs_map, outputs=outputs_map)
        ...
        saved_model.meta_graphs[0].meta_info_def.meta_graph_version = meta_graph_editor.meta_graph_version
            ...
            with GFile(args.test_data_path, 'r') as fin:
                ...
                for line_str in fin:
                    ...
                    if len(feature_vals) >= 32:
                        break
                out_vals = sess.run(outputs_map, feed_dict={inputs_map['features']: feature_vals})
