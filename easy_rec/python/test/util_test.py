...
class UtilTest(tf.test.TestCase):
  def test_get_ckpt_version(self):
    ver = estimator_utils.get_ckpt_version('oss://easyrec/ckpts/model.ckpt-6500.meta')
    assert ver == 6500, 'invalid version: %s' % str(ver)
    ver = estimator_utils.get_ckpt_version('oss://easyrec/ckpts/model.ckpt-6500')
    assert ver == 6500, 'invalid version: %s' % str(ver)

  def test_get_expression_greater(self):
    result = get_expression('age_level>item_age_level', ['age_level', 'item_age_level'])
  ...
  def test_get_expression_and(self):
    result = get_expression('(age_level>3)&(item_age_level<1)', ['age_level', 'item_age_level'])
    ...
    assert (
      result == "tf.greater(parsed_dict['age_level'], parsed_dict['item_age_level']) &"
