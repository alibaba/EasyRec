...
class RocketLaunching(RankModel):
  ...
  def __init__(
    ...
    super(RocketLaunching, self).__init__(
  ...
  def build_predict_graph(self):
    ...
    if self._model_config.HasField('share_dnn'):
      share_dnn_layer = dnn.DNN(
        self._model_config.share_dnn, self._l2_reg, 'share_dnn',
        self._is_training
      )
      share_feature = share_dnn_layer(self._features)
    booster_dnn_layer = dnn.DNN(
      self._model_config.booster_dnn, self._l2_reg, 'booster_dnn',
      self._is_training
    )
    light_dnn_layer = dnn.DNN(
      self._model_config.light_dnn, self._l2_reg, 'light_dnn',
      self._is_training
    )
    if self._model_config.HasField('share_dnn'):
      self.booster_feature = booster_dnn_layer(
        share_feature, self.hidden_layer_feature_output
      )
      input_embedding_stop_gradient = tf.stop_gradient(share_feature)
      self.light_feature = light_dnn_layer(
        input_embedding_stop_gradient, self.hidden_layer_feature_output
      )
    else:
      self.booster_feature = booster_dnn_layer(
        self._features, self.hidden_layer_feature_output
      )
    ...
    self._prediction_dict.update(
      self._output_to_prediction_impl(
        light_out, self._loss_type, num_class=self._num_class, suffix='_light'
      )
    )
