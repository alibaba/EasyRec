...
from easy_rec.python.utils import config_util, fg_util, hpo_util, pai_util
from easy_rec.python.utils.distribution_utils import (
    set_tf_config_and_get_train_worker_num,  # NOQA
)
...
from easy_rec.python.main import (  # NOQA
    _train_and_evaluate_impl as train_and_evaluate_impl,
)
...
tf.app.flags.DEFINE_string('all_col_types', '', 'column data types, for build record defaults, separated with ,')
...
tf.app.flags.DEFINE_string(
    'output_cols', None, 'output columns, such as: score float. multiple columns are separated by ,'
)
...
def main(argv):
    ...
    assert (
        FLAGS.distribute_strategy in DistributionStrategyMap
    ...
    if FLAGS.model_dir:
        ...
        assert pipeline_config.model_dir.startswith('oss://'), (
            'invalid model_dir format: %s' % pipeline_config.model_dir
        )
    ...
    if FLAGS.cmd == 'train':
        ...
        # parse selected_cols
        set_selected_cols(pipeline_config, FLAGS.selected_cols, FLAGS.all_cols, FLAGS.all_col_types)
        ...
        set_tf_config_and_get_train_worker_num(
            FLAGS.ps_hosts, FLAGS.worker_hosts, FLAGS.task_index, FLAGS.job_name, eval_method='none'
        )
        set_distribution_config(pipeline_config, num_worker, num_gpus_per_worker, distribute_strategy)
        ...
        config_util.auto_expand_share_feature_configs(pipeline_config)
        easy_rec.export(
            FLAGS.export_dir, pipeline_config, FLAGS.checkpoint_path, FLAGS.asset_files, FLAGS.verbose, **redis_params
        )
