...
class MultiTowerBST(RankModel):
  ...
  def __init__(
    ...
    super(MultiTowerBST, self).__init__(
    ...
    logging.info(
      'all tower num: {0}'.format(self._tower_num + self._bst_tower_num)
    )
  ...
  def attention_net(self, net, dim, cur_seq_len, seq_size, name):
    ...
    scores = tf.matmul(
      query_net, key_net, transpose_b=True
    )  # [B, seq_size, seq_size]

    hist_mask = tf.sequence_mask(
      cur_seq_len, maxlen=seq_size - 1
    )  # [B, seq_size-1]
    cur_id_mask = tf.ones(
      tf.stack([tf.shape(hist_mask)[0], 1]), dtype=tf.bool
    )  # [B, 1]
    mask = tf.concat([hist_mask, cur_id_mask], axis=1)  # [B, seq_size]
    masks = tf.reshape(
      tf.tile(mask, [1, seq_size]), (-1, seq_size, seq_size)
    )  # [B, seq_size, seq_size]
  ...
  def multi_head_att_net(
    self, id_cols, head_count, emb_dim, seq_len, seq_size
  ):
    ...
    for start_idx in range(0, emb_dim, part_cols_emd_dim):
      ...
      part_id_col = tf.slice(
        id_cols, [0, 0, start_idx], [-1, -1, part_cols_emd_dim]
      )
  ...
  def bst(self, bst_fea, seq_size, head_count, name):
    ...
    hist_id_col = tf.cond(
      tf.constant(seq_size) > cur_batch_max_seq_len, lambda: tf.pad(
        hist_id_col,
        [[0, 0], [0, seq_size - cur_batch_max_seq_len - 1], [0, 0]], 'CONSTANT'
      ), lambda: tf.slice(hist_id_col, [0, 0, 0], [-1, seq_size - 1, -1])
    )
    all_ids = tf.concat(
      [hist_id_col, tf.expand_dims(cur_id, 1)], axis=1
    )  # b, seq_size, emb_dim

    emb_dim = int(all_ids.shape[2])
    attention_net = self.multi_head_att_net(
      all_ids, head_count, emb_dim, seq_len, seq_size
    )
