...
class DataHubInput(Input):
    ...
    def __init__(self, data_config, feature_config, datahub_config, task_index=0, task_num=1):
        super(DataHubInput, self).__init__(data_config, feature_config, '', task_index, task_num)
        if DataHub is None:
            logging.error('please install datahub: ', 'pip install pydatahub ;Python 3.6 recommended')
        try:
            self._datahub_config = datahub_config
            if self._datahub_config is None:
                pass
            self._datahub = DataHub(
                self._datahub_config.akId, self._datahub_config.akSecret, self._datahub_config.region
            )
            self._num_epoch = 0
    ...
    def _datahub_generator(self):
        ...
        batch_defaults = [np.array([x] * self._data_config.batch_size) for x in record_defaults]
        try:
            ...
            for shard in shards:
                ...
                cursor_result = self._datahub.get_cursor(
                    self._datahub_config.project, self._datahub_config.topic, shard_id, CursorType.OLDEST
                )
                cursor = cursor_result.cursor
                limit = self._data_config.batch_size
                while True:
                    get_result = self._datahub.get_tuple_records(
                        self._datahub_config.project, self._datahub_config.topic, shard_id, record_schema, cursor, limit
                    )
    ...
    def _build(self, mode, params):
        ...
        dataset = tf.data.Dataset.from_generator(
            self._datahub_generator, output_types=list_type, output_shapes=list_shapes
        )
        if mode == tf.estimator.ModeKeys.TRAIN:
            dataset = dataset.shuffle(self._data_config.shuffle_buffer_size, seed=2020, reshuffle_each_iteration=True)
