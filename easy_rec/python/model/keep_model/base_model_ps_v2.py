...
from tensorflow.keras.layers import Dense, Concatenate, concatenate, Lambda, Add, add, Activation, \
    Average, BatchNormalization, Embedding, Input, Reshape, Flatten, Add
...
from base.context import Context, conf_overwrite
...
def make_sparse_indice(summed):
    """https://stackoverflow.com/questions/54790000/calculate-segment-ids-needed-in-tf-math-segment-sum-by-length-of-segments-in-ten
    summed is result of tf.cumsum
    ...
    return tf.searchsorted(
        summed, tf.range(summed[-1]), side='right')
...
def model_inputs_prepare(inputs, ctx):
    ...
    for idx in range(len(ctx.slot_size_vec)):
        len_arr.append(tf.reshape(tf.slice(
            inputs["sparse_len"],
            [0, ctx.slot_offset_vec[idx]],
    ...
    tmp_arr = []
    for idx in range(len(ctx.slot_size_vec)):
        tmp_arr.append(tf.slice(
            inputs["sparse_id"], [off_arr[idx]], [size_arr[idx]]))

    id_arr = []
    for idx in range(len(ctx.slot_size_vec)):
        id_arr.append(tf.clip_by_value(
            tmp_arr[idx] - inputs["uniq_num_by_dim"][idx], 0, 2**30))
    ...
    for idx in range(len(ctx.embed_dim_vec)):
        tmp = tf.slice(
            inputs["ps_embed"],
            [inputs["uniq_off_by_dim"][idx]],
            [inputs["uniq_off_by_dim"][idx + 1] - inputs["uniq_off_by_dim"][idx]])  # noqa
...
class ExportModel(Model):
    def __init__(self, model_conf, indim, name='export_model'):
        ...
        self.indim = indim
        self._set_inputs([tf.TensorSpec([None] + tmp, tf.float32, name='input' + str(i)) for i, tmp in enumerate(indim)])
        self.dnn = DNN([300, 150, 50], activation='relu',
                                  use_bn=True, output_activation='relu')
        self.din_layer = AttentionSequencePoolingLayer()
        self.output_layers = [Dense(1, activation='sigmoid', name='output_' + label_conf['input_name']) for label_conf in self.label_conf_list]
    ...
    def call(self, embed_input, training=False):
        ...
        listclick_len = tf.cast(tf.reduce_sum(len_mask, axis=1, keepdims=True), tf.int32)
        ...
        listcomplete_len = tf.cast(tf.reduce_sum(len_mask, axis=1, keepdims=True), tf.int32)


        click_att_data = self.din_layer([target_fea, listclick, listclick_len])
        click_att_data = tf.squeeze(click_att_data, axis=[1])

        complete_att_data = self.din_layer([target_fea, listcomplete, listcomplete_len])
        complete_att_data = tf.squeeze(complete_att_data, axis=[1])


        embed = tf.concat([click_att_data, complete_att_data, embed_input[3]], axis=1)
...
class BaseModel:
    def __init__(self, conf):
        ...
        self.save_path = self.data_conf.get('save_path').rstrip('/') + '/' + self.model_name + '/'
        if not os.path.exists(self.save_path):
            os.makedirs(self.save_path)
        self.save_hdfs_path = self.data_conf.get('save_hdfs_path').rstrip('/') + '/' + self.model_name + '/'
        self.save_iter = 1
        ...
        if self.profile:
            self.tb_callback = tf.keras.callbacks.TensorBoard(log_dir=self.save_path + '/profile',
                                profile_batch='10,12')

        if self.mode == 'Update':
            self.process_update_conf()
        self.feature_conf, self.group_length = self.parse_feature_conf(conf['feature'])
    ...
    def build_input(self):
        ...
        """
        ctx = self.ctx
        in_dim = np.ma.innerproduct(ctx.slot_size_vec[self.custom_group_count:], ctx.embed_dim_vec[self.custom_group_count:])
        if ctx.use_acc_ctr:
            in_dim += ctx.slot_num
        self.ctx.net_input_dim = in_dim
        input_dic = {}
        input_dic["sample_idx"] = Input(dtype=tf.int64, batch_shape=(None,))
        input_dic["label"] = Input(dtype=tf.float32, batch_shape=(None, len(self.model_conf['label'])))
        input_dic["sparse_len"] = Input(dtype=tf.int32, batch_shape=(None,
        None), name='sparse_len')
        input_dic["dense_fea"] = Input(dtype=tf.float32, batch_shape=(None,
        None), name='dense_fea')

        input_dic["uniq_num_by_dim"] = Input(dtype=tf.int32,
            batch_shape=(None,))
        input_dic["uniq_off_by_dim"] = Input(dtype=tf.int32,
            batch_shape=(None,))
        # single worker
        ...
        input_dic["ps_stat"] = Input(dtype=tf.float32,
            batch_shape=(None, 2), name='ps_stat')
        input_dic["b1_power"] = Input(dtype=tf.float32,
            batch_shape=(1,), name='b1_power')
        input_dic["b2_power"] = Input(dtype=tf.float32,
            batch_shape=(1,), name='b2_power')
        input_dic["test_mask"] = Input(dtype=tf.float32,
            batch_shape=(1, in_dim), name='test_mask')
        ...
        for idx, pair in enumerate(self.group_length.items()):
            ...
            group_embed = tf.scatter_nd(non_zero_indice, tmp, [batch_split*fea_size, length, fea_dim])
            group_embed = tf.reshape(group_embed, [-1, fea_size, length, fea_dim])
            group_embed = tf.reshape(tf.transpose(group_embed, perm=[0, 2, 1, 3]), [-1, length, fea_dim_total])
        ...
        for idx in range(self.custom_group_count, len(ctx.embed_dim_vec)):
            ...
            tmp = tf.math.segment_sum(
                tf.gather(embed_arr[idx], id_arr[idx]),
                tmp_seg_ids)
            tmp = tf.reshape(tmp,
                [batch_split, ctx.slot_size_vec[idx] * ctx.embed_dim_vec[idx]])
            if ctx.use_acc_ctr:
                acc_ctr = tf.reshape(tf.math.segment_mean(
                    tf.gather(ctr, tmp_arr[idx]), tmp_seg_ids),
                    [batch_split, ctx.slot_size_vec[idx], 1])
                tmp = tf.reshape(tmp,
                    [batch_split, ctx.slot_size_vec[idx], ctx.embed_dim_vec[idx]])
    ...
    def build_and_compile_model(self):
        ...
        self.checkpoint = tf.train.Checkpoint(step=tf.Variable(1), model=model, optimizer=opt)
        return model
    ...
    def load_model(self, path):
        if not self.data_conf.get('load_embed_only'):
            ...
            self.cpt_manager = tf.train.CheckpointManager(self.checkpoint,
                                                        cpt_path,
                                                        max_to_keep=1)
    ...
    def get_batch_input(self, batch, local_db, mask=None):
        ...
        input_dic["uniq_num_by_dim"] = np.cumsum([0] + batch["uniq_num_by_dim"].tolist()).astype(np.int32)  # noqa
        tmp = np.array(ctx.embed_dim_vec).astype(np.int32) * batch["uniq_num_by_dim"].astype(np.int32)  # noqa
        input_dic["uniq_off_by_dim"] = np.cumsum([0] + tmp.tolist()).astype(np.int32)  # noqa
        input_dic["sparse_id"] = batch["sparse_id"][0]

        input_dic["ps_embed"] = ps_embed[0]
        input_dic["ps_embed_m"] = np.zeros_like(ps_embed[1]) if self.adam_restart else ps_embed[1]
        input_dic["ps_embed_v"] = np.zeros_like(ps_embed[2]) if self.adam_restart else ps_embed[2]
        ...
        input_dic['test_mask'] = np.ones([1, self.ctx.net_input_dim], dtype=self.ctx.npval)
    ...
    def fit_day(self, train_itor, local_db):
        ...
        metrics = [AUCUtil(name=conf['input_name'] + '_metric') for conf in self.model_conf['label']]
        total_metrics = [BucketAUCUtil(name=conf['input_name'], batch_size=batch_size) for conf in self.model_conf['label']]
        step = 0
        while True:
            try:
                if self.profile:
            ...
            try:
                ...
                for embed, online_embed in zip(
                  ...
                  for idx, fea_name in enumerate(features):
                    fea_embed_on = embed_on[idx*embed_size: idx*embed_size + embed_size]
                    ...
                    if total_diff > 0 or sum(fea_embed_on) == 0:
                      ...
                      print(fea_embed_on)
                      print(fea_embed_off)
                      feature_diff_data[fea_name] = feature_diff_data.get(fea_name, 0) + 1
                  print('-'*60)
                print('total cnt', len(online_embeds))
                feature_diff_data = [[key, value] for key, value in feature_diff_data.items()]
    ...
    def aucdiff_run(self, train_itor, local_db):
        ctx = self.ctx
        batch_size = ctx.batch_size
        total_metrics_list = [[BucketAUCUtil(name=conf['input_name'], batch_size=batch_size) for conf in self.model_conf['label']] for _ in range(ctx.origin_slot_num + 1)]
        tmask = [np.ones([1, self.ctx.net_input_dim], dtype=self.ctx.npval) for _ in range(self.ctx.origin_slot_num)]  # noqa
        ...
        while True:
            try:
                ...
                for mask in range(self.ctx.origin_slot_num + 1):
                    ...
                    for gt, pred, loss, total_metric in zip(
                        output_dic['ground_truth'],
                        output_dic['prediction'],
                        output_dic['losses'],
                ...
                for mask in range(self.ctx.origin_slot_num):
                    print("# Mask %s from %s to %s." %
                                 (self.ctx.origin_slots.split(",", -1)[mask],
                                  start_arr[mask], end_arr[mask]))
    ...
    def run(self):
        ...
        if self.mode.startswith('Train'):
            ...
            end_time = datetime.datetime.strptime(f'{end_day} 00:00:00', '%Y-%m-%d %H:%M:%S')
            trained_days = 0
        ...
        if 'Eval' in self.mode:
            ...
            eval_time = datetime.datetime.strptime(f'{eval_day} 00:00:00', '%Y-%m-%d %H:%M:%S')
