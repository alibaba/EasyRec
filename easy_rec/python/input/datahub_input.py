...
class DataHubInput(Input):
  ...
  def __init__(
    ...
    self._offset_dict = {}
    if datahub_config:
      shard_result = self._datahub.list_shard(
      ...
      if offset_type == 'offset_time':
        ...
        for x in self._all_shards:
          ...
          cursor_result = self._datahub.get_cursor(
            self._datahub_config.project, self._datahub_config.topic, ks,
            CursorType.SYSTEM_TIME, ts
          )
      ...
      ) > 0, 'data_config.feature_fields are not set.'
      ...
      if len(self._dh_fea_ids) > 1:
        self._filter_fea_func = lambda record: ''.join(
          [record.values[x] for x in self._dh_fea_ids]
        ).split(chr(2))[1] == '-1024'
      else:
        dh_fea_id = self._dh_fea_ids[0]
        self._filter_fea_func = lambda record: record.values[
          dh_fea_id].split(self._data_config.separator)[1] == '-1024'
  ...
  def _parse_record(self, *fields):
    ...
    def _dump_offsets():
      all_offsets = {
        ...
        for x in self._shards if x.shard_id in self._offset_dict
      }
      return json.dumps(all_offsets)

    field_dict[Input.DATA_OFFSET
              ] = tf.py_func(_dump_offsets, [], dtypes.string)
  ...
  def _datahub_generator(self):
    ...
    try:
      self._datahub.wait_shards_ready(
        self._datahub_config.project, self._datahub_config.topic
      )
      topic_result = self._datahub.get_topic(
        self._datahub_config.project, self._datahub_config.topic
      )
      if topic_result.record_type != RecordType.TUPLE:
        logging.error(
          'datahub topic type(%s) illegal' % str(topic_result.record_type)
        )
      ...
      while True:
        ...
        for row_id, record in enumerate(get_result.records):
          if self._is_data_empty(record):
            logging.warning(
              'skip empty data record: %s' % self._dump_record(record)
            )
            continue
          if self._filter_fea_func is not None:
            if self._filter_fea_func(record):
              logging.warning(
                'filter data record: %s' % self._dump_record(record)
              )
              continue
