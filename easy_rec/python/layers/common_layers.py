...
class EnhancedInputLayer(object):
    ...
    def __call__(self, config, is_training, **kwargs):
        ...
        if config.do_bernoulli_mask:
            features, _ = self.inputs
            keep_prob = config.bernoulli_mask_rate
            bern = tf.distributions.Bernoulli(probs=keep_prob, dtype=tf.float32)
            mask = bern.sample(sample_shape=tf.shape(features))
            feature1 = features * mask
            feature2 = features * (1.0 - mask)
