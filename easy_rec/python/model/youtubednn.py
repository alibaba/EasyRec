...
from easy_rec.python.protos.youtubednn_pb2 import YouTubeDNN as YouTubeDNNConfig  # NOQA
...
class YouTubeDNN(MatchModel):
  ...
  def __init__(self,
               ...
               labels=None,
               is_training=False):
    super(YouTubeDNN, self).__init__(model_config, feature_configs, features,
                                     labels, is_training)
    assert self._model_config.WhichOneof('model') == 'youtubednn', \
        'invalid model config: %s' % self._model_config.WhichOneof('model')
    self._model_config = self._model_config.youtubednn
    assert isinstance(self._model_config, YouTubeDNNConfig)
  ...
  def build_predict_graph(self):
    ...
    item_tower_emb = self.item_tower_feature
  ...
  def build_output_dict(self):
    output_dict = super(YouTubeDNN, self).build_output_dict()
  ...
  def build_rtp_output_dict(self):
    output_dict = super(YouTubeDNN, self).build_rtp_output_dict()
