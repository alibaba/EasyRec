    ...
    if distribute_strategy == DistributionStrategy.MirroredStrategy:
        ...
        if eval_method == 'separate':
            if 'evaluator' in cluster:
                ...
                tf_config = {'cluster': cluster, 'task': {'type': task_type, 'index': task_index_}}
                ...
                if len(worker_hosts) > 1:
                    ...
                    if job_name == 'ps':
                        os.environ['TF_CONFIG'] = json.dumps(
                            {'cluster': cluster, 'task': {'type': job_name, 'index': task_index}}
                        )
                        ...
                        if task_index == 0:
                            ...
                            os.environ['TF_CONFIG'] = json.dumps(
                                {'cluster': cluster, 'task': {'type': job_name, 'index': task_index - 2}}
                            )
        else:
            if 'evaluator' in cluster:
                ...
                if task_type == 'evaluator':
                    tf_config = {'cluster': cluster, 'task': {'type': 'worker', 'index': train_worker_num - 2}}
                else:
                    tf_config = {'cluster': cluster, 'task': {'type': task_type, 'index': task_index_}}
                os.environ['TF_CONFIG'] = json.dumps(tf_config)
            else:
                ...
                if job_name == 'ps':
                    ...
                    if task_index == 0:
                        ...
                        os.environ['TF_CONFIG'] = json.dumps(
                            {'cluster': cluster, 'task': {'type': 'worker', 'index': task_index - 1}}
                        )
            if eval_method == 'none':
