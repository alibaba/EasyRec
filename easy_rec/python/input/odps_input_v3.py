...
class OdpsInputV3(Input):
    ...
    def __init__(self, data_config, feature_config, input_path, task_index=0, task_num=1):
        super(OdpsInputV3, self).__init__(data_config, feature_config, input_path, task_index, task_num)
        self._num_epoch = 0
    ...
    def _odps_read(self):
        ...
        for table_path in self._input_path:
            reader = common_io.table.TableReader(
                table_path, selected_cols=selected_cols, slice_id=self._task_index, slice_count=self._task_num
            )
