...
class BaseSampler(object):
  ...
  def _parse_nodes(self, nodes):
    if self._log_first_n > 0:
      logging.info(
        'num_example=%d num_eval_example=%d node_num=%d' % (self._num_sample, self._num_eval_sample, len(nodes.ids))
      )
      self._log_first_n -= 1
    features = []
    int_idx = 0
    float_idx = 0
    string_idx = 0
    for attr_gl_type, attr_np_type in zip(self._attr_gl_types, self._attr_np_types):
      if attr_gl_type == 'int':
...
class NegativeSamplerInMemory(BaseSampler):
  ...
  def __init__(
    ...
    if six.PY2 and isinstance(attr_delimiter, type('')):
  ...
  def _load_table(self, data_path, attr_delimiter):
    ...
    for tid in range(len(schema)):
      if schema[tid][0].startswith('id'):
        item_id_col = tid
        break
    print('NegativeSamplerInMemory: feature_id_col = %d, item_id_col = %d' % (fea_id_col, item_id_col))
    while True:
      try:
  ...
  def _get_impl(self, ids):
    ...
    indices = np.random.choice(len(self._item_ids), size=self._num_sample + self._batch_size, replace=False)
...
class NegativeSamplerV2(BaseSampler):
  ...
  def __init__(
    ...
    self._g = (
      gl.Graph()
      .node(
        tf.compat.as_str(user_data_path),
        node_type='user',
        decoder=gl.Decoder(weighted=True),
      )
      .node(
        tf.compat.as_str(item_data_path),
        node_type='item',
        decoder=gl.Decoder(
          attr_types=self._attr_gl_types,
          weighted=True,
      ...
      .edge(
        tf.compat.as_str(edge_data_path),
  ...
  def get(self, src_ids, dst_ids):
    ...
    Returns:
      Negative sampled feature dict.
    """
    sampled_values = tf.py_func(self._get_impl, [src_ids, dst_ids], self._attr_tf_types)
    result_dict = {}
    for k, t, v in zip(self._attr_names, self._attr_tf_types, sampled_values):
      v.set_shape([self._num_sample])
      result_dict[k] = v
...
class HardNegativeSampler(BaseSampler):
  ...
  def __init__(
    ...
    super(HardNegativeSampler, self).__init__(fields, num_sample, num_eval_sample)
    self._batch_size = batch_size
    self._g = (
      gl.Graph()
      .node(
      ...
      .edge(
        tf.compat.as_str(hard_neg_edge_data_path),
    ...
    expand_factor = int(math.ceil(self._num_sample / batch_size))
    self._neg_sampler = self._g.negative_sampler('item', expand_factor, strategy='node_weight')
    self._hard_neg_sampler = self._g.neighbor_sampler(['hard_neg_edge'], num_hard_sample, strategy='full')
  ...
  def get(self, src_ids, dst_ids):
    ...
    result_dict = {}
    for k, t, v in zip(self._attr_names, self._attr_tf_types, output_values[:-1]):
      v.set_shape([None])
      result_dict[k] = v
...
class HardNegativeSamplerV2(BaseSampler):
  ...
  def __init__(
    ...
    super(HardNegativeSamplerV2, self).__init__(fields, num_sample, num_eval_sample)
    self._batch_size = batch_size
    self._g = (
      gl.Graph()
      .node(
    ...
    expand_factor = int(math.ceil(self._num_sample / batch_size))
    self._neg_sampler = self._g.negative_sampler('edge', expand_factor, strategy='random', conditional=True)
    self._hard_neg_sampler = self._g.neighbor_sampler(['hard_neg_edge'], num_hard_sample, strategy='full')

  def _get_impl(self, src_ids, dst_ids):
    src_ids = np.array(src_ids, dtype=np.int64)
    src_ids_padded = np.pad(src_ids, (0, self._batch_size - len(src_ids)), 'edge')
    dst_ids = np.array(dst_ids, dtype=np.int64)
    dst_ids = np.pad(dst_ids, (0, self._batch_size - len(dst_ids)), 'edge')
...
def build(data_config):
  ...
  if sampler_type == 'negative_sampler':
    ...
    user_input_path = process_multi_file_input_path(sampler_config.user_input_path)
    item_input_path = process_multi_file_input_path(sampler_config.item_input_path)
    pos_edge_input_path = process_multi_file_input_path(sampler_config.pos_edge_input_path)
    ...
    user_input_path = process_multi_file_input_path(sampler_config.user_input_path)
    item_input_path = process_multi_file_input_path(sampler_config.item_input_path)
    hard_neg_edge_input_path = process_multi_file_input_path(sampler_config.hard_neg_edge_input_path)
