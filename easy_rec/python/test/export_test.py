...
class ExportTest(tf.test.TestCase):
    ...
    @RunAsSubprocess
    def _predict_and_check(self, data_path, saved_model_dir, cmp_result, keys=['probs'], separator=',', tol=1e-4):
        ...
        for i in range(len(output_res)):
            for key in keys:
                ...
                assert diff < tol, 'too much difference: %.6f for %s, tol=%.6f' % (diff, key, tol)
    ...
    def test_fg(self):
        self._export_test('samples/model_config/taobao_fg.config', self._extract_rtp_data, separator='')
    ...
    def test_export_with_out_in_ckpt_config(self):
      ...
      """ % (
        ...
        # prepare model
        self.assertTrue(
            test_utils.test_single_train_eval(pipeline_config_path, test_dir=test_dir, post_check_func=_post_check_func)
        )

    def _export_test(self, pipeline_config_path, extract_data_func=None, separator=',', keys=['probs']):
        test_dir = test_utils.get_tmp_dir()
        logging.info('test dir: %s' % test_dir)
    ...
    """ % (
        ...
        self._predict_and_check(test_data_path, export_dir_single, cmp_result, keys=keys, separator=separator)
        self._predict_and_check(test_data_path, export_dir_multi, cmp_result, keys=keys, separator=separator)
    ...
    def test_big_model_export(self):
        ...
        # prepare model
        self.assertTrue(
            test_utils.test_single_train_eval('samples/model_config/multi_tower_export.config', test_dir=test_dir)
        )
    ...
    def test_big_model_embedding_variable_export(self):
        ...
        self.assertTrue(
            test_utils.test_single_train_eval(
                'samples/model_config/taobao_fg_ev.config', test_dir=test_dir, total_steps=1000
            )
        )
    ...
    """ % (
            config_path,
            'data/test/rtp/taobao_valid_feature.txt',
            result_path,
        )
        proc = test_utils.run_cmd(predict_cmd % (), '%s/log_%s.txt' % (test_dir, 'predict'))
        proc.wait()
        ...
        self._predict_and_check(test_data_path, export_dir, cmp_result, separator='', with_lbl=False)
