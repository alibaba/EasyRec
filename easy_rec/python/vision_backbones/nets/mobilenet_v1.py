...
def get_scopes_of_levels(scope,
                         conv_defs=None,
                         with_logits=True):
  """
  Args:
    scope: scope name for mobilenet_v1.
    conv_defs: A list of ConvDef namedtuples specifying the net architecture.
  ...
  scopes_of_levels_reverse = []
  scopes_of_curr_level = []

  for i, conv_def in enumerate(conv_defs):
    end_point_base = scope + '/Conv2d_%d' % i
    if conv_def.stride > 1:
      if len(scopes_of_curr_level) > 0:
        scopes_of_levels_reverse.append(scopes_of_curr_level)
      scopes_of_curr_level = []

    if isinstance(conv_def, Conv):
      scopes_of_curr_level.append(end_point_base)

    elif isinstance(conv_def, DepthSepConv):
      scopes_of_curr_level.extend([
  ...
  if with_logits:
    return [[scope + "/Logits"]] + list(reversed(scopes_of_levels_reverse))
  else:
    return list(reversed(scopes_of_levels_reverse))
