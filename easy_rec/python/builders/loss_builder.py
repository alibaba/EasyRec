...
def build_kd_loss(kds, prediction_dict, label_dict):
    ...
    for kd in kds:
        ...
        if kd.loss_type == LossType.CROSS_ENTROPY_LOSS:
            loss_dict[loss_name] = tf.losses.log_loss(label, pred, weights=kd.loss_weight)
        elif kd.loss_type == LossType.L2_LOSS:
            loss_dict[loss_name] = tf.losses.mean_squared_error(labels=label, predictions=pred, weights=kd.loss_weight)
        else:
            assert False, 'unsupported loss type for kd: %s' % LossType.Name(kd.loss_type)
    return loss_dict
