...
class BinaryDataset:
  def __init__(
    ...
    self._compute_global_start_pos(total_sample_num, batch_size, global_rank, global_size, drop_last)
    ...
    for tmp_file_id in range(self._start_file_id, self._end_file_id + 1):
      self._label_file_arr[tmp_file_id] = os.open(label_bins[tmp_file_id], os.O_RDONLY)
      self._dense_file_arr[tmp_file_id] = os.open(dense_bins[tmp_file_id], os.O_RDONLY)
      self._category_file_arr[tmp_file_id] = os.open(category_bins[tmp_file_id], os.O_RDONLY)

    self._prefetch = min(prefetch, self._num_entries)
    self._prefetch_queue = queue.Queue()
    self._executor = concurrent.futures.ThreadPoolExecutor(max_workers=self._prefetch)

    self._os_close_func = os.close

  def _compute_global_start_pos(self, total_sample_num, batch_size, global_rank, global_size, drop_last):
    ...
    logging.info('num_batches = %d num_samples = %d' % (self._num_entries, self._num_samples))
    ...
    logging.info('start_file_id = %d start_file_pos = %d' % (start_file_id, self._start_file_pos))
    ...
    while batch_id < self._num_entries:
      ...
      if batch_id == self._num_entries:
        ...
        while start_file_id < len(self._sample_num_arr) and tmp_start_pos > self._sample_num_arr[start_file_id]:
    ...
    logging.info('end_file_id = %d end_file_pos = %d' % (self._end_file_id, self._end_file_pos))
  ...
  def _get(self, idx):
    ...
    dense_read_arr = []
    cate_read_arr = []
    while total_read_num < self._batch_size and curr_file_id < len(self._sample_num_arr):
      tmp_read_num = min(end_read_pos, self._sample_num_arr[curr_file_id]) - start_read_pos

      label_raw_data = os.pread(self._label_file_arr[curr_file_id], 4 * tmp_read_num, start_read_pos * 4)
      tmp_lbl_np = np.frombuffer(label_raw_data, dtype=np.int32).reshape([tmp_read_num, 1])
      ...
      part_dense_np = np.frombuffer(dense_raw_data, dtype=np.float32).reshape([tmp_read_num, 13])
      ...
      part_cate_np = np.frombuffer(category_raw_data, dtype=np.uint32).reshape([tmp_read_num, 26])
...
if __name__ == '__main__':
  ...
  parser.add_argument('--prefetch_size', type=int, default=10, help='prefetch size')
  ...
  logging.info('total_steps = %d total_time = %.3f' % (step + 1, time.time() - start_time))
  logging.info(
