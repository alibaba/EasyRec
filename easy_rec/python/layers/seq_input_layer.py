...
class SeqInputLayer(object):
  def __init__(
    ...
    self._fc_parser = FeatureColumnParser(feature_configs, wide_and_deep_dict, ev_params=ev_params)
  ...
  def __call__(
    ...
    with variable_scope.variable_scope(scope_name, reuse=variable_scope.AUTO_REUSE), ops.name_scope(scope_name + '/'):
      key_tensors = []
      hist_tensors = []
      check_op_list = []
      for x in feature_dict.seq_att_map:
        for key in x.key:
          if key not in feature_name_to_output_tensors or (
            ...
            with variable_scope.variable_scope(qfc._var_scope_name):
              tmp_key_tensor = feature_column_dict[key]._get_dense_tensor(builder)
              regularizers.apply_regularization(
                self._embedding_regularizer,
                weights_list=[tmp_key_tensor],
              )
        ...
        for hist_seq in x.hist_seq:
          ...
          with variable_scope.variable_scope(seq_fc._var_scope_name):
            cur_hist_seqs.append(feature_column_dict[hist_seq]._get_sequence_dense_tensor(builder))
        ...
        for aux_hist_seq in x.aux_hist_seq:
          ...
          with variable_scope.variable_scope(seq_fc._var_scope_name):
            aux_hist_embedding, _ = feature_column_dict[aux_hist_seq]._get_sequence_dense_tensor(builder)
