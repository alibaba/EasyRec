...
class OdpsRTPInput(Input):
    ...
    def __init__(self, data_config, feature_config, input_path, task_index=0, task_num=1):
        super(OdpsRTPInput, self).__init__(data_config, feature_config, input_path, task_index, task_num)
    ...
    def _build(self, mode, params):
        ...
        if self._data_config.pai_worker_queue and mode == tf.estimator.ModeKeys.TRAIN:
            logging.info('pai_worker_slice_num = %d' % self._data_config.pai_worker_slice_num)
            ...
            dataset = tf.data.TableRecordDataset(
                que_paths, record_defaults=record_defaults, selected_cols=selected_cols
            )
        else:
            dataset = tf.data.TableRecordDataset(
                self._input_path,
