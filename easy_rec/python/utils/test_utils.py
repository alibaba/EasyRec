...
def run_cmd(cmd_str, log_file):
    ...
    with open(log_file, 'w') as lfile:
        return subprocess.Popen(cmd_str.split(), stdout=lfile, stderr=subprocess.STDOUT)
...
def test_datahub_train_eval(
    pipeline_config_path, test_dir, process_pipeline_func=None, hyperparam_str='', total_steps=50, post_check_func=None
):
...
def yaml_replace(train_yaml_path, pipline_config_path, test_pipeline_config_path, test_export_dir=None):
...
def test_hdfs_train_eval(
    pipeline_config_path, train_yaml_path, test_dir, process_pipeline_func=None, hyperparam_str='', total_steps=2000
):
...
def test_hdfs_eval(pipeline_config_path, eval_yaml_path, test_dir, process_pipeline_func=None, hyperparam_str=''):
...
def test_hdfs_export(pipeline_config_path, export_yaml_path, test_dir, process_pipeline_func=None, hyperparam_str=''):
    ...
    yaml_replace(export_yaml_path, pipeline_config_path, test_pipeline_config_path, test_export_path)
    logging.info('test_pipeline_config_path is %s' % test_pipeline_config_path)
