...
class WideAndDeep(RankModel):
  def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
    ...
    assert model_config.WhichOneof('model') == 'wide_and_deep', 'invalid model config: %s' % model_config.WhichOneof(
      'model'
    )
  ...
  def get_grouped_vars(self, opt_num):
    ...
    if opt_num == 2:
      ...
      for tmp_var in tf.trainable_variables():
        if tmp_var.name.startswith('input_layer') and (not tmp_var.name.startswith('input_layer_1')):
      ...
      for tmp_var in tf.trainable_variables():
        if tmp_var.name.startswith('input_layer') and (not tmp_var.name.startswith('input_layer_1')):
          wide_vars.append(tmp_var)
        elif tmp_var.name.startswith('input_layer') or '/embedding_weights' in tmp_var.name:
