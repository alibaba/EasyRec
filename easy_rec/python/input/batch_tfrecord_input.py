...
class BatchTFRecordInput(Input):
    ...
    def __init__(self, data_config, feature_config, input_path, task_index=0, task_num=1):
        super(BatchTFRecordInput, self).__init__(data_config, feature_config, input_path, task_index, task_num)
        assert data_config.HasField('n_data_batch_tfrecord'), 'Need to set n_data_batch_tfrecord in config.'
        self._input_shapes = [x.input_shape for x in data_config.input_fields]
        self.feature_desc = {}
        for x, t, d, s in zip(
            self._input_fields, self._input_field_types, self._input_field_defaults, self._input_shapes
        ):
            d = self.get_type_defaults(t, d)
            t = self.get_tf_type(t)
            self.feature_desc[x] = tf.io.FixedLenSequenceFeature(dtype=t, shape=s, allow_missing=True)

    def _parse_tfrecord(self, example):
        try:
            _, features, _ = tf.parse_sequence_example(example, sequence_features=self.feature_desc)
        except AttributeError:
            _, features, _ = tf.io.parse_sequence_example(example, sequence_features=self.feature_desc)
        ...
        features = dict(
            (
                ...
                tf.reshape(
                    ...
                    + [x for i, x in enumerate(value.shape) if i not in (0, 1)],
                ),
            )
    ...
    def _build(self, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            dataset = tf.data.TFRecordDataset(file_paths, compression_type=data_compression_type)
