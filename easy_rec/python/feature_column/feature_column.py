...
from easy_rec.python.compat.feature_column import (  # NOQA
    feature_column_v2 as feature_column,
...
class FeatureColumnParser(object):
    ...
    def __init__(self, feature_configs, wide_deep_dict={}, wide_output_dim=-1, use_embedding_variable=False):
        ...
        for config in self._feature_configs:
            ...
            embed_info = {
                ...
                'initializer': config.initializer if config.HasField('initializer') else None,
                'max_partitions': config.max_partitions,
            }
            if embed_name in self._share_embed_names:
                assert (
                    embed_info == self._share_embed_infos[embed_name]
        ...
        for embed_name in self._share_embed_names:
            logging.info(
                ...
                % (embed_name, self._share_embed_names[embed_name], self._share_embed_infos[embed_name])
            )
        ...
        for embed_name in self._share_embed_names:
            ...
            partitioner = self._build_partitioner(self._share_embed_infos[embed_name]['max_partitions'])
    ...
    def parse_id_feature(self, config):
        ...
        if hash_bucket_size > 0:
            ...
            fc = feature_column.categorical_column_with_vocabulary_list(
                config.input_names[0], default_value=0, vocabulary_list=config.vocab_list
            )
    ...
    def parse_tag_feature(self, config):
        ...
        if len(config.input_names) > 1:
            ...
            tag_fc = feature_column.weighted_categorical_column(tag_fc, weight_feature_key=wgt_name, dtype=tf.float32)
    ...
    def parse_raw_feature(self, config):
        ...
        fc = feature_column.numeric_column(config.input_names[0], shape=(config.raw_input_dim,))
        ...
        if bounds:
            try:
                ...
                tf.logging.error('bucketized_column [%s] with bounds %s error' % (fc.name, str(bounds)))
                raise e
            if self.is_wide(config):
                self._add_wide_embedding_column(fc, config)
            if self.is_deep(config):
            ...
            tmp_id_col = feature_column.categorical_column_with_identity(
                config.input_names[0] + '_raw_proj_id', config.raw_input_dim, default_value=0
            )
            wgt_fc = feature_column.weighted_categorical_column(
                tmp_id_col, weight_feature_key=config.input_names[0] + '_raw_proj_val', dtype=tf.float32
            )
            if self.is_wide(config):
    ...
    def parse_combo_feature(self, config):
        ...
        fc = feature_column.crossed_column(config.input_names, config.hash_bucket_size, hash_key=None)
    ...
    def parse_lookup_feature(self, config):
        ...
        hash_bucket_size = config.hash_bucket_size
        fc = feature_column.categorical_column_with_hash_bucket(feature_name, hash_bucket_size, dtype=tf.string)
