...
from easy_rec.python.main import _train_and_evaluate_impl as train_and_evaluate_impl
...
tf.app.flags.DEFINE_string('cmd', 'train', 'command type, train/evaluate/export')
tf.app.flags.DEFINE_string('tables', '', 'tables passed by pai command')

# flags for train
tf.app.flags.DEFINE_integer('num_gpus_per_worker', 1, 'number of gpu to use in training')
tf.app.flags.DEFINE_boolean('with_evaluator', False, 'whether a evaluator is necessary')
...
tf.app.flags.DEFINE_string('distribute_strategy', '', 'training distribute strategy')
...
tf.app.flags.DEFINE_string(
  'checkpoint_path',
  '',
  'checkpoint to be evaluated or exported ' 'if not specified, use the latest checkpoint ' 'in train_config.model_dir',
)
# flags used for evaluate
tf.app.flags.DEFINE_string('eval_result_path', 'eval_result.txt', 'eval result metric file')
tf.app.flags.DEFINE_bool('distribute_eval', False, 'use distribute parameter server for train and eval.')
# flags used for export
tf.app.flags.DEFINE_string('export_dir', '', 'directory where model should be exported to')
...
tf.app.flags.DEFINE_integer('max_wait_ckpt_ts', 0, 'max wait time in seconds for checkpoints')
tf.app.flags.DEFINE_boolean('continue_train', True, 'use the same model to continue train or not')

# flags used for predict
tf.app.flags.DEFINE_string('saved_model_dir', '', 'directory where saved_model.pb exists')
tf.app.flags.DEFINE_string('outputs', '', 'output tables')
tf.app.flags.DEFINE_string('all_cols', '', 'union of (selected_cols, reserved_cols), separated with , ')
tf.app.flags.DEFINE_string(
  'all_col_types',
  '',
  'column data types, for build record defaults, separated with ,',
)
tf.app.flags.DEFINE_string('selected_cols', '', 'columns to keep from input table,  they are separated with ,')
tf.app.flags.DEFINE_string('reserved_cols', '', 'columns to keep from input table,  they are separated with ,')
tf.app.flags.DEFINE_string(
  'output_cols',
  None,
...
tf.app.flags.DEFINE_string('profiling_file', None, 'time stat file which can be viewed using chrome tracing')
...
tf.app.flags.DEFINE_integer('redis_batch_size', 1024, 'export to redis batch_size')
...
tf.app.flags.DEFINE_bool('place_embedding_on_cpu', False, 'whether to place embedding variables on cpu')
...
tf.app.flags.DEFINE_string('hpo_param_path', None, 'hyperparameter tuning param path')
tf.app.flags.DEFINE_string('hpo_metric_save_path', None, 'hyperparameter save metric path')
...
tf.app.flags.DEFINE_bool('enable_avx_str_split', False, 'enable avx str split to speedup')
...
def set_selected_cols(pipeline_config, selected_cols, all_cols, all_col_types):
  if selected_cols:
    ...
    if all_cols:
      ...
      all_col_types_map = {x.strip(): y.strip() for x, y in zip(all_cols_arr, all_col_types_arr)}
  ...
  print('[run.py] data_config.selected_cols = "%s"' % pipeline_config.data_config.selected_cols)
  print('[run.py] data_config.selected_col_types = "%s"' % pipeline_config.data_config.selected_col_types)
...
def main(argv):
  ...
  if FLAGS.enable_avx_str_split:
    ...
    logging.info('will enable avx str split: %s' % constant.is_avx_str_split_enabled())
  ...
  if FLAGS.config:
    config = pai_util.process_config(FLAGS.config, FLAGS.task_index, len(FLAGS.worker_hosts.split(',')))
  ...
  if FLAGS.model_dir:
    ...
    assert pipeline_config.model_dir.startswith('oss://'), 'invalid model_dir format: %s' % pipeline_config.model_dir
  ...
  if FLAGS.clear_model:
    if gfile.IsDirectory(pipeline_config.model_dir) and estimator_utils.is_chief():
  ...
  if FLAGS.cmd == 'train':
    ...
    if not FLAGS.train_tables and FLAGS.tables:
      ...
      assert len(tables) >= 2, 'at least 2 tables must be specified, but only[%d]: %s' % (
        len(tables),
        FLAGS.tables,
      )
    ...
    if FLAGS.boundary_table:
      ...
      config_util.add_boundaries_to_config(pipeline_config, FLAGS.boundary_table)
    ...
    assert FLAGS.eval_method in ['none', 'master', 'separate'], 'invalid evalaute_method: %s' % FLAGS.eval_method
    ...
    set_distribution_config(pipeline_config, num_worker, num_gpus_per_worker, distribute_strategy)
    ...
    if not FLAGS.distribute_eval:
      assert len(FLAGS.worker_hosts.split(',')) == 1, 'evaluate only need 1 worker'
    config_util.auto_expand_share_feature_configs(pipeline_config)
    ...
    if FLAGS.distribute_eval:
      ...
      easy_rec.distribute_evaluate(pipeline_config, FLAGS.checkpoint_path, None, FLAGS.eval_result_path)
    else:
      os.environ['distribute_eval'] = 'False'
      logging.info('will_use_eval')
      distribute_eval = os.environ.get('distribute_eval')
      logging.info('distribute_eval = {}'.format(distribute_eval))
      easy_rec.evaluate(pipeline_config, FLAGS.checkpoint_path, None, FLAGS.eval_result_path)
    ...
    logging.info('will use the following columns as model input: %s' % FLAGS.selected_cols)
    logging.info('will copy the following columns to output: %s' % FLAGS.reserved_cols)
    ...
    if not query_table:
      tables = FLAGS.tables.split(',')
      assert len(tables) >= 1, 'at least 1 tables must be specified, but only[%d]: %s' % (
        len(tables),
        FLAGS.tables,
      )
    ...
    raise ValueError('cmd should be one of train/evaluate/export/predict/export_checkpoint/vector_retrieve')
