...
def jrc_loss(
  ...
  logging.info('[{}] alpha: {}, loss_weight_strategy: {}'.format(loss_name, alpha, loss_weight_strategy))

  ce_loss = tf.losses.sparse_softmax_cross_entropy(labels, logits, weights=sample_weights)
  ...
  if loss_weight_strategy == 'fixed':
    ...
    raise ValueError('Unsupported loss weight strategy `%s` for jrc loss' % loss_weight_strategy)
