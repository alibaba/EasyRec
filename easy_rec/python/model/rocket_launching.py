...
from easy_rec.python.protos.rocket_launching_pb2 import (  # NOQA
    RocketLaunching as RocketLaunchingConfig,
)
...
class RocketLaunching(RankModel):
    def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
        super(RocketLaunching, self).__init__(model_config, feature_configs, features, labels, is_training)
        assert (
            self._model_config.WhichOneof('model') == 'rocket_launching'
    ...
    def sim(self, feature_emb1, feature_emb2):
        emb1_emb2_sim = tf.reduce_sum(tf.multiply(feature_emb1, feature_emb2), axis=1, keepdims=True)
    ...
    def feature_based_sim(self, feature_based_distillation, i, j):
        booster_feature_no_gradient = tf.stop_gradient(self.booster_feature['hidden_layer' + str(j)])
        if feature_based_distillation == 'COSION':
            booster_feature_no_gradient_norm = self.norm(booster_feature_no_gradient)
            light_feature_norm = self.norm(self.light_feature['hidden_layer' + str(i)])
            sim_middle_layer = tf.reduce_mean(self.sim(booster_feature_no_gradient_norm, light_feature_norm))
            return sim_middle_layer
        else:
            return tf.sqrt(
                tf.reduce_sum(tf.square(booster_feature_no_gradient - self.light_feature['hidden_layer' + str(i)]))
            )
    ...
    def build_predict_graph(self):
        ...
        if self._model_config.HasField('share_dnn'):
            share_dnn_layer = dnn.DNN(self._model_config.share_dnn, self._l2_reg, 'share_dnn', self._is_training)
            share_feature = share_dnn_layer(self._features)
        booster_dnn_layer = dnn.DNN(self._model_config.booster_dnn, self._l2_reg, 'booster_dnn', self._is_training)
        light_dnn_layer = dnn.DNN(self._model_config.light_dnn, self._l2_reg, 'light_dnn', self._is_training)
        if self._model_config.HasField('share_dnn'):
            self.booster_feature = booster_dnn_layer(share_feature, self.hidden_layer_feature_output)
            input_embedding_stop_gradient = tf.stop_gradient(share_feature)
            self.light_feature = light_dnn_layer(input_embedding_stop_gradient, self.hidden_layer_feature_output)
        else:
            self.booster_feature = booster_dnn_layer(self._features, self.hidden_layer_feature_output)
        ...
        if self._model_config.feature_based_distillation:
            ...
            booster_out = tf.layers.dense(
                self.booster_feature, self._num_class, kernel_regularizer=self._l2_reg, name='booster_output'
            )

            light_out = tf.layers.dense(
                self.light_feature, self._num_class, kernel_regularizer=self._l2_reg, name='light_output'
            )
    ...
    def build_loss_graph(self):
            ...
            for i, unit_i in enumerate(light_hidden_units):
                for j, unit_j in enumerate(booster_hidden_units):
                    if light_hidden_units[i] == booster_hidden_units[j]:
                        self._prediction_dict['similarity_' + str(count)] = self.feature_based_sim(
        ...
        self._loss_dict['hint_loss'] = loss_builder.build(
            LossType.L2_LOSS, label=booster_logits_no_grad, pred=logits_light, loss_weight=self._sample_weight
        )
    ...
    def get_outputs(self):
        return ['logits_booster', 'probs_booster', 'y_booster', 'logits_light', 'probs_light', 'y_light']
