...
class InputLayer(object):
    ...
    def __init__(
        ...
        self._seq_feature_groups_config = [
            x.sequence_features for x in feature_groups_config if x.HasField('sequence_features')
        ]
        self._group_name_to_seq_features = {
            x.group_name: x.sequence_features for x in feature_groups_config if x.HasField('sequence_features')
        }
        self._seq_input_layer = None
        if len(self._seq_feature_groups_config) > 0:
            self._seq_input_layer = seq_input_layer.SeqInputLayer(feature_configs, self._seq_feature_groups_config)
        wide_and_deep_dict = self.get_wide_deep_dict()
        self._fc_parser = FeatureColumnParser(
            feature_configs, wide_and_deep_dict, wide_output_dim, use_embedding_variable=use_embedding_variable
        )
    ...
    def target_attention(self, dnn_config, deep_fea, name):
        ...
        din_net = tf.concat(
            [cur_ids, hist_id_col, cur_ids - hist_id_col, cur_ids * hist_id_col], axis=-1
    ...
    def call_seq_input_layer(self, features, seq_att_map_config, feature_name_to_output_tensors=None):
        group_name = seq_att_map_config.group_name
        allow_key_search = seq_att_map_config.allow_key_search
        seq_features = self._seq_input_layer(features, group_name, feature_name_to_output_tensors, allow_key_search)
        regularizers.apply_regularization(self._embedding_regularizer, weights_list=[seq_features['key']])
        regularizers.apply_regularization(self._embedding_regularizer, weights_list=[seq_features['hist_seq_emb']])
        seq_dnn_config = None
        ...
        seq_fea = self.target_attention(seq_dnn_config, seq_features, name='seq_dnn')
        return seq_fea
        ...
        if is_combine:
            ...
            if group_name in self._group_name_to_seq_features:
                seq_fea = self.call_seq_input_layer(
                    features, self._group_name_to_seq_features[group_name], feature_name_to_output_tensors
                )
    ...
    def single_call_input_layer(self, features, group_name, is_combine=True, feature_name_to_output_tensors=None):
        """Get features by group_name.
        ...
        if is_combine:
            ...
            for column in sorted(group_seq_columns, key=lambda x: x.name):
                with tf.variable_scope(None, default_name=column._var_scope_name):
                    ...
                    if sequence_combiner is None:
                        raise ValueError('sequence_combiner is none, please set sequence_combiner or use TagFeature')
                    if sequence_combiner.WhichOneof('combiner') == 'attention':
                        ...
                        attn_score = tf.nn.softmax(tf.where(seq_mask, attn_logits, attn_logits_padding))
                        seq_feature = tf.reduce_sum(attn_score[:, :, tf.newaxis] * seq_feature, axis=1)
            ...
            regularizers.apply_regularization(self._embedding_regularizer, weights_list=embedding_reg_lst)
            ...
            assert len(group_columns) == 0, 'there are none sequence columns: %s' % str(group_columns)
            builder = feature_column._LazyBuilder(features)
            seq_features = []
            embedding_reg_lst = []
