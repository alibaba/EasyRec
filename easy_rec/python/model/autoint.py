...
from easy_rec.python.protos.autoint_pb2 import AutoInt as AutoIntConfig
...
class AutoInt(RankModel):
    def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
        super(AutoInt, self).__init__(model_config, feature_configs, features, labels, is_training)
        assert (
            self._model_config.WhichOneof('model') == 'autoint'
        ...
        if self._model_config.feature_groups[0].HasField('sequence_features'):
            self._feature_num += len(self._model_config.feature_groups[0].sequence_features.seq_att_map[0].hist_seq)
            self._seq_key_num = len(self._model_config.feature_groups[0].sequence_features.seq_att_map[0].key)
        ...
        assert (
            len(set(fea_emb_dim_list)) == 1 and len(fea_emb_dim_list) == self._feature_num
    ...
    def build_predict_graph(self):
        ...
        attention_fea = tf.reshape(self._features, shape=[-1, self._feature_num + self._seq_key_num, self._d_model])
        ...
        attention_fea = tf.reshape(attention_fea, shape=[-1, attention_fea.shape[1] * attention_fea.shape[2]])
