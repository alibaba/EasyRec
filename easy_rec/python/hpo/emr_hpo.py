...
def hpo_config(
  config_path, hyperparams, exp_dir, metric_name, el_submit_params
):
  earlystop = {
  ...
  assert len(el_params
            ) % 2 == 0, 'invalid number of el_submit params: %d[%s]' % (
  ...
  cmd = ['el_submit'] + el_params + [
    '-a', 'easy_rec_hpo', '-m', 'local', '-f',
    '{},train_eval.py,{}'.format(config_path,
                                 param_path), '--interact', 'INTERACT', '-c',
    'python -m easy_rec.python.train_eval --hpo_metric_save_path {} '
    '--hpo_param_path {} --pipeline_config_path {} --model_dir {}'.
    format(metric_path, param_path_file, config_path, model_path)
  ]
...
if __name__ == '__main__':
  ...
  parser.add_argument(
    ...
    type=str,
    help=
    'el_submit parameters(-t x -m x [-pn x -pc x -pm x] -wn x -wc x -wm x -wg x)',
  ...
  data, tmp_dir = hpo_config(
    args.config_path, hyperparams, args.exp_dir, args.metric_name,
