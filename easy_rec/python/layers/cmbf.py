...
class CMBF(object):
  ...
  def __init__(self, model_config, feature_configs, features, cmbf_config, input_layer):
    ...
    for fea_group in model_config.feature_groups:
      if fea_group.group_name == 'general':
        ...
        assert self._txt_feature_num == len(
    ...
    print(
      'txt_feature_num: {0}, img_feature_num: {1}, txt_seq_feature_num: {2}'.format(
  ...
  def image_self_attention_tower(self):
    ...
    if img_fea_num > 1:  # in case of video frames or ROIs (Region Of Interest)
      ...
      if self._img_patch_num > 1:  # image feature dimension: patch_num * emb_size
        ...
        assert (
          img_emb_size * self._img_patch_num == self._img_emb_size
        ...
        if img_fea_num != self._img_emb_size:
          image_features = tf.layers.dense(image_features, img_fea_num, name='img_projection')
  ...
  def __call__(self, is_training, *args, **kwargs):
    ...
    if None not in [img_attention_fea, txt_attention_fea]:
      (
        img_embeddings,
        txt_embeddings,
      ) = multihead_cross_attention.cross_attention_tower(
        img_attention_fea,
        txt_attention_fea,
      ...
      # img_emb shape: [batch_size, image_(region_)num/image_feature_dim, multi_head_num * image_cross_head_size]
      print('img_embeddings:', img_embeddings.shape)
      # txt_emb shape: [batch_size, general_feature_num + max_txt_seq_len, multi_head_num * text_cross_head_size]
