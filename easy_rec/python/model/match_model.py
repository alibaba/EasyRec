  ...
  def _mask_in_batch(self, logits):
    ...
    if getattr(self._model_config, 'ignore_in_batch_neg_sam', False):
      in_batch = logits[:, :batch_size] - (
        1 - tf.diag(tf.ones([batch_size], dtype=tf.float32))
      ) * 1e32
      ...
      if self._item_ids is not None:
        mask_in_batch_neg = tf.to_float(
          tf.equal(
            self._item_ids[None, :batch_size], self._item_ids[:batch_size,
                                                              None]
          )
        ) - tf.diag(tf.ones([batch_size], dtype=tf.float32))
        tf.summary.scalar(
          'in_batch_neg_conflict', tf.reduce_sum(mask_in_batch_neg)
        )
        return tf.concat([
  ...
  def _list_wise_sim(self, user_emb, item_emb):
    ...
    if hard_neg_indices is None:
      ...
      hard_neg_sim = tf.scatter_nd(
        hard_neg_indices, hard_neg_user_item_sim, hard_neg_shape
      )
    ...
    if self._loss_type == LossType.SOFTMAX_CROSS_ENTROPY:
      ...
      if all(
        ...
        self._loss_dict['amm_loss_u'
                       ] = self._model_config.amm_u_weight * tf.reduce_mean(
                         tf.square(
                           self._prediction_dict['augmented_a_u'] -
        ...
        self._loss_dict[
          'amm_loss_i'] = self._model_config.amm_i_weight * tf.reduce_mean(
            tf.square(
              self._prediction_dict['augmented_a_i'][:batch_size] -
  ...
  def _build_list_wise_metric_graph(self, eval_config):
    ...
    for metric in eval_config.metrics_set:
      if metric.WhichOneof('metric') == 'recall_at_topk':
        metric_dict[
          'recall@%d' % metric.recall_at_topk.topk
        ] = metrics_tf.recall_at_k(label, logits, metric.recall_at_topk.topk)

        logits_v2 = tf.concat(
          [pos_item_sim[:, None], logits[:, batch_size:]], axis=1
        )
        labels_v2 = tf.zeros_like(logits_v2[:, :1], dtype=tf.int64)
        metric_dict['recall_neg_sam@%d' % metric.recall_at_topk.topk
                   ] = metrics_tf.recall_at_k(
                     labels_v2, logits_v2, metric.recall_at_topk.topk
                   )

        metric_dict['recall_in_batch@%d' % metric.recall_at_topk.topk
                   ] = metrics_tf.recall_at_k(
  ...
  def _build_point_wise_metric_graph(self, eval_config):
    ...
    for metric in eval_config.metrics_set:
      if metric.WhichOneof('metric') == 'auc':
        ...
        metric_dict['auc'] = metrics_tf.auc(
