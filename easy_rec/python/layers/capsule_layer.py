...
class CapsuleLayer:
  ...
  def squash(self, inputs):
    ...
    scale_factor = (
      tf.pow(input_norm_eps / (1 + input_norm_eps), self._squash_pow) * self._scale_ratio / tf.sqrt(input_norm_eps)
    )
  ...
  def _build_capsule_simi(self, high_capsules, capsule_num):
    ...
    avg_simi = tf.reduce_sum((simi + 1) * is_multi) / (2.0 * tf.reduce_sum(is_multi))
    return avg_simi
  ...
  def __call__(self, seq_feas, seq_lens):
    ...
    if self._const_caps_num:
      ...
      logging.info('will use log(seq_len) number of capsules, max_capsules: %d' % self._max_k)
    ...
    for iter_id in range(self._num_iters):
      ...
      if self._routing_logits_scale > 0:
        ...
        routing_logits = tf.einsum('bse, bhe->bsh', seq_feas_high_norm, high_capsules) * self._routing_logits_scale
      else:
