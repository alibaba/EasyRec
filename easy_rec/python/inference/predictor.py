...
class EmbeddingPredictor(Predictor):
  ...
  def __init__(self,
               ...
               selected_cols=None,
               output_sep=' '):
    super(EmbeddingPredictor, self).__init__(model_path, profiling_file, fg_json_path)
    self._output_sep = output_sep
    input_type = DatasetConfig.InputType.Name(data_config.input_type).lower()
    self._with_header = data_config.with_header
  ...
  def _get_reserved_cols(self, reserved_cols):
    reserved_cols = CSVPredictor(self._model_path,self._data_config)._get_reserved_cols(reserved_cols)
    return reserved_cols
  ...
  def _get_num_cols(self, file_paths):
    CSVPredictor(self._model_path,self._data_config)._get_num_cols(file_paths)
  ...
  def _get_writer(self, output_path, slice_id):
    if not gfile.Exists(output_path):
      gfile.MakeDirs(output_path)
    res_path = os.path.join(output_path, '%s.%s'%(self._emb_name,self._vec_engine))
    table_writer = gfile.GFile(res_path, 'w')
    return table_writer
  ...
  def _write_lines(self, table_writer, outputs):
    ...
    for output in outputs:
      if self._vec_engine == 'holo' or self._vec_engine == 'mysql':
        res = "%s %s" % (output[0], output[1])
        tmp_output.append(res)
      elif self._vec_engine == 'faiss':
        res = [output[0], ]
        res.extend(["%s:%s"%(idx+1, emb) for idx, emb in enumerate(output[1].split(','))])
        tmp_output.append(' '.join(res))
    outputs = '\n'.join(
        [self._output_sep.join([str(i) for i in output]) for output in tmp_output])
    table_writer.write(outputs + '\n')

  def _get_reserve_vals(self, reserved_cols, output_cols, all_vals, outputs):
    output_cols = []
    output_cols.append(self._emb_name)
