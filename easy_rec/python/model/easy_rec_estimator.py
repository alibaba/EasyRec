...
class EasyRecEstimator(tf.estimator.Estimator):
  ...
  def evaluate(
    ...
    return super(EasyRecEstimator,
                 self).evaluate(input_fn, steps, hooks, checkpoint_path, name)
  ...
  def train(
    ...
    return super(EasyRecEstimator, self
                ).train(input_fn, hooks, steps, max_steps, saving_listeners)
  ...
  def feature_configs(self):
    if len(self._pipeline_config.feature_configs) > 0:
      ...
      self._pipeline_config.feature_config.features
    ) > 0:
  ...
  def _train_model_fn(self, features, labels, run_config):
    ...
    if Input.DATA_OFFSET in features:
      ...
      update_offset = tf.assign(
        data_offset_var[task_index], features[Input.DATA_OFFSET]
      )
      ...
      for x in update_ops:
        if isinstance(x, ops.Operation) and x.inputs[0].name in global_vars:
          ops.add_to_collection(
            constant.DENSE_UPDATE_VARIABLES, global_vars[x.inputs[0].name]
          )
    ...
    if self.train_config.sync_replicas and run_config.num_worker_replicas > 1:
      logging.info(
        'sync_replicas: num_worker_replias = %d' %
    ...
    if run_config.num_worker_replicas > 1 and \
      ...
      hooks.append(
        estimator_utils.ExitBarrierHook(
          run_config.num_worker_replicas, run_config.is_chief, self.model_dir
        )
      )
    ...
    train_op = optimizers.optimize_loss(
      ...
      not_apply_grad_after_first_step=run_config.is_chief
      and self._pipeline_config.data_config.chief_redundant,
  ...
  def _eval_model_fn(self, features, labels, run_config):
    ...
    tf.logging.info(
      'eval graph construct finished. Time %.3fs' % (end - start)
    )
    return tf.estimator.EstimatorSpec(
  ...
  def _export_model_fn(self, features, labels, run_config, params):
    ...
    dense_train_var_path = os.path.join(
      self.model_dir, constant.DENSE_UPDATE_VARIABLES
    )
    if gfile.Exists(dense_train_var_path):
      with gfile.GFile(dense_train_var_path, 'r') as fin:
        ...
        for var_name, var_id in var_name_id_lst:
          ...
          ops.add_to_collection(
            constant.DENSE_UPDATE_VARIABLES, all_vars[var_name]
          )
    ...
    scaffold = tf.train.Scaffold(
      saver=self.
      saver_cls(var_list=var_list, sharded=True, save_relative_paths=True)
    )
