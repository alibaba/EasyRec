...
from easy_rec.python.layers import fm, input_layer
...
class FM(RankModel):
    def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
        super(FM, self).__init__(model_config, feature_configs, features, labels, is_training)
        assert (
            self._model_config.WhichOneof('model') == 'fm'
    ...
    def build_predict_graph(self):
        ...
        if self._num_class > 1:
            fm_fea = tf.layers.dense(fm_fea, self._num_class, kernel_regularizer=self._l2_reg, name='fm_logits')
        else:
            fm_fea = tf.reduce_sum(fm_fea, 1, keepdims=True)

        bias = tf.get_variable('fm_bias', [self._num_class], initializer=tf.zeros_initializer(), trainable=True)
