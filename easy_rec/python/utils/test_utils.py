...
def proc_wait(proc, timeout=1200):
  ...
  if proc.poll() is None:
    logging.warning(
      'proc[pid=%d] timeout[%d], will kill the proc' % (proc.pid, timeout)
    )
    proc.terminate()
...
def _load_config_for_test(
  pipeline_config_path, test_dir, total_steps=50, num_epochs=0
):
...
def test_datahub_train_eval(
  ...
  if isinstance(pipeline_config_path, EasyRecConfig):
    ...
    pipeline_config = _load_config_for_test(
      pipeline_config_path, test_dir, total_steps
    )
  ...
  with open(train_yaml_path, 'r', encoding='utf-8') as _file:
    ...
    if test_export_dir is not None:
      _command = _command.replace(
        pipline_config_path, test_pipeline_config_path
      ).replace('{EXPOERT_DIR}', test_export_dir)
    else:
      _command = _command.replace(
        pipline_config_path, test_pipeline_config_path
      )
...
def test_hdfs_train_eval(
  ...
  test_pipeline_config_path = os.path.join(test_dir, 'pipeline.config')
  yaml_replace(
    train_yaml_path, pipeline_config_path, test_pipeline_config_path
  )
...
def test_hdfs_export(
  ...
  yaml_replace(
    export_yaml_path, pipeline_config_path, test_pipeline_config_path,
    test_export_path
  )
  logging.info('test_pipeline_config_path is %s' % test_pipeline_config_path)
...
def _ps_worker_train(
  ...
  for idx in range(num_worker - 1):
    ...
    procs[worker_name] = run_cmd(
      train_cmd, '%s/log_%s.txt' % (test_dir, worker_name)
    )
  if num_evaluator > 0:
    ...
    procs['evaluator'] = run_cmd(
      train_cmd, '%s/log_%s.txt' % (test_dir, 'evaluator')
    )

  return procs


def _ps_worker_distribute_eval(
  pipeline_config_path,
  checkpoint_path,
  test_dir,
  num_worker,
  ...
  set_gpu_id('')
  procs['ps'] = run_cmd(
    train_cmd, '%s/distribute_eval_log_%s.txt' % (test_dir, 'ps')
  )
...
def test_distributed_train_eval(
  ...
  logging.info('testing pipeline config %s' % pipeline_config_path)
  pipeline_config = _load_config_for_test(
    pipeline_config_path, test_dir, total_steps, num_epoch
  )
  ...
  try:
    ...
    if train_config.train_distribute == DistributionStrategy.NoStrategy:
      ...
      num_worker = 2
      procs = _multi_worker_mirror_train(
        test_pipeline_config_path, test_dir, num_worker
      )
    else:
...
def test_distribute_eval_test(cur_eval_path, test_dir):
  ...
  single_ret = {
    k: single_data[k]
    for k in single_data.keys() if 'loss' not in k and 'step' not in k
  }
  distribute_ret = {
    k: distribute_data[k]
    for k in distribute_data.keys() if 'loss' not in k
  }
...
def test_distributed_eval(
  ...
  logging.info('testing pipeline config %s' % pipeline_config_path)
  pipeline_config = _load_config_for_distribute_eval(
    pipeline_config_path, test_dir
  )
  train_config = pipeline_config.train_config
  config_util.save_pipeline_config(pipeline_config, test_dir)
  test_pipeline_config_path = os.path.join(test_dir, 'pipeline.config')
  ...
  try:
    if train_config.train_distribute == DistributionStrategy.NoStrategy:
      ...
      procs = _ps_worker_distribute_eval(
        test_pipeline_config_path, checkpoint_path, test_dir, num_worker,
        num_evaluator
      )
    else:
      raise NotImplementedError
