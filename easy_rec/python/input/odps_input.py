...
class OdpsInput(Input):
    def __init__(self, data_config, feature_config, input_path, task_index=0, task_num=1):
        super(OdpsInput, self).__init__(data_config, feature_config, input_path, task_index, task_num)
    ...
    def _build(self, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            if self._data_config.pai_worker_queue:
                ...
                file_queue = tf.train.string_input_producer(
                    self._input_path, num_epochs=self.num_epochs, capacity=1000, shuffle=self._data_config.shuffle
                )
        else:
            file_queue = tf.train.string_input_producer(self._input_path, num_epochs=1, capacity=1000, shuffle=False)
        ...
        fields = tf.decode_csv(
            value, record_defaults=record_defaults, field_delim=self._data_config.separator, name='decode_csv'
        )
