...
from easy_rec.python.compat import estimator_train, exporter
...
def _create_eval_export_spec(pipeline_config, eval_data, check_mode=False):
  ...
  if export_config.exporter_type == 'final':
    ...
    def _metric_cmp_fn(best_eval_result, current_eval_result):
      ...
      if export_config.metric_bigger:
        return (
          best_eval_result[export_config.best_exporter_metric] < current_eval_result[export_config.best_exporter_metric]
        )
      else:
        return (
          best_eval_result[export_config.best_exporter_metric] > current_eval_result[export_config.best_exporter_metric]
  ...
  eval_input_fn = _get_input_fn(data_config, feature_configs, eval_data, **input_fn_kwargs)
...
def _train_and_evaluate_impl(
  ...
  estimator, run_config = _create_estimator(pipeline_config, distribution=distribution, params=params)
  ...
  assert (
    train_steps is not None or data_config.num_epochs > 0
  ...
  if fit_on_eval and (not estimator_utils.is_evaluator()):
    ...
    if fit_on_eval_steps is not None:
      ...
      logging.info('\ttrain_steps=%d fit_on_eval_steps=%d' % (train_steps, fit_on_eval_steps))
    ...
    estimator.train(
      ...
      hooks=list(train_spec.hooks),
      saving_listeners=(train_spec.saving_listeners if hasattr(train_spec, 'saving_listeners') else None),
    )
...
def distribute_evaluate(
  ...
  assert tf.gfile.IsDirectory(eval_tmp_results_dir), 'tmp results dir not create success.'
  ...
  if server_target:
    ...
    with device(replica_device_setter(ps_tasks=cur_ps_num, worker_device=cur_work_device, cluster=cluster)):
      distribution = strategy_builder.build(train_config)
      estimator, run_config = _create_estimator(pipeline_config, distribution)
      eval_spec = _create_eval_export_spec(pipeline_config, eval_data)
      ckpt_path = _get_ckpt_path(pipeline_config, eval_checkpoint_path)
    ...
    session_config = ConfigProto(
      ...
      device_filters=['/job:ps', '/job:worker/task:%d' % cur_task_index],
    )
    ...
    if cur_job_name == 'master':
      ...
      cur_hooks = EvaluateExitBarrierHook(cur_worker_num, True, ckpt_dir, metric_ops)
    else:
      cur_stop_grace_period_sesc = 10
      cur_hooks = EvaluateExitBarrierHook(cur_worker_num, False, ckpt_dir, metric_ops)
...
def export(
  ...
  **extra_params,
  ...
  if data_config.input_type == data_config.InputType.OdpsRTPInputV2:
    input_fn_kwargs['fg_json_path'] = pipeline_config.fg_json_path
  serving_input_fn = _get_input_fn(data_config, feature_configs, None, export_config, **input_fn_kwargs)
  ckpt_path = _get_ckpt_path(pipeline_config, checkpoint_path)
