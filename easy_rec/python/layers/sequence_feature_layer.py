...
class SequenceFeatureLayer(object):
  ...
  def negative_sampler_target_attention(
    ...
    cur_id, hist_id_col, seq_len, aux_hist_emb_list = deep_fea[
      'key'], deep_fea['hist_seq_emb'], deep_fea['hist_seq_len'], deep_fea[
    ...
    cur_ids = tf.reshape(
      cur_ids, tf.shape(hist_id_col)
    )  # (B * neg_num_add_1, seq_max_len, seq_emb_dim)
    ...
    scores = tf.where(
      mask, scores, padding
    )  # [B*neg_num_add_1, 1, seq_max_len]

    # Scale
    scores = tf.nn.softmax(scores)  # (B * neg_num_add_1, 1, seq_max_len)
    hist_din_emb = tf.matmul(
      scores, hist_id_col
    )  # [B * neg_num_add_1, 1, seq_emb_dim]
