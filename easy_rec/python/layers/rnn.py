...
def _rnn_step(time,
  ...
  # if t >= max_sequence_length: return (zero_output, state)
  # if t < min_sequence_length: return call_cell().
  ...
  def _maybe_copy_some_through():
    ...
    Pass through either no or some past state.
    """
    new_output, new_state = call_cell()
  ...
  if skip_conditionals:
    ...
    empty_update = lambda: flat_zero_output + flat_state  # noqa:E731
...
def bidirectional_dynamic_rnn(cell_fw,
  ...
  output_bw = _reverse(
      tmp, seq_lengths=sequence_length, seq_dim=time_dim, batch_dim=batch_dim)
...
def _dynamic_rnn_loop(cell,
  ...
  def _time_step(time, output_ta_t, state, att_scores=None):
    ...
    if att_scores is not None:
      ...
      call_cell = lambda: cell(input_t, state, att_score)  # noqa:E731
    else:
      call_cell = lambda: cell(input_t, state)  # noqa:E731
...
def static_rnn(cell,
  ...
  with vs.variable_scope(scope or 'rnn') as varscope:
    ...
    for time, input_ in enumerate(inputs):
      ...
      call_cell = lambda: cell(input_, state)  # noqa:E731
