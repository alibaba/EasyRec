...
class BinaryDataset:
  ...
  def __init__(
    ...
    self._compute_global_start_pos(
      total_sample_num, batch_size, global_rank, global_size, drop_last
    )
    ...
    for tmp_file_id in range(self._start_file_id, self._end_file_id + 1):
      self._label_file_arr[tmp_file_id] = os.open(
        label_bins[tmp_file_id], os.O_RDONLY
      )
      self._dense_file_arr[tmp_file_id] = os.open(
        dense_bins[tmp_file_id], os.O_RDONLY
      )
      self._category_file_arr[tmp_file_id] = os.open(
        category_bins[tmp_file_id], os.O_RDONLY
      )
  ...
  def _compute_global_start_pos(
    self, total_sample_num, batch_size, global_rank, global_size, drop_last
  ):
    ...
    logging.info(
      'start_file_id = %d start_file_pos = %d' %
  ...
  def _get(self, idx):
    ...
    while total_read_num < self._batch_size and curr_file_id < len(
      ...
      tmp_read_num = min(
        end_read_pos, self._sample_num_arr[curr_file_id]
      ) - start_read_pos

      label_raw_data = os.pread(
        self._label_file_arr[curr_file_id], 4 * tmp_read_num,
        start_read_pos * 4
      )
      tmp_lbl_np = np.frombuffer(label_raw_data,
                                 dtype=np.int32).reshape([tmp_read_num, 1])
      label_read_arr.append(tmp_lbl_np)

      dense_raw_data = os.pread(
        self._dense_file_arr[curr_file_id], 52 * tmp_read_num,
        start_read_pos * 52
      )
      part_dense_np = np.frombuffer(dense_raw_data, dtype=np.float32).reshape(
        [tmp_read_num, 13]
      )
      ...
      category_raw_data = os.pread(
        self._category_file_arr[curr_file_id], 104 * tmp_read_num,
        start_read_pos * 104
      )
      part_cate_np = np.frombuffer(category_raw_data, dtype=np.uint32).reshape(
        [tmp_read_num, 26]
      )
...
if __name__ == '__main__':
  ...
  parser.add_argument(
    '--batch_size', type=int, default=1024, help='batch_size'
  )
  parser.add_argument(
