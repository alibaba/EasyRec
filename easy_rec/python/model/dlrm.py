...
class DLRM(RankModel):
  ...
  def build_predict_graph(self):
    bot_dnn = dnn.DNN(
      self._model_config.bot_dnn, self._l2_reg, 'bot_dnn', self._is_training
    )
    dense_fea = bot_dnn(self._dense_feature)
    logging.info(
      'arch_interaction_op = %s' % self._model_config.arch_interaction_op
    )
    ...
    top_dnn = dnn.DNN(
      self._model_config.top_dnn, self._l2_reg, 'top_dnn', self._is_training
    )
