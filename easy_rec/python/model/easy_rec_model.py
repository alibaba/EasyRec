...
from easy_rec.python.utils import constant, estimator_utils, restore_filter
...
_meta_type = get_register_class_meta(_EASY_REC_MODEL_CLASS_MAP, have_abstract_class=True)
...
class EasyRecModel(six.with_metaclass(_meta_type, object)):
    ...
    def l2_regularization(self):
        model_config = getattr(self._base_model_config, self._base_model_config.WhichOneof('model'))
        l2_regularization = 0.0
        if hasattr(model_config, 'dense_regularization') and model_config.HasField('dense_regularization'):
            # backward compatibility
            tf.logging.warn('dense_regularization is deprecated, please use l2_regularization')
    ...
    def build_input_layer(self, model_config, feature_configs):
        self._input_layer = input_layer.InputLayer(
            ...
            if model_config.HasField('variational_dropout')
            else None,
            is_training=False,
        )
    ...
    def restore(self, ckpt_path, include_global_step=False, ckpt_var_map_path='', force_restore_shape_compatible=False):
        ...
        if ckpt_path.endswith('/') or tf.gfile.IsDirectory(ckpt_path + '/'):
            ...
            print('ckpt_path is model_dir,  will use the latest checkpoint: %s' % ckpt_path)
            ...
            if variable_name in ckpt_var2shape_map:
                ...
                if ckpt_var_shape == var_shape:
                    vars_in_ckpt[variable_name] = (
                        list(variable) if isinstance(variable, PartitionedVariable) else variable
                    )
                    ...
                    if force_restore_shape_compatible:
                        ...
                        print('incompatible restore %s[%s, %s]' % (variable_name, str(var_shape), str(ckpt_var_shape)))
                    else:
                        logging.warning(
        ...
        if force_restore_shape_compatible:
            return estimator_utils.IncompatibleShapeRestoreHook(incompatible_shape_var_map)
        else:
            return None
    ...
    def _get_restore_vars(self, ckpt_var_map_path):
        ...
        for one_var in all_vars:
            ...
            if re.search(PARTITION_PATTERN, var_name) and (
                not var_name.endswith('/AdamAsync_2') and not var_name.endswith('/AdamAsync_3')
            ):
        ...
        if ckpt_var_map_path != '':
            ...
            for var_name in name2var:
                if var_name in name_map:
                    ...
                    logging.warning('Failed to find in var_map_file(%s): %s' % (ckpt_var_map_path, var_name))
            return name2var
        else:
            var_filter, scope_update = self.get_restore_filter()
            if var_filter is not None:
                name2var = {var_name: name2var[var_name] for var in name2var if var_filter.keep(var.name)}
            # drop scope prefix if necessary
            if scope_update is not None:
                name2var = {scope_update(var_name): name2var[var_name] for var_name in name2var}
            return name2var
    ...
    def get_restore_filter(self):
        ...
        all_filters = [restore_filter.KeywordFilter(x, True) for x in self._base_model_config.restore_filters]

        return restore_filter.CombineFilter(all_filters, restore_filter.Logical.AND), None
