...
from easy_rec.python.core.metrics import gauc, max_f1, session_auc
...
class MetricsTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_gauc(self, reduction, expected):
        ...
        with tf.Session() as sess:
            sess.run(
                update_op, feed_dict={labels: [1, 0, 1, 1, 0], probs: [0.9, 0.8, 0.7, 0.6, 0.5], uids: [1, 1, 1, 1, 1]}
            )
            sess.run(update_op, feed_dict={labels: [1, 0, 0, 1], probs: [0.9, 0.8, 0.7, 0.6], uids: [2, 2, 2, 2]})
            score = sess.run(value_op)
    ...
    def test_session_auc(self, reduction, expected):
        ...
        value_op, update_op = session_auc(labels, probs, session_ids, reduction=reduction)
        with tf.Session() as sess:
            sess.run(
                update_op,
                feed_dict={labels: [1, 0, 1, 1, 0], probs: [0.9, 0.8, 0.7, 0.6, 0.5], session_ids: [1, 1, 1, 1, 1]},
            )
            sess.run(
                update_op, feed_dict={labels: [1, 0, 0, 1], probs: [0.9, 0.8, 0.7, 0.6], session_ids: [2, 2, 2, 2]}
            )
            score = sess.run(value_op)
        self.assertAlmostEqual(score, expected)
