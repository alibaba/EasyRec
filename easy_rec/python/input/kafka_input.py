...
class KafkaInput(Input):
    def __init__(self, data_config, feature_config, kafka_config, task_index=0, task_num=1):
        super(KafkaInput, self).__init__(data_config, feature_config, '', task_index, task_num)
    ...
    def _parse_csv(self, line):
        ...
        with tf.control_dependencies([check_op]):
            fields = tf.decode_csv(
                line, field_delim=self._data_config.separator, record_defaults=record_defaults, name='decode_csv'
            )
    ...
    def _build(self, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            i = self._task_index
            assert (
                len(train.offset) == 1 or len(train.offset) == train.partitions
            ), 'number of train.offset must be 1 or train.partitions'
            while i < train.partitions:
                offset_i = train.offset[i] if i < len(train.offset) else train.offset[-1]
            ...
            dataset = kafka_io.KafkaDataset(topics, servers=train.server, group=train.group, eof=False)
            ...
            i = 0
            assert (
                len(eval.offset) == 1 or len(eval.offset) == eval.partitions
            ...
            while i < eval.partitions:
                ...
                topics.append(eval.topic + ':' + str(i) + ':' + str(eval.offset) + ':-1')
                i = i + 1
            ...
            dataset = kafka_io.KafkaDataset(topics, servers=eval.server, group=eval.group, eof=False)
            dataset = dataset.repeat(1)
