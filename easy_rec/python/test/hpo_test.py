...
class HPOTest(tf.test.TestCase):
  ...
  def test_save_eval_metrics(self):
    ...
    tmp_file = os.path.join(
      test_dir, 'easy_rec_hpo_test_%d.metric' % time.time()
    )
  ...
  def test_edit_config(self):
    ...
    tmp_config = config_util.edit_config(
      tmp_config, self.load_config(tmp_file)
    )
  ...
  def test_edit_config_v7(self):
    ...
    for i, tmp_fea in enumerate(tmp_config.feature_configs):
      if tmp_fea.input_names[0] == 'c21':
        assert len(tmp_fea.boundaries
                  ) == 4 and np.abs(tmp_fea.boundaries[0] - 10.0) < 1e-5
  ...
  def test_edit_config_v8(self):
    ...
    for i, tmp_fea in enumerate(tmp_config.feature_configs):
      if tmp_fea.input_names[0] == 'c21':
        assert len(tmp_fea.boundaries
                  ) == 4 and np.abs(tmp_fea.boundaries[0] - 4.0) < 1e-5
        assert tmp_fea.embedding_dim == 32
