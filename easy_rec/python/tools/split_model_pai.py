    ...
    for sig_name in signatures:
        if signatures[sig_name].method_name == tf.saved_model.signature_constants.PREDICT_METHOD_NAME:
            ...
            for input_name in signatures[sig_name].inputs:
                ...
                tf.logging.info('"%s": %s; %s' % (input_name, _TYPE_TO_STRING[input_tensor.dtype], input_tensor_shape))
            ...
            for output_name in signatures[sig_name].outputs:
                ...
                tf.logging.info('"%s": %s; %s' %
                                (output_name, _TYPE_TO_STRING[output_tensor.dtype], output_tensor_shape))
...
def export(model_dir, meta_graph_def, variable_protos, input_tensor_names, output_tensor_names, part_name, part_dir):
    ...
    inference_graph, variables_to_keep, init_op_names = extract_sub_graph(meta_graph_def.graph_def, output_node_names, variable_protos)
...
def main(argv):
    ...
    meta_graph_def, variable_protos, input_tensor_names, output_tensor_names = load_meta_graph_def(model_dir)
