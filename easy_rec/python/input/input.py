...
from easy_rec.python.utils.input_utils import (  # NOQA
    get_type_defaults,
    string_to_number,
)
...
class Input(six.with_metaclass(_meta_type, object)):
    def __init__(self, data_config, feature_configs, input_path, task_index=0, task_num=1):
    ...
    def create_multi_placeholders(self, placeholder_named_by_input, export_fields_name=None):
    ...
    def create_placeholders(self):
        ...
        for tmp_id, fid in enumerate(effective_fids):
            ...
            if ftype in [DatasetConfig.STRING]:
                ...
                features[input_name] = string_to_number(input_vals[:, tmp_id], ftype, tmp_id)
        features = self._preprocess(features)
    ...
    def create_embedding_placeholders(self):
        ...
        embed_input_desc_files = tf.gfile.Glob(os.path.join(dump_input_dir, 'input_layer_*.txt'))
        for one_file in embed_input_desc_files:
            with tf.gfile.GFile(one_file, 'r') as fin:
                for line_str in fin:
                    ...
                    inputs[input_name] = tf.placeholder(tf.float32, shape_config['shape'], name=input_name)
        return inputs
    ...
    def _get_features(self, fields):
        ...
        if constant.SAMPLE_WEIGHT in fields:
            logging.info('will use field %s as sample weight' % self._data_config.sample_weight)
            ...
            if feature_type == fc.TagFeature:
                ...
                if len(field.get_shape()) < 2 or field.get_shape()[-1] == 1:
                    ...
                    if fc.HasField('kv_separator'):
                        ...
                        parsed_dict[input_0] = tf.sparse.SparseTensor(indices, tmp_ks, parsed_dict[input_0].dense_shape)
                    ...
                    if not fc.HasField('hash_bucket_size'):
                        vals = tf.string_to_number(parsed_dict[input_0].values, tf.int32, name='tag_fea_%s' % input_0)
                        parsed_dict[input_0] = tf.sparse.SparseTensor(
                            parsed_dict[input_0].indices, vals, parsed_dict[input_0].dense_shape
                        )
                    if len(fc.input_names) > 1:
                ...
                if len(field.get_shape()) < 2:
                    ...
                    if fc.HasField('seq_multi_sep'):
                        ...
                        out_shape = tf.concat([parsed_dict[input_0].dense_shape, multi_vals.dense_shape[1:]], axis=0)
                        parsed_dict[input_0] = tf.sparse.SparseTensor(out_indices, multi_vals.values, out_shape)
                    if fc.num_buckets > 0:
                        parsed_dict[input_0] = tf.sparse.SparseTensor(
                            parsed_dict[input_0].indices,
                            tf.string_to_number(
                                parsed_dict[input_0].values, tf.int64, name='sequence_str_2_int_%s' % input_0
                            ),
                ...
                if field_dict[input_0].dtype == tf.string:
                    if fc.raw_input_dim > 1:
                        ...
                        tmp_vals = tf.string_to_number(
                            tmp_fea.values, tf.float32, name='multi_raw_fea_to_flt_%s' % input_0
                        )
                ...
                if fc.max_val > fc.min_val:
                    parsed_dict[input_0] = (parsed_dict[input_0] - fc.min_val) / (fc.max_val - fc.min_val)
                if not fc.boundaries and fc.num_buckets <= 1 and self._data_config.sample_weight != input_0:
                    ...
                    parsed_dict[input_0 + '_raw_proj_id'] = tf.SparseTensor(
                        indices=indices, values=indices_1[:, 0], dense_shape=[sample_num, fc.raw_input_dim]
                    )
                    ...
                    if field_dict[input_0].dtype != tf.string:
                        if field_dict[input_0].dtype in [tf.float32, tf.double]:
                            assert fc.precision > 0, (
                                'it is dangerous to convert float or double to string due to '
                        ...
                        if 'as_string' in dir(tf.strings):
                            parsed_dict[input_0] = tf.strings.as_string(field_dict[input_0], precision=precision)
                        else:
                            parsed_dict[input_0] = tf.as_string(field_dict[input_0], precision=precision)
        ...
        for input_id, input_name in enumerate(self._label_fields):
            ...
            if field_dict[input_name].dtype == tf.string:
                if self._label_dim[input_id] > 1:
                    ...
                    parsed_dict[input_name] = tf.string_split(field_dict[input_name], self._label_sep[input_id]).values
                    parsed_dict[input_name] = tf.reshape(parsed_dict[input_name], [-1, self._label_dim[input_id]])
                else:
                    parsed_dict[input_name] = field_dict[input_name]
                parsed_dict[input_name] = tf.string_to_number(parsed_dict[input_name], tf.float32, name=input_name)
            else:
    ...
    def _lookup_preprocess(self, fc, field_dict):
        ...
        if len(key_fields.get_shape()) == 0:
            ...
            indices = [tf.expand_dims(indices_0, axis=1), tf.expand_dims(indices_1, axis=1)]
        ...
        indices = tf.concat([tf.expand_dims(indices_0, axis=1), tf.expand_dims(indices_1, axis=1)], axis=1)
        ...
        def _input_fn(mode=None, params=None, config=None):
            ...
            if mode in (tf.estimator.ModeKeys.TRAIN, tf.estimator.ModeKeys.EVAL, tf.estimator.ModeKeys.PREDICT):
                ...
                if export_config.dump_embedding_shape:
                    ...
                    inputs, features = self.create_multi_placeholders(placeholder_named_by_input, export_fields_name)
