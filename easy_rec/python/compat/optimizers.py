...
OPTIMIZER_CLS_NAMES = {
    ...
    'Momentum': lambda learning_rate: train.MomentumOptimizer(
...
def optimize_loss(
    ...
    with vs.variable_scope(name, 'OptimizeLoss', [loss, global_step]):
        ...
        if not_apply_grad_after_first_step:
            train_tensor = control_flow_ops.cond(global_step > 0, lambda: loss, _apply_grad)
        else:
...
def adaptive_clipping_fn(
    std_factor=2.0, decay=0.95, static_max_norm=None, global_step=None, report_summary=False, epsilon=1e-8, name=None
):
