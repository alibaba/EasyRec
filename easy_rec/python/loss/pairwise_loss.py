...
def pairwise_loss(labels, logits, session_ids=None, margin=0, temperature=1.0, weights=1.0, name=''):
  ...
  logging.info('[{}] margin: {}, temperature: {}'.format(loss_name, margin, temperature))

  if temperature != 1.0:
    logits /= temperature
  pairwise_logits = tf.math.subtract(tf.expand_dims(logits, -1), tf.expand_dims(logits, 0)) - margin
  pairwise_mask = tf.greater(tf.expand_dims(labels, -1), tf.expand_dims(labels, 0))
  ...
  loss = tf.losses.sigmoid_cross_entropy(pairwise_pseudo_labels, pairwise_logits, weights=pairwise_weights)
  # set rank loss to zero if a batch has no positive sample.
  # loss = tf.where(tf.is_nan(loss), tf.zeros_like(loss), loss)
  return loss


def pairwise_focal_loss(
  labels,
  logits,
  session_ids=None,
  hinge_margin=None,
  ...
  logging.info(
    '[{}] hinge margin: {}, gamma: {}, alpha: {}, ohem_ratio: {}, temperature: {}'.format(
      loss_name, hinge_margin, gamma, alpha, ohem_ratio, temperature
    )
  )
...
def pairwise_logistic_loss(
  ...
  assert 0 < ohem_ratio <= 1.0, loss_name + ' ohem_ratio must be in (0, 1]'
  logging.info(
    '[{}] hinge margin: {}, ohem_ratio: {}, temperature: {}'.format(loss_name, hinge_margin, ohem_ratio, temperature)
  )
  ...
  pairwise_logits = tf.math.subtract(tf.expand_dims(logits, -1), tf.expand_dims(logits, 0))
  if use_label_margin:
    pairwise_logits -= tf.math.subtract(tf.expand_dims(labels, -1), tf.expand_dims(labels, 0))
  ...
  losses = compute_weighted_loss(losses, pairwise_weights, reduction=tf.losses.Reduction.NONE)
...
def pairwise_hinge_loss(
  ...
  pairwise_logits = tf.math.subtract(tf.expand_dims(logits, -1), tf.expand_dims(logits, 0))
  pairwise_labels = tf.math.subtract(tf.expand_dims(labels, -1), tf.expand_dims(labels, 0))
