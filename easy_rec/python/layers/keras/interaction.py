  ...
  def call(self, inputs, **kwargs):
    ...
    if isinstance(inputs, (list, tuple)):
      ...
      try:
        ...
        raise ValueError(
          'Input tensors` dimensions must be equal, original'
...
class Cross(tf.keras.layers.Layer):
  ...
  def __init__(self, params, name='cross', reuse=None, **kwargs):
    ...
    kernel_initializer = params.get_or_default(
  ...
  def call(self, input, **kwargs):
    ...
    for i in range(len(self._hidden_feature_sizes)):
      ...
      intermediate_tensor_expanded = tf.tile(
        intermediate_tensor_expanded, [1, hk, 1, 1, 1]
      )
    ...
    return tf.concat(
      pooled_feature_map_list, axis=-1
    )  # shape = (b, h1 + ... + hk)
