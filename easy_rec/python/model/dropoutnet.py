...
class DropoutNet(EasyRecModel):
  ...
  def __init__(
    ...
    if self._input_layer.has_group('user_content'):
      self.user_content_feature, _ = self._input_layer(
        self._feature_dict, 'user_content'
      )
    ...
    if self._input_layer.has_group('item_content'):
      self.item_content_feature, _ = self._input_layer(
        self._feature_dict, 'item_content'
      )
    ...
    with tf.name_scope('user_tower'):
      ...
      if self.user_content_feature is not None:
        user_content_dnn = dnn.DNN(
          self.user_content_layers, self._l2_reg, 'user_content',
          self._is_training
        )
      ...
      if self.user_preference_feature is not None:
        ...
        user_prefer_dnn = dnn.DNN(
          self.user_preference_layers, self._l2_reg, 'user_preference',
          self._is_training
        )
      ...
      user_dnn = dnn.DNN(
        self.user_tower_layers, self._l2_reg, 'user_dnn', self._is_training
      )
    ...
    with tf.name_scope('item_tower'):
      ...
      if self.item_content_feature is not None:
        item_content_dnn = dnn.DNN(
          self.item_content_layers, self._l2_reg, 'item_content',
          self._is_training
        )
      ...
      if self.item_preference_feature is not None:
        ...
        item_prefer_dnn = dnn.DNN(
          self.item_preference_layers, self._l2_reg, 'item_preference',
          self._is_training
        )
      ...
      item_dnn = dnn.DNN(
        self.item_tower_layers, self._l2_reg, 'item_dnn', self._is_training
      )
    ...
    for loss in self._losses:
      if loss.loss_type == LossType.SOFTMAX_CROSS_ENTROPY_WITH_NEGATIVE_MINING:
        assert self._model_config.HasField(
          'softmax_loss'
        ), '`softmax_loss` must be configured'
        ...
        loss_value = tf.losses.sigmoid_cross_entropy(
          labels, logits, self._sample_weight
        )
