...
class BST(Layer):
  ...
  def encode(self, seq_input, max_position):
    ...
    if self.config.output_all_token_embeddings:
      out_fea = tf.reshape(attention_fea, [-1, max_position * self.config.hidden_size])
    else:
  ...
  def call(self, inputs, training=None, **kwargs):
    ...
    if target is not None and keep_target:
      ...
      assert seq_embed_size == target_size, (
        'the embedding size of sequence and target item is not equal' ' in feature group:' + self.name
      )
