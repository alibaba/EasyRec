...
class InputLayer(object):
  ...
  def __init__(
    ...
    self.sequence_feature_layer = sequence_feature_layer.SequenceFeatureLayer(
      feature_configs, feature_groups_config, ev_params, embedding_regularizer,
      kernel_regularizer, is_training, is_predicting
    )
    ...
    self._group_name_to_seq_features = {
      ...
      for x in feature_groups_config if len(x.sequence_features) > 0
    }
  ...
  def get_combined_feature(self, features, group_name, is_dict=False):
    ...
    negative_sampler = self._feature_groups[group_name
                                           ]._config.negative_sampler

    place_on_cpu = os.getenv('place_embedding_on_cpu')
    place_on_cpu = eval(place_on_cpu) if place_on_cpu else False
    with conditional(
      self._is_predicting and place_on_cpu, ops.device('/CPU:0')
    ):
    ...
    if group_name in self._group_name_to_seq_features:
      ...
      concat_features = array_ops.concat(
        [concat_features, all_seq_fea], axis=-1
      )
    if is_dict:
  ...
  def get_sequence_feature(self, features, group_name):
    ...
    for fc in group_seq_columns:
      with variable_scope.variable_scope(
        'input_layer/' + fc.categorical_column.name
      ):
  ...
  def single_call_input_layer(
    self, features, group_name, feature_name_to_output_tensors=None
  ):
    """Get features by group_name.
