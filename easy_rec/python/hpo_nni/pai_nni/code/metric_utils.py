...
from easy_rec.python.hpo_nni.pai_nni.code.utils import get_value, set_value, parse_config


def _get_best_eval_result(event_files, metric_dict={'auc': 1}, trial_id=None, use_best=False, nni_report=True,
                ...
                if event.HasField('summary'):
                    ...
                    if len(event_eval_result) >= 2:
                        ...
                        if event.step > report_step and nni_report:
                            ...
                            if trial_id:
                                set_value(trial_id + '_report_step', event.step, trial_id=trial_id)
                            print("event_eval_result:", event_eval_result, " temp metric:", temp)
...
def get_result(filepath, dst_filepath, metric_dict={'auc': 1}, trial_id=None, oss_config=None, nni_report=True):
    if filepath:
        copy_dir(filepath, dst_filepath, oss_config)
    full_event_file_pattern = os.path.join(dst_filepath,
                                           '*.tfevents.*')
    print("event_file:", full_event_file_pattern)
    best_eval_result, best_event = _get_best_eval_result(full_event_file_pattern, metric_dict=metric_dict,
                                                         trial_id=trial_id, nni_report=nni_report)
...
def copy_dir(ori_filepath, dst_filepath, oss_config=None):
    print("start copy")
    bucket, path = get_bucket(ori_filepath=ori_filepath,
...
def report_result(filepath, dst_filepath, metric_dict, trial_id=None, oss_config=None, nni_report=True):
    worker = Thread(target=load_loop, args=(filepath, dst_filepath, metric_dict, trial_id, oss_config, nni_report))
    worker.start()


def load_loop(filepath, dst_filepath, metric_dict, trial_id, oss_config, nni_report):
    while True:
        print("get result")
        best_eval_result, best_event = get_result(filepath, dst_filepath, metric_dict=metric_dict, trial_id=trial_id,
                                                  oss_config=oss_config, nni_report=nni_report)
        print(get_value(trial_id + '_exit', trial_id=trial_id), get_value(trial_id + '_exit', trial_id=trial_id) == '1')
        # train end normaly
