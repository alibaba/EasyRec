...
_register_abc_meta = get_register_class_meta(_PREDICTOR_CLASS_MAP, have_abstract_class=True)
...
class PredictorImpl(object):
    ...
    def get_input_fields_from_pipeline_config(self, model_path):
        ...
        input_fields_info = {
            input_field.input_name: (input_field.input_type, input_field.default_val) for input_field in input_fields
        }
    ...
    def _build_model(self):
        ...
        session_config = tf.ConfigProto(
            gpu_options=gpu_options, allow_soft_placement=True, log_device_placement=(self._profiling_file is not None)
        )
        ...
        with self._graph.as_default():
            with self._session.as_default():
                ...
                if tf.gfile.IsDirectory(model_path):
                    ...
                    self._input_fields_info = self.get_input_fields_from_pipeline_config(model_path)
                    ...
                    if len(inputs.items()) > 1:
                        for gid, item in enumerate(inputs.items()):
                            ...
                            try:
                                ...
                                logging.warning('could not determine input_id from input_name: %s' % input_name)
                                input_id = gid
                    ...
                    for name, tensor in outputs.items():
                        ...
                        self._outputs_map[name] = self._graph.get_tensor_by_name(tensor.name)
                    ...
                    for any_proto in asset_files:
                        ...
                        asset_path = os.path.join(model_path, constants.ASSETS_DIRECTORY, asset_file.filename)
                        assert tf.gfile.Exists(asset_path), '%s is missing in saved model' % asset_path
    ...
    def predict(self, input_data_dict, output_names=None):
        ...
        for input_name, tensor in six.iteritems(self._inputs_map):
            ...
            assert tensor_shape[0] is None or (
                tensor_shape[0] == input_shape[0]
        ...
        with self._graph.as_default():
            with self._session.as_default():
                if self._profiling_file is None:
                    ...
                    results = self._session.run(fetch_dict, feed_dict, options=run_options, run_metadata=run_metadata)
...
class Predictor(PredictorInterface):
    ...
    def predict_table(
        ...
        def _get_defaults(col_name, col_type):
            if col_name in self._input_fields_info:
                ...
                assert col_type in defaults, 'invalid col_type: %s, col_type: %s' % (col_name, col_type)
        ...
        record_defaults = [_get_defaults(col_name, col_type) for col_name, col_type in zip(all_cols, all_col_types)]
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            while True:
                try:
                    ...
                    break
                if progress % 100 == 0:
                    logging.info('progress: batch_num=%d sample_num=%d' % (progress, progress * batch_size))
                    logging.info('time_stats: read: %.2f predict: %.2f write: %.2f' % (sum_t0, sum_t1, sum_t2))
            logging.info('Final_time_stats: read: %.2f predict: %.2f write: %.2f' % (sum_t0, sum_t1, sum_t2))
            table_writer.close()
    ...
    def predict(self, input_data_dict_list, output_names=None, batch_size=1):
        ...
        assert (
            isinstance(input_data_dict_list[0], dict)
            or isinstance(input_data_dict_list[0], list)
            or isinstance(input_data_dict_list[0], str)
        ), 'input is not a list or dict or str'
        if batch_size > 0:
        ...
        for batch_idx in range(num_batches):
            batch_data_list = input_data_dict_list[batch_idx * batch_size : (batch_idx + 1) * batch_size]
    ...
    def batch(self, data_list):
        ...
        for data in data_list:
            if isinstance(data, dict):
                ...
                assert len(self._predictor_impl.input_names) == len(
                    data
