...
class SequenceFeatureLayer(object):
  ...
  def negative_sampler_target_attention(
    ...
    hist_id_col_tmp = tf.tile(hist_id_col[:, :, :], multiples=[1, neg_num_add_1, 1])
    hist_id_col = tf.reshape(hist_id_col_tmp, [batch_size * neg_num_add_1, seq_max_len, seq_emb_dim])

    concat_features = tf.tile(concat_features[:, tf.newaxis, :], multiples=[1, neg_num_add_1, 1])
    ...
    cur_ids = tf.reshape(cur_ids, tf.shape(hist_id_col))  # (B * neg_num_add_1, seq_max_len, seq_emb_dim)
    ...
    scores = tf.where(mask, scores, padding)  # [B*neg_num_add_1, 1, seq_max_len]

    # Scale
    scores = tf.nn.softmax(scores)  # (B * neg_num_add_1, 1, seq_max_len)
    hist_din_emb = tf.matmul(scores, hist_id_col)  # [B * neg_num_add_1, 1, seq_emb_dim]
    hist_din_emb = tf.reshape(
      hist_din_emb, [batch_size, neg_num_add_1, seq_emb_dim]
  ...
  def target_attention(
    ...
    if allow_key_transform and (cur_id_dim != seq_emb_dim):
      if seq_emb_dim > cur_id_dim and not transform_dnn:
        ...
        hist_id_col = tf.layers.dense(hist_id_col, seq_emb_dim, name=cur_fea_layer_name)
    else:
      cur_id = cur_id[: tf.shape(hist_id_col)[0], ...]  # for negative sampler

    cur_ids = tf.tile(cur_id, [1, seq_max_len])
    cur_ids = tf.reshape(cur_ids, tf.shape(hist_id_col))  # (B, seq_max_len, seq_emb_dim)
    ...
    hist_din_emb = tf.reshape(hist_din_emb, [-1, seq_emb_dim])  # [B, seq_emb_dim]
    if len(aux_hist_emb_list) > 0:
      all_hist_dim_emb = [hist_din_emb]
      for hist_col in aux_hist_emb_list:
