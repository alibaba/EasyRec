...
class MultiTowerRecall(RankModel):
  def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
    super(MultiTowerRecall, self).__init__(model_config, feature_configs, features, labels, is_training)
  ...
  def build_predict_graph(self):
    ...
    item_tower_feature = tf.concat(
      [
        ...
        tf.tile(neg_item_feature[tf.newaxis, :, :], multiples=[batch_size, 1, 1]),
      ],
      axis=1,
    )  # noqa: E126

    user_dnn = dnn.DNN(
      self._model_config.user_tower.dnn,
      self._l2_reg,
      'user_dnn',
      self._is_training,
    )
    ...
    item_dnn = dnn.DNN(
      self._model_config.item_tower.dnn,
      self._l2_reg,
      'item_dnn',
      self._is_training,
    )
