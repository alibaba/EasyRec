...
def circle_loss(
  ...
  ap = tf.nn.relu(
    -tf.stop_gradient(pair_wise_cosine_matrix * positive_mask) + 1 + margin
  )
  an = tf.nn.relu(
    tf.stop_gradient(pair_wise_cosine_matrix * negative_mask) + margin
  )

  logit_p = -ap * (pair_wise_cosine_matrix - delta_p
                  ) * gamma * positive_mask - (1 - positive_mask) * 1e12
  logit_n = an * (pair_wise_cosine_matrix - delta_n
                 ) * gamma * negative_mask - (1 - negative_mask) * 1e12
