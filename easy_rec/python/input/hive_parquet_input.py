...
class HiveParquetInput(Input):
  ...
  def _parquet_read(self):
    for input_path in self._input_hdfs_path:
      ...
      for start_idx in range(0, total_records_num, self._data_config.batch_size):
        end_idx = min(total_records_num, start_idx + self._data_config.batch_size)
  ...
  def _build(self, mode, params):
    ...
    dataset = tf.data.Dataset.from_generator(self._parquet_read, output_types=list_type, output_shapes=list_shapes)
