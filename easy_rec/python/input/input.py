...
class Input(six.with_metaclass(_meta_type, object)):
  ...
  def __init__(
    ...
    **kwargs,
    ...
    # calling self._build
    self._mode = None
    if pipeline_config is not None and pipeline_config.model_config.HasField('ev_params'):
      self._has_ev = True
    else:
      self._has_ev = False
    ...
    for fc in self._feature_configs:
      ...
      if fc.feature_type in [fc.TagFeature, fc.SequenceFeature]:
        if fc.hash_bucket_size > 0 or len(fc.vocab_list) > 0 or fc.HasField('vocab_file'):
    ...
    self._effective_fields = [self._input_fields[x] for x in self._effective_fids]
  ...
  def _load_label_fn(self, config):
    ...
    udf_path = config.user_define_fn_path if config.HasField('user_define_fn_path') else None
    dtype = config.user_define_fn_res_type if config.HasField('user_define_fn_res_type') else None
  ...
  def get_feature_input_fields(self):
    return [x for x in self._input_fields if x not in self._label_fields and x != self._data_config.sample_weight]
  ...
  def create_multi_placeholders(self, export_config):
    ...
    if export_config.filter_inputs:
      ...
      effective_fids = [
        fid
        for fid in range(len(self._input_fields))
    ...
    for fid in effective_fids:
      ...
      if input_name in export_fields_name:
        tf_type = (
          ...
          if input_name in self._multi_value_types
          else get_tf_type(self._input_field_types[fid])
        )
        logging.info('multi value input_name: %s, dtype: %s' % (input_name, tf_type))
        finput = array_ops.placeholder(tf_type, [None, None], name=placeholder_name)
      else:
  ...
  def create_placeholders(self, export_config):
    self._mode = tf.estimator.ModeKeys.PREDICT
    inputs_placeholder = array_ops.placeholder(tf.string, [None], name='features')
    input_vals = tf.string_split(inputs_placeholder, self._data_config.separator, skip_empty=False).values
    ...
    if export_config.filter_inputs:
      effective_fids = list(self._effective_fids)
      logging.info(
        'number of effective inputs:%d, total number inputs: %d' % (len(effective_fids), len(self._input_fields))
      )
    else:
      effective_fids = [
        fid
      ...
      logging.info('will not filter any input[except labels], total number inputs:%d' % len(effective_fids))
    ...
    return OrderedDict(
      [
        (
          x,
          (
            tf.squeeze(labels[x], axis=1)
            if len(labels[x].get_shape()) == 2 and labels[x].get_shape()[1] == 1
            else labels[x]
          ),
        )
        for x in labels
  ...
  def _parse_combo_feature(self, fc, parsed_dict, field_dict):
    ...
    if len(fc.combo_input_seps) > 0:
      assert len(fc.combo_input_seps) == len(fc.input_names), 'len(combo_separator)[%d] != len(fc.input_names)[%d]' % (
      ...
      if len(fc.combo_input_seps) > 0:
        ...
        for input_id, input_name in enumerate(fc.input_names):
          ...
          if len(input_sep) > 0:
            ...
            split_inputs.append(tf.string_split(field_dict[input_name], fc.combo_input_seps[input_id]))
          else:
            split_inputs.append(tf.reshape(field_dict[input_name], [-1, 1]))
        parsed_dict[feature_name] = sparse_ops.sparse_cross(split_inputs, fc.combo_join_sep)
      else:
        inputs = [self._as_string(field_dict[input_name], fc) for input_name in fc.input_names]
        parsed_dict[feature_name] = string_ops.string_join(inputs, fc.combo_join_sep)
  ...
  def _parse_tag_feature(self, fc, parsed_dict, field_dict):
    ...
    if len(field.get_shape()) < 2 or field.get_shape()[-1] == 1:
      ...
      if fc.HasField('kv_separator'):
        ...
        check_list = [tf.py_func(check_string_to_number, [tmp_vs, input_0], Tout=tf.bool)] if self._check_mode else []
        with tf.control_dependencies(check_list):
          tmp_vs = tf.string_to_number(tmp_vs, tf.float32, name='kv_tag_wgt_str_2_flt_%s' % input_0)
        parsed_dict[feature_name] = tf.sparse.SparseTensor(indices, tmp_ks, parsed_dict[feature_name].dense_shape)
      ...
      if len(fc.input_names) > 1:
        ...
        with tf.control_dependencies(check_list):
          field_vals = tf.string_to_number(field.values, tf.float32, name='tag_wgt_str_2_flt_%s' % input_1)
  ...
  def _parse_raw_feature(self, fc, parsed_dict, field_dict):
    ...
    if field_dict[input_0].dtype == tf.string:
      ...
      if fc.raw_input_dim > 1:
        ...
        check_list = [tf.py_func(check_string_to_number, [vals, input_0], Tout=tf.bool)] if self._check_mode else []
        with tf.control_dependencies(check_list):
          emb = tf.string_to_number(vals, tf.float32, name='raw_fea_to_flt_%s' % input_0)
        if fc.combiner == 'max':
          emb = tf.segment_max(emb, segment_ids)
        elif fc.combiner == 'sum':
          emb = tf.segment_sum(emb, segment_ids)
        ...
        with tf.control_dependencies(check_list):
          parsed_dict[feature_name] = tf.string_to_number(field_dict[input_0], tf.float32)
    ...
    if fc.max_val > fc.min_val:
      parsed_dict[feature_name] = (parsed_dict[feature_name] - fc.min_val) / (fc.max_val - fc.min_val)

    if fc.HasField('normalizer_fn'):
      logging.info('apply normalizer_fn %s to `%s`' % (fc.normalizer_fn, feature_name))
      parsed_dict[feature_name] = self._normalizer_fn[feature_name](parsed_dict[feature_name])

    if (
      not fc.boundaries and fc.num_buckets <= 1 and fc.embedding_dim > 0 and self._data_config.sample_weight != input_0
    ):
  ...
  def _parse_seq_feature(self, fc, parsed_dict, field_dict):
    ...
    if len(field.get_shape()) < 2:
      ...
      if fc.HasField('seq_multi_sep'):
        ...
        parsed_dict[feature_name] = tf.sparse.SparseTensor(out_indices, multi_vals.values, out_shape)
      ...
      if fc.num_buckets > 1 and fc.max_val > fc.min_val:
        normalized_values = (parsed_dict[feature_name].values - fc.min_val) / (fc.max_val - fc.min_val)
        parsed_dict[feature_name] = tf.sparse.SparseTensor(
          parsed_dict[feature_name].indices,
          normalized_values,
          parsed_dict[feature_name].dense_shape,
    ...
    if (
      not fc.boundaries
      and fc.num_buckets <= 1
      and self._data_config.sample_weight != input_0
      and sub_feature_type == fc.RawFeature
      and fc.raw_input_dim == 1
    ):
      logging.info(
        (
          'Not set boundaries or num_buckets or hash_bucket_size, '
      ...
      logging.info(
        'Not set boundaries or num_buckets or hash_bucket_size,'
        ' %s will process as three dimension sequence raw feature' % feature_name
      )
      parsed_dict[feature_name] = tf.sparse_to_dense(
        parsed_dict[feature_name].indices,
        [
  ...
  def _preprocess(self, field_dict):
    ...
    for input_id, input_name in enumerate(self._label_fields):
      ...
      if field_dict[input_name].dtype == tf.string:
        if self._label_dim[input_id] > 1:
          ...
          with tf.control_dependencies(check_list):
            label_dict[input_name] = tf.string_split(field_dict[input_name], self._label_sep[input_id]).values
            label_dict[input_name] = tf.reshape(label_dict[input_name], [-1, self._label_dim[input_id]])
        else:
        ...
        with tf.control_dependencies(check_list):
          label_dict[input_name] = tf.string_to_number(label_dict[input_name], tf.float32, name=input_name)
      else:
    ...
    if self._mode != tf.estimator.ModeKeys.PREDICT:
      for func_config in self._data_config.extra_label_func:
        ...
        logging.info('generating new label `%s` by transform: %s' % (lbl_name, func_name))
        lbl_fn = load_by_path(func_name)
        label_dict[lbl_name] = lbl_fn(label_dict)

      if self._data_config.HasField('sample_weight'):
        parsed_dict[constant.SAMPLE_WEIGHT] = field_dict[self._data_config.sample_weight]
  ...
  def _lookup_preprocess(self, fc, field_dict):
    ...
    indices = tf.concat(
      [tf.expand_dims(indices_0, axis=1), tf.expand_dims(indices_1, axis=1)],
      axis=1,
    )
  ...
  def _safe_shard(self, dataset):
    if self._data_config.chief_redundant:
      return dataset.shard(max(self._task_num - 1, 1), max(self._task_index - 1, 0))
    else:
