...
class ESMM(MultiTaskModel):
  ...
  def build_loss_graph(self):
    ...
    if self._cvr_tower_cfg.loss_type == LossType.CLASSIFICATION:
      ctcvr_label = tf.cast(
        self._labels[cvr_label_name] * self._labels[ctr_label_name], tf.float32
      )
      ...
      self._loss_dict['weighted_cross_entropy_loss_%s' % cvr_tower_name
                     ] = self._cvr_tower_cfg.weight * cvr_loss
      ...
      self._loss_dict['weighted_l2_loss_%s' % cvr_tower_name
                     ] = self._cvr_tower_cfg.weight * cvr_loss
    ...
    self._loss_dict['weighted_cross_entropy_loss_%s' % ctr_tower_name
                   ] = self._ctr_tower_cfg.weight * ctr_loss
  ...
  def build_predict_graph(self):
    ...
    if self.has_backbone:
      ...
      for group_id in range(self._group_num):
        ...
        dnn_model = dnn.DNN(
          group.dnn, self._l2_reg, group_name, self._is_training
        )
  ...
  def get_outputs(self):
    ...
    if self._cvr_tower_cfg.loss_type == LossType.CLASSIFICATION:
      ...
      raise ValueError(
        'invalid cvr_tower loss type: %s' % str(self._cvr_tower_cfg.loss_type)
      )
    return outputs
