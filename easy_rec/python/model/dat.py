...
class DAT(MatchModel):
  ...
  def __init__(
    ...
    self.user_augmented_vec, _ = self._input_layer(
      self._feature_dict, 'user_id_augment'
    )

    self.item_tower = copy_obj(self._model_config.item_tower)
    self.item_deep_feature, _ = self._input_layer(self._feature_dict, 'item')
    self.item_augmented_vec, _ = self._input_layer(
      self._feature_dict, 'item_id_augment'
    )
  ...
  def build_predict_graph(self):
    ...
    user_dnn = dnn.DNN(
      self.user_tower.dnn, self._l2_reg, 'user_dnn', self._is_training
    )
    ...
    item_dnn = dnn.DNN(
      self.item_tower.dnn, self._l2_reg, 'item_dnn', self._is_training
    )
