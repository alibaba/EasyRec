...
from easy_rec.python.inference.predictor import (  # NOQA
  SINGLE_PLACEHOLDER_FEATURE_KEY,
  Predictor,
)
...
class CSVPredictor(Predictor):
  ...
  def _parse_line(self, line):
    check_list = [
      tf.py_func(
        check_split,
        [line, self._input_sep, len(self._record_defaults)],
        Tout=tf.bool,
      )
    ]
  ...
  def _get_dataset(self, input_path, num_parallel_calls, batch_size, slice_num, slice_id):
    ...
    if self._is_rtp:
      ...
      self._record_defaults = [self._get_defaults(col_name) for col_name in self._all_fields]
