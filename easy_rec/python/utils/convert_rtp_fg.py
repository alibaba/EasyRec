    ...
    if 'hash_bucket_size' in feature:
        ...
        if feature_config.hash_bucket_size > 10000000:
            if 'max_partitions' not in feature:
                logging.error(
                    'it is suggested to set max_partitions > 1 for large hash buckets[%s]' % feature['feature_name']
                )
                sys.exit(1)
...
def process_features(
    ...
    if 'extra_combo_info' in feature:
        ...
        curr_embed_dim = extra_combo_info.get(
            'embedding_dimension', extra_combo_info.get('embedding_dim', embedding_dim)
        )
        ...
        try:
            if 'feature_name' in feature:
                ...
                pipeline_config = process_features(
                    feature_type, feature_name, feature, pipeline_config, embedding_dim, incol_separator
                )
...
def convert_rtp_fg(
    ...
    """
        ...
        feature_groups = {}
        group_map = {'u': 'user', 'i': 'item', 'ctx': 'combo', 'q': 'combo', 'comb': 'combo'}
        for feature in rtp_features:
