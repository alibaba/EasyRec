...
tf.app.flags.DEFINE_bool(
  'with_header', False, 'whether the input csv file has header'
)
...
tf.app.flags.DEFINE_string(
  'output_sep', chr(1), 'separator of predict result file'
)
  ...
  if FLAGS.saved_model_dir:
    ...
    if FLAGS.model_dir:
      ...
      if file_io.file_exists(pipeline_config_path):
        logging.info(
          'update pipeline_config_path to %s' % pipeline_config_path
        )
      else:
        pipeline_config_path = FLAGS.pipeline_config_path
    else:
    ...
    pred_result = predict(
      pipeline_config_path, FLAGS.checkpoint_path, FLAGS.input_path
    )
