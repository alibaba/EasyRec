...
from easy_rec.python.compat.feature_column import (
...
class FeatureColumnParser(object):
  ...
  def __init__(self, feature_configs, wide_deep_dict={}, wide_output_dim=-1, ev_params=None):
    ...
    def _cmp_embed_config(a, b):
      return (
        ...
        and a.combiner == b.combiner
        and a.initializer == b.initializer
        and a.max_partitions == b.max_partitions
        and a.embedding_name == b.embedding_name
      )
    ...
    not_shared = [x for x in self._share_embed_names if self._share_embed_names[x] == 1]
    ...
    self._deep_share_embed_columns = {embed_name: [] for embed_name in self._share_embed_names}
    self._wide_share_embed_columns = {embed_name: [] for embed_name in self._share_embed_names}
    ...
    for embed_name in self._share_embed_names:
      ...
      if self._share_embed_infos[embed_name].HasField('initializer'):
        initializer = hyperparams_builder.build_initializer(self._share_embed_infos[embed_name].initializer)
  ...
  def parse_id_feature(self, config):
    ...
    """
    feature_name = config.feature_name if config.HasField('feature_name') else config.input_names[0]
  ...
  def parse_raw_feature(self, config):
    ...
    """
    feature_name = config.feature_name if config.HasField('feature_name') else config.input_names[0]
    fc = feature_column.numeric_column(key=feature_name, shape=(config.raw_input_dim,), feature_name=feature_name)
    ...
    if config.boundaries:
      ...
      logging.info('discrete %s into %d buckets' % (feature_name, config.num_buckets))

    if bounds:
      try:
        fc = feature_column.bucketized_column(fc, bounds)
      except Exception as e:
        logging.error('bucketized_column [%s] with bounds %s error' % (fc.name, str(bounds)))
        raise e
      if self.is_wide(config):
        self._add_wide_embedding_column(fc, config)
      if self.is_deep(config):
  ...
  def parse_expr_feature(self, config):
    ...
    """
    feature_name = config.feature_name if config.HasField('feature_name') else config.input_names[0]
    fc = feature_column.numeric_column(feature_name, shape=(1,), feature_name=feature_name)
    if self.is_wide(config):
      self._add_wide_embedding_column(fc, config)
    if self.is_deep(config):
      self._deep_columns[feature_name] = fc
  ...
  def parse_combo_feature(self, config):
    ...
    Args:
      config: instance of easy_rec.python.protos.feature_config_pb2.FeatureConfig
    """
    feature_name = config.feature_name if config.HasField('feature_name') else None
  ...
  def parse_lookup_feature(self, config):
    ...
    hash_bucket_size = self._get_hash_bucket_size(config)
    fc = feature_column.categorical_column_with_hash_bucket(
      feature_name, hash_bucket_size, dtype=tf.string, feature_name=feature_name
    )
  ...
  def parse_sequence_feature(self, config):
    ...
    if sub_feature_type == config.IdFeature:
      ...
      bounds = None
      fc = sequence_feature_column.sequence_numeric_column(feature_name, shape=(1,), feature_name=feature_name)
      if config.hash_bucket_size > 0:
        ...
        # the feature values are already normalized into [0, 1]
        bounds = [x / float(config.num_buckets) for x in range(0, config.num_buckets)]
        logging.info('sequence feature discrete %s into %d buckets' % (feature_name, config.num_buckets))
      if bounds:
        try:
          fc = sequence_feature_column.sequence_numeric_column_with_bucketized_column(fc, bounds)
        except Exception as e:
          logging.error('sequence features bucketized_column [%s] with bounds %s error' % (feature_name, str(bounds)))
          raise e
  ...
  def _build_partitioner(self, config):
    if config.max_partitions > 1:
      if self._global_ev_params is not None or config.HasField('ev_params'):
        ...
        return partitioned_variables.fixed_size_partitioner(num_shards=config.max_partitions)
      else:
        return partitioned_variables.min_max_variable_partitioner(max_partitions=config.max_partitions)
    else:
      return None
  ...
  def _add_deep_embedding_column(self, fc, config):
    ...
    if config.embedding_name in self._deep_share_embed_columns:
      ...
      fc.max_seq_length = config.max_seq_len if config.HasField('max_seq_len') else -1
