...
class InputLayer(object):
  ...
  def __init__(
    ...
    self._fc_parser = FeatureColumnParser(feature_configs, wide_and_deep_dict, wide_output_dim, ev_params=ev_params)
  ...
  def get_combined_feature(self, features, group_name, is_dict=False):
    ...
    if group_name in self._group_name_to_seq_features:
      ...
      concat_features = array_ops.concat([concat_features, all_seq_fea], axis=-1)
    if is_dict:
  ...
  def get_sequence_feature(self, features, group_name):
    ...
    if self._variational_dropout_config is not None:
      raise ValueError('variational dropout is not supported in not combined mode now.')
    ...
    for fc in group_seq_columns:
      with variable_scope.variable_scope('input_layer/' + fc.categorical_column.name):
  ...
  def __call__(self, features, group_name, is_combine=True, is_dict=False):
    ...
    with conditional(self._is_predicting and place_on_cpu, ops.device('/CPU:0')):
      ...
      plain_features, feature_list = self.get_plain_feature(features, group_name)
    return seq_features, plain_features, feature_list

  def single_call_input_layer(self, features, group_name, feature_name_to_output_tensors=None):
    """Get features by group_name.
    ...
    if self._variational_dropout_config is not None:
      features_dimension = OrderedDict([(k.raw_name, int(v.shape[-1])) for k, v in cols_to_output_tensors.items()])
      concat_features = array_ops.concat([output_features] + seq_features, axis=-1)
      ...
      group_features = tf.split(concat_features, list(features_dimension.values()), axis=-1)
    else:
      concat_features = array_ops.concat([output_features] + seq_features, axis=-1)
      group_features = [cols_to_output_tensors[x] for x in group_columns] + [
