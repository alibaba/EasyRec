...
from easy_rec.python.layers.duo_bst import DUOBST
...
class SequenceEncoder(object):
    ...
    def __call__(self, features, group_name, is_training=True, *args, **kwargs):
        ...
        for encoder in group_config.sequence_encoders:
            ...
            cl_params = encoder.cl_param
            aug_type = encoder.duo_aug_type
            if encoder_type == 'bst':
                bst = BST(encoder.bst, self._l2_reg, name=group_name)
                encoding = bst([seq_features, target_feature], is_training, cl_params=cl_params, **kwargs)
                outputs.append(encoding)

            if encoder_type == 'duobst':
                duobst = DUOBST(encoder.duobst, self._l2_reg, name=group_name)
                encoding = duobst([seq_features, target_feature], is_training, aug_type=aug_type, **kwargs)
                outputs.append(encoding)
