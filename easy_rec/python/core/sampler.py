        ...
        if 'TF_CONFIG' in os.environ:
            ...
            if 'ps' in tf_config['cluster']:
                ...
                if tf_config['task']['type'] in ['chief', 'master']:
                    ...
                    self._g.init(cluster=cluster, job_name='client', task_index=tf_config['task']['index'] + 2)
                # TODO(hongsheng.jhs): check cluster has evaluator or not?
                elif tf_config['task']['type'] == 'evaluator':
                    self._g.init(cluster=cluster, job_name='client', task_index=tf_config['task']['index'] + 1)
                    ...
                    self._g.init(cluster=cluster, job_name='server', task_index=tf_config['task']['index'])
            else:
                # worker mode
                ...
                if tf_config['task']['type'] in ['chief', 'master']:
                    ...
                    self._g.init(task_index=tf_config['task']['index'] + 1, task_count=task_count)
                # TODO(hongsheng.jhs): check cluster has evaluator or not?
        else:
            # local mode
            self._g.init()
...
class NegativeSampler(BaseSampler):
    ...
    def __init__(self, data_path, fields, num_sample, batch_size, attr_delimiter=':', num_eval_sample=None):
        ...
        self._g = gl.Graph().node(
            ...
            node_type='item',
            decoder=gl.Decoder(attr_types=self._attr_gl_types, weighted=True, attr_delimiter=attr_delimiter),
        )
        self._init_graph()
...
class NegativeSamplerV2(BaseSampler):
    ...
    def __init__(
        ...
        self._g = (
            gl.Graph()
            .node(tf.compat.as_str(user_data_path), node_type='user', decoder=gl.Decoder(weighted=True))
            .node(
            ...
            .edge(
                tf.compat.as_str(edge_data_path), edge_type=('user', 'item', 'edge'), decoder=gl.Decoder(weighted=True)
            )
        )
        self._init_graph()
