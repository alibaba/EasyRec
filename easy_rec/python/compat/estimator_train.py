...
def train_and_evaluate(estimator, train_spec, eval_spec):
  ...
  if config.task_type == run_config_lib.TaskType.EVALUATOR and config.task_id > 0:
    raise ValueError(
      'For distributed training, there can only be one `evaluator` task ' '(with task id 0).  Given task id {}'.format(
        config.task_id
      )
    )
  ...
  if estimator_utils.is_evaluator():
    export_dir_base = os.path.join(compat.as_str_any(estimator.model_dir), compat.as_str_any('export'))
    for exporter in eval_spec.exporters:
      if isinstance(exporter, FinalExporter):
        export_path = os.path.join(compat.as_str_any(export_dir_base), compat.as_str_any(exporter.name))
