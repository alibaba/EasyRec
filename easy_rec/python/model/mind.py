...
class MIND(EasyRecModel):
    def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
        ...
        assert (
            self._model_config.WhichOneof('model') == 'mind'
        ), 'invalid model config: %s' % self._model_config.WhichOneof('model')
        self._model_config = self._model_config.mind

        self._hist_seq_features = self._input_layer(self._feature_dict, 'hist', is_combine=False)
    ...
    def build_predict_graph(self):
        ...
        time_id_fea = [x[0] for x in self._hist_seq_features if 'time_id/' in x[0].name]
        time_id_fea = time_id_fea[0] if len(time_id_fea) > 0 else None

        hist_seq_feas = [x[0] for x in self._hist_seq_features if 'time_id/' not in x[0].name]
        ...
        if self._model_config.user_seq_combine == MINDConfig.SUM:
            ...
            for i in range(1, len(hist_embed_dims)):
                assert (
                    hist_embed_dims[i] == hist_embed_dims[0]
        ...
        if self._model_config.HasField('pre_capsule_dnn') and len(self._model_config.pre_capsule_dnn.hidden_units) > 0:
            pre_dnn_layer = dnn.DNN(
                self._model_config.pre_capsule_dnn, self._l2_reg, 'pre_capsule_dnn', self._is_training
            )
        ...
        user_features = tf.tile(tf.expand_dims(self._user_features, axis=1), [1, tf.shape(high_capsules)[1], 1])
        ...
        assert self._model_config.simi_func in [Similarity.COSINE, Similarity.INNER_PRODUCT]
        ...
        self._prediction_dict['user_emb'] = tf.reduce_join(
            tf.reduce_join(tf.as_string(user_features), axis=-1, separator=','), axis=-1, separator='|'
        )
    ...
    def _build_interest_metric(self):
        ...
        simi = tf.reduce_sum(simi, axis=1) / tf.maximum(tf.to_float(user_feature_num * (user_feature_num - 1)), 1.0)
    ...
    def build_metric_graph(self, eval_config):
        ...
        for metric in eval_config.metrics_set:
            if metric.WhichOneof('metric') == 'auc':
                ...
                metric_dict['auc'] = metrics.auc(self._labels[0], self._prediction_dict['logits'])
