...
class RankModel(EasyRecModel):
    def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
        super(RankModel, self).__init__(model_config, feature_configs, features, labels, is_training)
    ...
    def _output_to_prediction_impl(self, output, loss_type, num_class=1, suffix=''):
        prediction_dict = {}
    ...
    def _add_to_prediction_dict(self, output):
        self._prediction_dict.update(
            self._output_to_prediction_impl(output, loss_type=self._loss_type, num_class=self._num_class)
        )

    def _build_loss_impl(self, loss_type, label_name, loss_weight=1.0, num_class=1, suffix=''):
        loss_dict = {}
        if loss_type == LossType.CLASSIFICATION:
        ...
        loss_dict[loss_name] = loss_builder.build(loss_type, self._labels[label_name], pred, loss_weight, num_class)
        return loss_dict

    def build_loss_graph(self):
        self._loss_dict.update(
            self._build_loss_impl(
                self._loss_type, label_name=self._label_name, loss_weight=self._sample_weight, num_class=self._num_class
            )
        )

        # build kd loss
        kd_loss_dict = loss_builder.build_kd_loss(self.kd, self._prediction_dict, self._labels)
        self._loss_dict.update(kd_loss_dict)

        return self._loss_dict

    def _build_metric_impl(self, metric, loss_type, label_name, num_class=1, suffix=''):
        metric_dict = {}
        if metric.WhichOneof('metric') == 'auc':
            ...
            if num_class == 1:
                ...
                metric_dict['auc' + suffix] = tf.metrics.auc(
                    label, self._prediction_dict['probs' + suffix], num_thresholds=metric.auc.num_thresholds
                )
            elif num_class == 2:
                label = tf.to_int64(self._labels[label_name])
                metric_dict['auc' + suffix] = tf.metrics.auc(
                    label, self._prediction_dict['probs' + suffix][:, 1], num_thresholds=metric.auc.num_thresholds
                )
            else:
            ...
            if num_class == 1:
                label = tf.to_int64(self._labels[label_name])
                metric_dict['f1' + suffix] = metrics_lib.max_f1(label, self._prediction_dict['logits' + suffix])
            elif num_class == 2:
                label = tf.to_int64(self._labels[label_name])
                metric_dict['f1' + suffix] = metrics_lib.max_f1(label, self._prediction_dict['logits' + suffix][:, 1])
            else:
                raise ValueError('Wrong class number')
            ...
            metric_dict['recall_at_topk' + suffix] = tf.metrics.recall_at_k(
                label, self._prediction_dict['logits' + suffix], metric.recall_at_topk.topk
            )
            ...
            if loss_type in [LossType.L2_LOSS, LossType.SIGMOID_L2_LOSS]:
                metric_dict['mean_absolute_error' + suffix] = tf.metrics.mean_absolute_error(
            ...
            label = tf.to_float(self._labels[label_name])
            if loss_type in [LossType.L2_LOSS, LossType.SIGMOID_L2_LOSS]:
                metric_dict['mean_squared_error' + suffix] = tf.metrics.mean_squared_error(
                    label, self._prediction_dict['y' + suffix]
                )
            elif loss_type == LossType.CLASSIFICATION and num_class == 1:
                metric_dict['mean_squared_error' + suffix] = tf.metrics.mean_squared_error(
            ...
            label = tf.to_float(self._labels[label_name])
            if loss_type in [LossType.L2_LOSS, LossType.SIGMOID_L2_LOSS]:
                metric_dict['root_mean_squared_error' + suffix] = tf.metrics.root_mean_squared_error(
                    label, self._prediction_dict['y' + suffix]
                )
            elif loss_type == LossType.CLASSIFICATION and num_class == 1:
                metric_dict['root_mean_squared_error' + suffix] = tf.metrics.root_mean_squared_error(
            ...
            assert num_class > 1
            label = tf.to_int64(self._labels[label_name])
            metric_dict['accuracy' + suffix] = tf.metrics.accuracy(label, self._prediction_dict['y' + suffix])
        return metric_dict

    def _build_distribute_metric_impl(self, metric, loss_type, label_name, num_class=1, suffix=''):
        if pai_util.is_on_pai():
            from easy_rec.python.core import (  # NOQA
                metrics_impl_pai as distribute_metrics_tf,
            )
        else:
            from easy_rec.python.core import (  # NOQA
                metrics_impl_tf as distribute_metrics_tf,
            )
        metric_dict = {}
        if metric.WhichOneof('metric') == 'auc':
            assert loss_type == LossType.CLASSIFICATION
            if num_class == 1:
                label = tf.to_int64(self._labels[label_name])
                metric_dict['auc' + suffix] = distribute_metrics_tf.auc(label, self._prediction_dict['probs' + suffix])
            elif num_class == 2:
                label = tf.to_int64(self._labels[label_name])
            ...
            label = tf.to_float(self._labels[label_name])
            if loss_type in [LossType.L2_LOSS, LossType.SIGMOID_L2_LOSS]:
                metric_dict['mean_absolute_error' + suffix] = distribute_metrics_tf.mean_absolute_error(
                    label, self._prediction_dict['y' + suffix]
                )
            elif loss_type == LossType.CLASSIFICATION and num_class == 1:
                metric_dict['mean_absolute_error' + suffix] = distribute_metrics_tf.mean_absolute_error(
            ...
            label = tf.to_float(self._labels[label_name])
            if loss_type in [LossType.L2_LOSS, LossType.SIGMOID_L2_LOSS]:
                metric_dict['mean_squared_error' + suffix] = distribute_metrics_tf.mean_squared_error(
                    label, self._prediction_dict['y' + suffix]
                )
            elif loss_type == LossType.CLASSIFICATION and num_class == 1:
                metric_dict['mean_squared_error' + suffix] = distribute_metrics_tf.mean_squared_error(
            ...
            label = tf.to_float(self._labels[label_name])
            if loss_type in [LossType.L2_LOSS, LossType.SIGMOID_L2_LOSS]:
                metric_dict['root_mean_squared_error' + suffix] = distribute_metrics_tf.root_mean_squared_error(
                    label, self._prediction_dict['y' + suffix]
                )
            elif loss_type == LossType.CLASSIFICATION and num_class == 1:
                metric_dict['root_mean_squared_error' + suffix] = distribute_metrics_tf.root_mean_squared_error(
        ...
        for metric in eval_config.metrics_set:
            metric_dict.update(
                self._build_metric_impl(
                    metric, loss_type=self._loss_type, label_name=self._label_name, num_class=self._num_class
                )
            )
        return metric_dict
