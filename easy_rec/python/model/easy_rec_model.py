...
class EasyRecModel(six.with_metaclass(_meta_type, object)):
  ...
  def l2_regularization(self):
    ...
    if hasattr(model_config, 'dense_regularization') and model_config.HasField('dense_regularization'):
      ...
      logging.warn('dense_regularization is deprecated, please use l2_regularization')
  ...
  def build_input_layer(self, model_config, feature_configs):
    self._input_layer = input_layer.InputLayer(
      ...
      variational_dropout_config=(
        model_config.variational_dropout if model_config.HasField('variational_dropout') else None
      ),
  ...
  def build_feature_output_dict(self):
    ...
    for feature_name in self._feature_dict:
      ...
      if isinstance(feature_value, tf.SparseTensor):
        ...
        feature_value = tf.sparse_to_dense(feature_value.indices, feature_value.dense_shape, sparse_values, '')
      ...
      if variable_name in ckpt_var2shape_map:
        ...
        if ckpt_var_shape == var_shape:
          vars_in_ckpt[variable_name] = (
            list(variable) if isinstance(variable, variables.PartitionedVariable) else variable
          )
        ...
        with ops.control_dependencies([variable._initializer_op]):
          variable._initializer_op = dynamic_variable_ops.dummy_var_assign(variable.handle, keys, vals)
      else:
  ...
  def _get_restore_vars(self, ckpt_var_map_path):
    ...
    for one_var in all_vars:
      ...
      if re.search(PARTITION_PATTERN, var_name) and one_var._save_slice_info is not None:
  ...
  def get_grouped_vars(self, opt_num):
    ...
    for tmp_var in variables.trainable_variables():
      if tmp_var.name.startswith('input_layer') or '/embedding_weights' in tmp_var.name:
