...
class HiveRTPInput(Input):
  ...
  def __init__(
    ...
    self._selected_cols = (
      [c.strip() for c in self._data_config.selected_cols.split(',')] if self._data_config.selected_cols else None
    )
    logging.info('select cols: %s' % self._selected_cols)
    hive_util = HiveUtils(data_config=self._data_config, hive_config=self._hive_config)
    self._input_hdfs_path = hive_util.get_table_location(self._hive_config.table_name)
    (
  ...
  def _parse_csv(self, line):
    ...
    for tid, field_name in enumerate(self._input_table_col_names):
      if field_name in self._selected_cols[:-1]:
        ...
        record_defaults.append(self.get_type_defaults(self._input_field_types[idx], self._input_field_defaults[idx]))
      else:
    ...
    record_types = [t for x, t in zip(self._input_fields, self._input_field_types) if x not in non_feature_cols]
    ...
    with tf.control_dependencies(check_list):
      fields = tf.string_split(fields[-1], self._data_config.separator, skip_empty=False)
    ...
    for i in range(feature_num):
      field = string_to_number(tmp_fields[:, i], record_types[i], rtp_record_defaults[i], i)
      fields.append(field)
  ...
  def _build(self, mode, params):
    ...
    assert len(file_paths) > 0, 'match no files with %s' % self._hive_config.table_name
