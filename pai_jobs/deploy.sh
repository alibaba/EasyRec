...
cd easy_rec_flow
sed -i -e "s/parameter name=\"version\" use=\"optional\" default=\"[0-9A-Za-z_-]\+\"/parameter name=\"version\" use=\"optional\" default=\"$VERSION\"/g" easy_rec.xml
tar -cvzf easy_rec_flow.tar.gz easy_rec.lua  easy_rec.xml

if [ $mode -ne 2 ]
then
  cd ../xflow-deploy
  package=../easy_rec_flow/easy_rec_flow.tar.gz
  ...
  rm -rf ../easy_rec_flow/easy_rec_flow ../easy_rec_flow/easy_rec_flow.tar.gz
else
  cd $curr_dir
  echo "Generated RESOURCES: $RES_PATH"
  echo "Generated XFLOW: easy_rec_flow/easy_rec_flow.tar.gz"
fi
