...
from easy_rec.python.layers import dnn, senet
...
class DSSM_SENet(DSSM):
  def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
    ...
    (
      self.user_seq_features,
      self.user_plain_features,
      self.user_feature_list,
    ) = self._input_layer(self._feature_dict, 'user', is_combine=False)
    ...
    ) = self._input_layer(self._feature_dict, 'item', is_combine=False)
