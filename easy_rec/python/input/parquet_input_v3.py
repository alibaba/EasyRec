...
class ParquetInputV3(Input):
    ...
    **kwargs,
    ...
    for f in data_config.input_fields:
      if f.HasField('ignore_val'):
        self._ignore_val_dict[f.input_name] = get_type_defaults(f.input_type, f.ignore_val)
  ...
  def _parse_dataframe_value(self, value):
    ...
    sparse_value = gen_ragged_conversion_ops.ragged_tensor_to_sparse(value.nested_row_splits, value.values)
  ...
  def _build(self, mode, params):
    ...
    logging.info('[task_index=%d] total_file_num=%d task_num=%d' % (self._task_index, file_num, self._task_num))
    ...
    num_parallel_reads = min(self._data_config.num_parallel_calls, len(input_files) // task_num)
