...
class TrainEvalTest(tf.test.TestCase):
  ...
  def test_keep_ckpt_max(self):
    ...
    def _post_check_func(pipeline_config):
      ckpt_prefix = os.path.join(
        pipeline_config.model_dir, 'model.ckpt-*.meta'
      )
  ...
  def _test_kd(self, config0, config1):
    ...
    def _gen_kd_data(train_path, eval_path):
      pred_result = predict(
        config_path, None, pipeline_config.train_input_path
      )
    ...
    config_util.save_pipeline_config(
      pipeline_config, self._test_dir, 'kd_pipeline.config'
    )
    self._success = test_utils.test_single_train_eval(
  ...
  def test_train_with_ps_worker_with_evaluator(self):
    ...
    logging.info(
      'final_export_dir=%s all_saved_files=%s' %
  ...
  def test_cmd_config_param(self):
    ...
    def _post_check_config(pipeline_config):
      train_saved_config_path = os.path.join(
        self._test_dir, 'train/pipeline.config'
      )
      pipeline_config = config_util.get_configs_from_pipeline_file(
  ...
  @unittest.skipIf(
    hvd is None or sok is None, 'horovod and sok is not installed'
  )
  def test_sok(self):
    self._success = test_utils.test_distributed_train_eval(
