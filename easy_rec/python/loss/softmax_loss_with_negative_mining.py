...
def support_vector_guided_softmax_loss(pos_score, neg_scores, margin=0, t=1, smooth=1.0, threshold=0, weights=1.0):
    ...
    loss = tf.losses.sparse_softmax_cross_entropy(tf.zeros_like(pos_score, dtype=tf.int32), logits, weights=weights)
...
def softmax_loss_with_negative_mining(
    user_emb, item_emb, labels, num_negative_samples=4, embed_normed=False, weights=1.0, gamma=1.0, margin=0, t=1
):
    """Softmax loss with negative mining.

    Given mini batches for `user_emb` and `item_emb`,
    this function computes for each element in `user_emb` the cosine distance between it
    and the corresponding `item_emb`, and additionally the cosine distance between `user_emb`
    ...
    sim_scores = tf.keras.backend.batch_dot(mask_user_emb, mask_item_emb, axes=(1, 2))
    pos_score = tf.slice(sim_scores, [0, 0], [-1, 1])
    neg_scores = tf.slice(sim_scores, [0, 1], [-1, -1])

    loss = support_vector_guided_softmax_loss(pos_score, neg_scores, margin=margin, t=t, smooth=gamma, weights=weights)
    return loss
