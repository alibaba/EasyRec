...
class CheckpointSaverHook(CheckpointSaverHook):
    ...
    def after_create_session(self, session, coord):
        ...
        if self._write_graph:
            ...
            tf.train.write_graph(
                tf.get_default_graph().as_graph_def(add_shapes=True), self._checkpoint_dir, 'graph.pbtxt'
            )
    ...
    def _save(self, session, step):
        ...
        self._get_saver().save(session, self._save_path, global_step=step, write_meta_graph=self._write_graph)
        save_dir, save_name = os.path.split(self._save_path)

        self._summary_writer.add_session_log(
            tf.SessionLog(status=tf.SessionLog.CHECKPOINT, checkpoint_path=self._save_path), step
        )
            ...
            for var in global_variables:
                ...
                if var_name in ckpt_var2shape_map:
                    if restore_status[var_name]:
                        logging.warning(
                            'variable %s find in more than one checkpoint, skipped %s' % (var_name, ckpt_path)
                        )
                        continue
