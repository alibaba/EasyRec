...
from tensorflow.keras.layers import Dense, Activation, BatchNormalization, Multiply
...
class CustomizedModel(Model):
    def __init__(self, model_conf, indim, name='export_model'):
        ...
        self.label_conf_list = [conf for conf in model_conf['label'] if conf.get('weight', 1) > 0]
        ...
        self.expert_list = [DNN(self.expert_layers, activation='relu', use_bn=True, output_activation='relu', need_ppnet=self.need_ppnet, pp_size=self.pp_feature_cnt*embed_size) for _ in range(self.experts)]

        self.tower_list = [DNN(self.tower_layers, activation='relu', use_bn=True, output_activation='sigmoid', need_ppnet=self.need_ppnet, pp_size=self.pp_feature_cnt*embed_size) for _ in self.label_conf_list]

        self.gate_list = [DNN([self.experts], activation='relu', use_bn=False, output_activation='linear') for _ in self.label_conf_list]
    ...
    def call(self, embed_input, training=False):
        ...
        for i, label_conf in enumerate(self.label_conf_list):
            # (N, n, 1)
            gate = tf.expand_dims(tf.nn.softmax(self.gate_list[i](embed_input)), axis=2)
            # (N, d1)
