...
from easy_rec.python.loss.softmax_loss_with_negative_mining import (
    softmax_loss_with_negative_mining,  # NOQA
)
...
def cosine_similarity(user_emb, item_emb):
    user_item_sim = tf.reduce_sum(tf.multiply(user_emb, item_emb), axis=1, name='cosine')
...
class DropoutNet(EasyRecModel):
    def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
        ...
        assert (
            self._model_config.WhichOneof('model') == 'dropoutnet'
    ...
    def build_predict_graph(self):
        ...
        assert (
            last_item_hidden == last_user_hidden
        ...
        with tf.name_scope('user_tower'):
            ...
            if self.user_content_feature is not None:
                user_content_dnn = dnn.DNN(self.user_content_layers, self._l2_reg, 'user_content', self._is_training)
            ...
            if self.user_preference_feature is not None:
                ...
                user_prefer_dnn = dnn.DNN(
                    self.user_preference_layers, self._l2_reg, 'user_preference', self._is_training
                )
            ...
            user_dnn = dnn.DNN(self.user_tower_layers, self._l2_reg, 'user_dnn', self._is_training)
        ...
        with tf.name_scope('item_tower'):
            ...
            if self.item_content_feature is not None:
                item_content_dnn = dnn.DNN(self.item_content_layers, self._l2_reg, 'item_content', self._is_training)
            ...
            if self.item_preference_feature is not None:
                ...
                item_prefer_dnn = dnn.DNN(
                    self.item_preference_layers, self._l2_reg, 'item_preference', self._is_training
                )
            ...
            item_dnn = dnn.DNN(self.item_tower_layers, self._l2_reg, 'item_dnn', self._is_training)
        ...
        self._prediction_dict['user_emb'] = tf.reduce_join(tf.as_string(user_emb), axis=-1, separator=',')
        self._prediction_dict['item_emb'] = tf.reduce_join(tf.as_string(item_emb), axis=-1, separator=',')
    ...
    def build_loss_graph(self):
        ...
        for loss in self._losses:
            if loss.loss_type == LossType.SOFTMAX_CROSS_ENTROPY_WITH_NEGATIVE_MINING:
                assert self._model_config.HasField('softmax_loss'), '`softmax_loss` must be configured'
                ...
                loss_value = tf.losses.sigmoid_cross_entropy(labels, logits, self._sample_weight)
    ...
    def build_metric_graph(self, eval_config):
        ...
        for metric in eval_config.metrics_set:
            if metric.WhichOneof('metric') == 'auc':
                metric_dict['auc'] = metrics.auc(labels, prob, weights=self._sample_weight)
                ...
                metric_dict['precision'] = metrics.precision(labels, predict, weights=self._sample_weight)
            elif metric.WhichOneof('metric') == 'recall':
                metric_dict['recall'] = metrics.recall(labels, predict, weights=self._sample_weight)
            else:
