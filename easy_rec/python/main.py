...
def _create_eval_export_spec(pipeline_config, eval_data, check_mode=False):
  ...
  eval_input_fn = _get_input_fn(
    data_config, feature_configs, eval_data, **input_fn_kwargs
  )
...
def train_and_evaluate(pipeline_config_path, continue_train=False):
  ...
  ), 'pipeline_config_path not exists'
...
def _train_and_evaluate_impl(
  ...
  if estimator_utils.is_chief():
    ...
    config_util.save_pipeline_config(
      pipeline_config, pipeline_config.model_dir
    )
  ...
  if fit_on_eval and (not estimator_utils.is_evaluator()):
    ...
    if fit_on_eval_steps is not None:
      ...
      logging.info(
        '\ttrain_steps=%d fit_on_eval_steps=%d' %
    ...
    estimator.train(
      ...
      hooks=list(train_spec.hooks),
      saving_listeners=train_spec.saving_listeners
      if hasattr(train_spec, 'saving_listeners') else None
    )
...
def evaluate(
  ...
  if server_target:
    ...
    input_iter = eval_spec.input_fn(mode=tf.estimator.ModeKeys.EVAL
                                   ).make_one_shot_iterator()
    input_feas, input_lbls = input_iter.get_next()
    from tensorflow.python.framework.ops import device
    from tensorflow.python.training.device_setter import replica_device_setter
    from tensorflow.python.training.monitored_session import ChiefSessionCreator  # NOQA
    ...
    with device(
      ...
      estimator_spec = estimator._eval_model_fn(
...
def distribute_evaluate(
  ...
  ), 'tmp results dir not create success.'
  ...
  if server_target:
    ...
    from tensorflow.python.training.device_setter import replica_device_setter
    from tensorflow.python.training.monitored_session import ChiefSessionCreator  # NOQA
    from tensorflow.python.training.monitored_session import WorkerSessionCreator  # NOQA
    from tensorflow.python.training.monitored_session import MonitoredSession
    ...
    with device(
      replica_device_setter(
        ps_tasks=cur_ps_num, worker_device=cur_work_device, cluster=cluster
      )
    ):
      distribution = strategy_builder.build(train_config)
      estimator, run_config = _create_estimator(pipeline_config, distribution)
    ...
    if cur_job_name == 'master':
      ...
      model_ready_for_local_init_op = tf.variables_initializer(
        metric_variables
      )
    ...
    if cur_job_name == 'master':
      ...
      cur_hooks = EvaluateExitBarrierHook(
        cur_worker_num, True, ckpt_dir, metric_ops
      )
    else:
      cur_stop_grace_period_sesc = 10
      cur_hooks = EvaluateExitBarrierHook(
        cur_worker_num, False, ckpt_dir, metric_ops
      )
    with MonitoredSession(
...
def predict(pipeline_config, checkpoint_path='', data_path=None):
  ...
  pred_result = estimator.predict(
    eval_spec.input_fn, checkpoint_path=ckpt_path
  )
...
def export(
  ...
  **extra_params
  ...
  serving_input_fn = _get_input_fn(
    data_config, feature_configs, None, export_config, **input_fn_kwargs
  )
  ckpt_path = _get_ckpt_path(pipeline_config, checkpoint_path)
  if 'oss_path' in extra_params:
    ...
    return export_big_model_to_oss(
      export_dir, pipeline_config, extra_params, serving_input_fn, estimator,
      ckpt_path, verbose
    )
