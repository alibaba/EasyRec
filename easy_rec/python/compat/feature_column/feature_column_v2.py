...
from tensorflow.python.framework import (  # NOQA
    dtypes,
    ops,
    sparse_tensor as sparse_tensor_lib,
    tensor_shape,
)
...
from tensorflow.python.platform import gfile, tf_logging as logging
...
from easy_rec.python.compat.feature_column import (  # NOQA
    feature_column as fc_old,
    utils as fc_utils,
)
...
class StateManager(object):
    ...
    def create_variable(
        self, feature_column, name, shape, dtype=None, trainable=True, use_resource=True, initializer=None
    ):
...
class _BaseFeaturesLayer(Layer):
    ...
    def __init__(self, feature_columns, expected_column_type, trainable, name, **kwargs):
        super(_BaseFeaturesLayer, self).__init__(name=name, trainable=trainable, **kwargs)
        ...
        for column in self._feature_columns:
            if not isinstance(column, expected_column_type):
                raise ValueError(
                    ...
                    'embedding_column or indicator_column. Given: {}'.format(expected_column_type, column)
                )
...
class DenseFeatures(_BaseFeaturesLayer):
    ...
    def __init__(self, feature_columns, trainable=True, name=None, **kwargs):
        ...
        super(DenseFeatures, self).__init__(
            feature_columns=feature_columns, trainable=trainable, name=name, expected_column_type=DenseColumn, **kwargs
        )
...
class _LinearModelLayer(Layer):
    ...
    def call(self, features):
        ...
        with ops.name_scope(self.name):
            ...
            predictions = nn_ops.bias_add(predictions_no_bias, self.bias, name='weighted_sum')
            return predictions
...
def embedding_column(
    ...
    if (initializer is not None) and (not callable(initializer)):
        raise ValueError(
            'initializer must be callable if specified. ' 'Embedding of column_name: {}'.format(categorical_column.name)
        )
    if initializer is None:
        initializer = init_ops.truncated_normal_initializer(mean=0.0, stddev=0.01 / math.sqrt(dimension))
...
def shared_embedding_columns(
    ...
    if isinstance(
        c0, (fc_old._WeightedCategoricalColumn, WeightedCategoricalColumn)
    ):  # pylint: disable=protected-access
        c0 = c0.categorical_column
    for c in sorted_columns[1:]:
        if isinstance(
            c, (fc_old._WeightedCategoricalColumn, WeightedCategoricalColumn)
        ):  # pylint: disable=protected-access
        ...
        if num_buckets != c._num_buckets:  # pylint: disable=protected-access
            raise ValueError(
                ...
                'not match column: {} with buckets: {}'.format(c0, num_buckets, c, c._num_buckets)
            )  # pylint: disable=protected-access
    ...
    for c in sorted_columns[1:]:
        ...
        if num_buckets != c.num_buckets:
            raise ValueError(
                'To use shared_embedding_column, all categorical_columns must have '
                'the same number of buckets. Given column: {} with buckets: {} does  '
                'not match column: {} with buckets: {}'.format(c0, num_buckets, c, c.num_buckets)
            )
...
def bucketized_column(source_column, boundaries):
    ...
    if not isinstance(source_column, (NumericColumn, fc_old._NumericColumn)):  # pylint: disable=protected-access
        raise ValueError(
            'source_column must be a column generated with numeric_column(). ' 'Given: {}'.format(source_column)
        )
...
def categorical_column_with_hash_bucket(key, hash_bucket_size, dtype=dtypes.string):
    ...
    if hash_bucket_size < 1:
        raise ValueError(
            'hash_bucket_size must be at least 1. ' 'hash_bucket_size: {}, key: {}'.format(hash_bucket_size, key)
        )
...
def categorical_column_with_vocabulary_file(
    key, vocabulary_file, vocabulary_size=None, num_oov_buckets=0, default_value=None, dtype=dtypes.string
):
...
def categorical_column_with_vocabulary_file_v2(
    key, vocabulary_file, vocabulary_size=None, dtype=dtypes.string, default_value=None, num_oov_buckets=0
):
    """A `CategoricalColumn` with a vocabulary file.
    ...
    if vocabulary_size is None:
        ...
        logging.info(
            'vocabulary_size = %d in %s is inferred from the number of elements ' 'in the vocabulary_file %s.',
            vocabulary_size,
            key,
            vocabulary_file,
        )
...
def categorical_column_with_vocabulary_list(key, vocabulary_list, dtype=None, default_value=-1, num_oov_buckets=0):
    ...
    if dtype is None:
        ...
        raise ValueError(
            'dtype {} and vocabulary dtype {} do not match, column_name: {}'.format(dtype, vocabulary_dtype, key)
        )
    fc_utils.assert_string_or_int(dtype, prefix='column_name: {}'.format(key))
    fc_utils.assert_key_is_string(key)
...
def categorical_column_with_identity(key, num_buckets, default_value=None):
    ...
    if (default_value is not None) and ((default_value < 0) or (default_value >= num_buckets)):
        raise ValueError(
            'default_value {} not in range [0, {}), column_name {}'.format(default_value, num_buckets, key)
        )
    fc_utils.assert_key_is_string(key)
...
def _create_weighted_sum(column, transformation_cache, state_manager, sparse_combiner, weight_var):
    ...
    if isinstance(column, CategoricalColumn):
        ...
        return _create_dense_column_weighted_sum(
            column=column, transformation_cache=transformation_cache, state_manager=state_manager, weight_var=weight_var
        )
...
def _create_categorical_column_weighted_sum(column, transformation_cache, state_manager, sparse_combiner, weight_var):
    ...
    return embedding_ops.safe_embedding_lookup_sparse(
        weight_var, id_tensor, sparse_weights=weight_tensor, combiner=sparse_combiner, name='weighted_sum'
    )
        ...
        with ops.control_dependencies(
            [
                check_ops.assert_positive(
                    ...
                    message='Feature (key: {}) cannot have rank 0. Given: {}'.format(key, feature_tensor),
                )
            ]
        ):
...
def _to_sparse_input_and_drop_ignore_values(input_tensor, ignore_value=None):
    ...
    with ops.name_scope(
        ...
        return sparse_tensor_lib.SparseTensor(
            ...
            dense_shape=array_ops.shape(input_tensor, out_type=dtypes.int64, name='dense_shape'),
        )
...
def _normalize_feature_columns(feature_columns):
    ...
    for column in feature_columns:
        if not isinstance(column, FeatureColumn):
            raise ValueError(
                'Items of feature_columns must be a FeatureColumn. ' 'Given (type {}): {}.'.format(type(column), column)
            )
...
class NumericColumn(
    ...
    collections.namedtuple('NumericColumn', ('key', 'shape', 'default_value', 'dtype', 'normalizer_fn')),
):
    """see `numeric_column`."""
    ...
    def _from_config(cls, config, custom_objects=None, columns_by_name=None):
        ...
        kwargs = config.copy()
        kwargs['normalizer_fn'] = utils.deserialize_keras_object(config['normalizer_fn'], custom_objects=custom_objects)
        kwargs['dtype'] = dtypes.as_dtype(config['dtype'])
        return cls(**kwargs)
...
class BucketizedColumn(
    ...
    def _is_v2_column(self):
        return (
            isinstance(self.source_column, FeatureColumn) and self.source_column._is_v2_column
        )  # pylint: disable=protected-access
    ...
    def _transform_feature(self, inputs):
        ...
        return math_ops._bucketize(source_tensor, boundaries=self.boundaries)  # pylint: disable=protected-access
    ...
    def _get_sparse_tensors_for_input_tensor(self, input_tensor):
        ...
        i1 = array_ops.reshape(
            array_ops.tile(array_ops.expand_dims(math_ops.range(0, batch_size), 1), [1, source_dimension]), (-1,)
        )
        ...
        dense_shape = math_ops.cast(array_ops.stack([batch_size, source_dimension]), dtypes.int64)
...
class EmbeddingColumn(
    ...
    def _is_v2_column(self):
        return (
            isinstance(self.categorical_column, FeatureColumn) and self.categorical_column._is_v2_column
        )  # pylint: disable=protected-access
    ...
    def _old_get_dense_tensor_internal(self, sparse_tensors, weight_collections, trainable):
        ...
        if not self.use_embedding_variable:
            ...
            if os.environ.get('tf.estimator.mode', '') != os.environ.get('tf.estimator.ModeKeys.TRAIN', 'train'):
                initializer = init_ops.zeros_initializer()
            else:
                initializer = self.initializer
    ...
    def _from_config(cls, config, custom_objects=None, columns_by_name=None):
        ...
        kwargs['initializer'] = utils.deserialize_keras_object(config['initializer'], custom_objects=custom_objects)
        return cls(**kwargs)
...
class SharedEmbeddingColumn(
    ...
    fc_old._SequenceDenseColumn,  # pylint: disable=protected-access
    collections.namedtuple(
        'SharedEmbeddingColumn', ('categorical_column', 'shared_embedding_column_creator', 'combiner', 'max_norm')
    ),
):
    """See `embedding_column`."""
...
class HashedCategoricalColumn(
    CategoricalColumn,
    fc_old._CategoricalColumn,  # pylint: disable=protected-access
    collections.namedtuple('HashedCategoricalColumn', ('key', 'hash_bucket_size', 'dtype')),
):
    ...
    def _transform_input_tensor(self, input_tensor):
        ...
        if self.dtype.is_integer != input_tensor.dtype.is_integer:
            raise ValueError(
                ...
                'key: {}, column dtype: {}, tensor dtype: {}'.format(self.key, self.dtype, input_tensor.dtype)
            )
...
class VocabularyFileCategoricalColumn(
    ...
    collections.namedtuple(
        ...
        ('key', 'vocabulary_file', 'vocabulary_size', 'num_oov_buckets', 'dtype', 'default_value'),
    ),
):
...
class IdentityCategoricalColumn(
    CategoricalColumn,
    fc_old._CategoricalColumn,  # pylint: disable=protected-access
    collections.namedtuple('IdentityCategoricalColumn', ('key', 'number_buckets', 'default_value')),
):
    ...
    def _transform_input_tensor(self, input_tensor):
        ...
        if self.default_value is None:
            ...
            assert_less = check_ops.assert_less(
                values, num_buckets, data=(values, num_buckets), name='assert_less_than_num_buckets'
            )
            ...
            values = array_ops.where(
                math_ops.logical_or(values < zero, values >= num_buckets, name='out_of_range'),
                array_ops.fill(
        ...
        return sparse_tensor_lib.SparseTensor(
            indices=input_tensor.indices, values=values, dense_shape=input_tensor.dense_shape
        )
...
class WeightedCategoricalColumn(
    CategoricalColumn,
    fc_old._CategoricalColumn,  # pylint: disable=protected-access
    collections.namedtuple('WeightedCategoricalColumn', ('categorical_column', 'weight_feature_key', 'dtype')),
):
    ...
    def transform_feature(self, transformation_cache, state_manager):
        ...
        return (transformation_cache.get(self.categorical_column, state_manager), weight_tensor)
...
class CrossedColumn(
    ...
    def transform_feature(self, transformation_cache, state_manager):
        ...
        return sparse_ops.sparse_cross_hashed(
            inputs=feature_tensors, num_buckets=self.hash_bucket_size, hash_key=self.hash_key
        )
    ...
    def _transform_id_weight_pair(self, id_weight_pair):
        ...
        if weight_tensor is not None:
            weighted_column = sparse_ops.sparse_merge(
                sp_ids=id_tensor, sp_values=weight_tensor, vocab_size=int(self._variable_shape[-1])
            )
            # Remove (?, -1) index.
            ...
            if expected_batch_size is None:
                ...
                raise ValueError(
                    ...
                    'Batch size of columns ({}, {}): ({}, {})'.format(
                        columns[bath_size_column_index].name, columns[i].name, expected_batch_size, batch_size
                    )
                )
...
def serialize_feature_column(fc):
    ...
    if isinstance(fc, six.string_types):
        ...
        return utils.serialize_keras_class_and_config(fc.__class__.__name__, fc._get_config())
    else:
