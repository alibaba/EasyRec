...
def circle_loss(embeddings, labels, sessions=None, margin=0.25, gamma=32, embed_normed=False):
    """paper: Circle Loss: A Unified Perspective of Pair Similarity Optimization http://arxiv.org/pdf/2002.10857.pdf.
    ...
    """
    norm_embeddings = embeddings if embed_normed else tf.nn.l2_normalize(embeddings, axis=-1)
    pair_wise_cosine_matrix = tf.matmul(norm_embeddings, norm_embeddings, transpose_b=True)
    ...
    ap = tf.nn.relu(-tf.stop_gradient(pair_wise_cosine_matrix * positive_mask) + 1 + margin)
    an = tf.nn.relu(tf.stop_gradient(pair_wise_cosine_matrix * negative_mask) + margin)

    logit_p = -ap * (pair_wise_cosine_matrix - delta_p) * gamma * positive_mask - (1 - positive_mask) * 1e12
    logit_n = an * (pair_wise_cosine_matrix - delta_n) * gamma * negative_mask - (1 - negative_mask) * 1e12
...
def get_anchor_positive_triplet_mask(labels, sessions=None):
    ...
    if sessions is None or sessions is labels:
        ...
        sessions_equal = tf.equal(tf.expand_dims(sessions, 0), tf.expand_dims(sessions, 1))
