...
def auto_import(user_path=None):
    ...
    if parent_dir != '':
        for idx in range(len(pre_defined_dirs)):
            pre_defined_dirs[idx] = (os.path.join(parent_dir, pre_defined_dirs[idx][0]), pre_defined_dirs[idx][1])
...
def register_class(class_map, class_name, cls):
    assert (
        class_name not in class_map or class_map[class_name] == cls
    ), 'confilict class %s , %s is already register to be %s' % (cls, class_name, str(class_map[class_name]))
        ...
        def __new__(mcs, name, bases, attrs):
            ...
            def create_class(cls, name):
                if name in class_map:
                    ...
                    raise Exception(
                        'Class %s is not registered. Available ones are %s' % (name, list(class_map.keys()))
                    )
