...
import logging
import odps
from easy_rec.python.hpo_nni.pai_nni.code.utils import set_value, try_parse
...
def create_odps(project, access_id, access_key, endpoint, biz_id=None):
    def init():
        ...
        o = odps.ODPS(access_id=access_id,
        ...
        if not o.exist_project(proj):
            raise ValueError(
                'ODPS init failed, please check your project name.')
        return o
...
def run_command(o, easyrec_cmd_config, trial_id=None):
    ...
    instance = o.run_xflow(xflow_name=command.name,
