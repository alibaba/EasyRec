...
class PDN(MatchModel):
  ...
  def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
    ...
    u2i_seqs = self._input_layer(self._feature_dict,
                                           'u2i', is_combine=False)
    self._seq_len = u2i_seqs[0][1]
    self._u2i_seq = tf.concat([ x[0] for x in u2i_seqs ], axis=2)
    # item side info
    i_seqs = self._input_layer(self._feature_dict, 'i_seq', is_combine=False)
    self._i_seq = tf.concat([ x[0] for x in i_seqs ], axis=2)

    # item to target similarities
    i2i_seqs = self._input_layer(self._feature_dict, 'i2i', is_combine=False)
  ...
  def build_predict_graph(self):
    ...
    u2i_dnn_layer = dnn.DNN(self._model_config.u2i_dnn, self._l2_reg,
                            'u2i_dnn', self._is_training)
    ...
    sim_seq_concat = tf.concat([sim_seq_cross, sim_seq_cross, self._i2i_seq,
