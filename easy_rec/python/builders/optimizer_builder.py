...
def build(optimizer_config):
    ...
    if optimizer_type == 'rms_prop_optimizer':
        ...
        optimizer = tf.train.RMSPropOptimizer(
            learning_rate, decay=config.decay, momentum=config.momentum_optimizer_value, epsilon=config.epsilon
        )
    ...
    if optimizer_type == 'adam_optimizer':
        ...
        learning_rate = _create_learning_rate(config.learning_rate)
        summary_vars.append(learning_rate)
        optimizer = tf.train.AdamOptimizer(learning_rate, beta1=config.beta1, beta2=config.beta2)
    ...
    if optimizer_type == 'adamw_optimizer':
        ...
        optimizer = weight_decay_optimizers.AdamWOptimizer(
            weight_decay=config.weight_decay, learning_rate=learning_rate, beta1=config.beta1, beta2=config.beta2
        )
    ...
    if optimizer_type == 'momentumw_optimizer':
        ...
        optimizer = weight_decay_optimizers.MomentumWOptimizer(
            weight_decay=config.weight_decay, learning_rate=learning_rate, momentum=config.momentum_optimizer_value
        )
    ...
    if optimizer_type == 'adam_async_optimizer':
        ...
        learning_rate = _create_learning_rate(config.learning_rate)
        summary_vars.append(learning_rate)
        optimizer = tf.train.AdamAsyncOptimizer(learning_rate, beta1=config.beta1, beta2=config.beta2)
...
def _create_learning_rate(learning_rate_config):
    ...
    if learning_rate_type == 'manual_step_learning_rate':
        ...
        learning_rate = learning_schedules.manual_stepping(
            tf.train.get_or_create_global_step(), learning_rate_step_boundaries, learning_rate_sequence, config.warmup
        )
