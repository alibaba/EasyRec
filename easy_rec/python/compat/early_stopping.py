...
def stop_if_higher_hook(
    estimator, metric_name, threshold, eval_dir=None, min_steps=0, run_every_secs=60, run_every_steps=None
):
...
def custom_early_stop_hook(
    estimator, eval_dir, custom_stop_func, custom_stop_func_params, run_every_secs=60, run_every_steps=None
):
    ...
    if isinstance(custom_stop_func, str):
...
def _stop_if_threshold_crossed_hook(
    estimator, metric_name, threshold, higher_is_better, eval_dir, min_steps, run_every_secs, run_every_steps
):
        ...
        for step, metrics in eval_results.items():
            ...
            if is_lhs_better(val, threshold):
                tf_logging.info(
                    ...
                    'threshold (%s) for early stopping.',
                    step,
                    metric_name,
                    val,
                    greater_or_lesser,
...
def _get_or_create_stop_var():
    with variable_scope.variable_scope(name_or_scope=EARLY_STOP_SIG_SCOPE, values=[], reuse=variable_scope.AUTO_REUSE):
...
class _StopOnPredicateHook(session_run_hook.SessionRunHook):
    ...
    def __init__(self, should_stop_fn, run_every_secs=60, run_every_steps=None):
        ...
        self._timer = basic_session_run_hooks.SecondOrStepTimer(every_secs=run_every_secs, every_steps=run_every_steps)
    ...
    def after_run(self, run_context, run_values):
        ...
        if self._timer.should_trigger_for_step(global_step):
            ...
            if self._should_stop_fn():
                tf_logging.info('Requesting early stopping at global step %d', global_step)
