            ...
            with variable_scope.variable_scope(
                ...
                if cols_to_vars is not None:
                    ...
                    cols_to_vars[column] = ops.get_collection(
                        ops.GraphKeys.GLOBAL_VARIABLES, scope=variable_scope.get_variable_scope().name
                    )
...
class _BiasLayer(base.Layer):
    ...
    def build(self, _):
        self._bias_variable = self.add_variable(
            'bias_weights', shape=[self._units], initializer=init_ops.zeros_initializer(), trainable=self.trainable
        )
...
def _get_expanded_variable_list(variable):
    if isinstance(variable, variables.Variable) or resource_variable_ops.is_resource_variable(variable):
        ...
        **kwargs
        ...
        for column in sorted(self._feature_columns, key=lambda x: x.name):
            ...
            column_layer = _FCLinearWrapper(
                column, units, sparse_combiner, self._weight_collections, trainable, column_name, **kwargs
            )
            column_layers[column_name] = column_layer
        self._column_layers = self._add_layers(column_layers)
        self._bias_layer = _BiasLayer(
            units=units, trainable=trainable, weight_collections=self._weight_collections, name='bias_layer', **kwargs
        )
        self._cols_to_vars = {}
...
def _create_categorical_column_weighted_sum(
    column, builder, units, sparse_combiner, weight_collections, trainable, weight_var=None
):
    ...
    return embedding_ops.safe_embedding_lookup_sparse(
        weight, id_tensor, sparse_weights=weight_tensor, combiner=sparse_combiner, name='weighted_sum'
    )
...
class _NumericColumn(
    _DenseColumn, collections.namedtuple('_NumericColumn', ['key', 'shape', 'default_value', 'dtype', 'normalizer_fn'])
):
    """see `numeric_column`."""
...
class _BucketizedColumn(
    _DenseColumn, _CategoricalColumn, collections.namedtuple('_BucketizedColumn', ['source_column', 'boundaries'])
):
...
class _EmbeddingColumn(
    ...
    def _get_dense_tensor_internal(self, inputs, weight_collections=None, trainable=None):
        ...
        embedding_weights = self.layer_creator(
            weight_collections=weight_collections, scope=variable_scope.get_variable_scope()
        )
    ...
    def _get_dense_tensor_internal(self, inputs, weight_collections=None, trainable=None):
        ...
        with ops.name_scope(None, default_name=self.name):
            ...
            if shared_embedding_collection:
                ...
                if embedding_weights.get_shape() != embedding_shape and not self.use_embedding_variable:
                    raise ValueError(
...
class _HashedCategoricalColumn(
    _CategoricalColumn, collections.namedtuple('_HashedCategoricalColumn', ['key', 'hash_bucket_size', 'dtype'])
):
...
class _VocabularyListCategoricalColumn(
    _CategoricalColumn,
    collections.namedtuple(
        '_VocabularyListCategoricalColumn', ('key', 'vocabulary_list', 'dtype', 'default_value', 'num_oov_buckets')
    ),
):
...
class _IdentityCategoricalColumn(
    _CategoricalColumn, collections.namedtuple('_IdentityCategoricalColumn', ('key', 'num_buckets', 'default_value'))
):
...
class _WeightedCategoricalColumn(
    _CategoricalColumn,
    collections.namedtuple('_WeightedCategoricalColumn', ('categorical_column', 'weight_feature_key', 'dtype')),
):
...
class _CrossedColumn(
    _CategoricalColumn, collections.namedtuple('_CrossedColumn', ['keys', 'hash_bucket_size', 'hash_key'])
):
    """See `crossed_column`."""
...
class _IndicatorColumn(
    _DenseColumn, _SequenceDenseColumn, collections.namedtuple('_IndicatorColumn', ['categorical_column'])
):
...
class _SequenceCategoricalColumn(
    _CategoricalColumn, collections.namedtuple('_SequenceCategoricalColumn', ['categorical_column'])
):
