...
class ESMM(MultiTaskModel):
    def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
        super(ESMM, self).__init__(model_config, feature_configs, features, labels, is_training)
        assert (
            self._model_config.WhichOneof('model') == 'esmm'
        ...
        assert (
            self._model_config.ctr_tower.loss_type == LossType.CLASSIFICATION
    ...
    def build_loss_graph(self):
        ...
        if self._cvr_tower_cfg.loss_type == LossType.CLASSIFICATION:
            ...
            ctcvr_label = self._labels[cvr_label_name] * tf.cast(self._labels[ctr_label_name], cvr_dtype)
            cvr_loss = tf.losses.mean_squared_error(
                labels=ctcvr_label, predictions=self._prediction_dict['y_ctcvr'], weights=self._sample_weight
            )
            self._loss_dict['weighted_l2_loss_%s' % cvr_tower_name] = self._cvr_tower_cfg.weight * cvr_loss
    ...
    def build_predict_graph(self):
        ...
        if self._group_num > 0:
            ...
            for group_id in range(self._group_num):
                ...
                dnn_model = dnn.DNN(group.dnn, self._l2_reg, group_name, self._is_training)
        ...
        cvr_tower_output = tf.layers.dense(
            inputs=cvr_tower_output, units=1, kernel_regularizer=self._l2_reg, name='%s/dnn_output' % cvr_tower_name
        )
        ...
        ctr_tower_output = tf.layers.dense(
            inputs=ctr_tower_output, units=1, kernel_regularizer=self._l2_reg, name='%s/dnn_output' % ctr_tower_name
        )
    ...
    def get_outputs(self):
        ...
        if self._cvr_tower_cfg.loss_type == LossType.CLASSIFICATION:
            ...
            raise ValueError('invalid cvr_tower loss type: %s' % str(self._cvr_tower_cfg.loss_type))
        return outputs
