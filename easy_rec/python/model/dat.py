...
class DAT(MatchModel):
  ...
  def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
    super(DAT, self).__init__(model_config, feature_configs, features, labels, is_training)
    assert self._model_config.WhichOneof('model') == 'dat', 'invalid model config: %s' % self._model_config.WhichOneof(
      'model'
    )

    feature_group_names = [fg.group_name for fg in self._model_config.feature_groups]
    ...
    self.user_augmented_vec, _ = self._input_layer(self._feature_dict, 'user_id_augment')

    self.item_tower = copy_obj(self._model_config.item_tower)
    self.item_deep_feature, _ = self._input_layer(self._feature_dict, 'item')
    self.item_augmented_vec, _ = self._input_layer(self._feature_dict, 'item_id_augment')
  ...
  def build_predict_graph(self):
    ...
    user_tower_feature = tf.concat([self.user_deep_feature, self.user_augmented_vec], axis=-1)
    ...
    item_tower_feature = tf.concat([self.item_deep_feature, self.item_augmented_vec], axis=-1)
    ...
    if self._loss_type == LossType.CLASSIFICATION:
      raise ValueError('Currently DAT model only supports SOFTMAX_CROSS_ENTROPY loss.')
