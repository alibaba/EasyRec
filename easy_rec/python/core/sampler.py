...
class BaseSampler(object):
  ...
  def set_eval_num_sample(self):
    print(
      'set_eval_num_sample: %d %d' % (self._num_sample, self._num_eval_sample)
    )
  ...
  def _parse_nodes(self, nodes):
    if self._log_first_n > 0:
      logging.info(
        'num_example=%d num_eval_example=%d node_num=%d' %
    ...
    float_idx = 0
    string_idx = 0
    for attr_gl_type, attr_np_type in zip(
      self._attr_gl_types, self._attr_np_types
    ):
      if attr_gl_type == 'int':
...
class NegativeSamplerInMemory(BaseSampler):
  ...
  def __init__(
    ...
    super(NegativeSamplerInMemory,
          self).__init__(fields, num_sample, num_eval_sample)
    self._batch_size = batch_size
  ...
  def _load_table(self, data_path, attr_delimiter):
    ...
    for tid in range(len(schema)):
      if schema[tid][0].startswith('id'):
        ...
        break
    print(
      'NegativeSamplerInMemory: feature_id_col = %d, item_id_col = %d' %
        ...
        if line_id == 0:
          ...
          for tid in range(len(schema)):
            ...
            if schema[tid][0].startswith('feature'):
              fea_id_col = tid
          print(
            'feature_id_col = %d, item_id_col = %d' %
            (fea_id_col, item_id_col)
          )
        else:
          self._item_ids.append(int(cols[item_id_col]))
          fea_vals = cols[fea_id_col].split(attr_delimiter)
          assert len(fea_vals) == len(self._cols
                                     ), 'invalid row[%d][%d %d]:%s %s' % (
                                       line_id, len(fea_vals), len(self._cols),
                                       cols[item_id_col], cols[fea_id_col]
                                     )
...
class NegativeSamplerV2(BaseSampler):
  ...
  def get(self, src_ids, dst_ids):
    ...
    Returns:
      Negative sampled feature dict.
    """
    sampled_values = tf.py_func(
      self._get_impl, [src_ids, dst_ids], self._attr_tf_types
    )
    result_dict = {}
    for k, t, v in zip(self._attr_names, self._attr_tf_types, sampled_values):
      v.set_shape([self._num_sample])
...
class HardNegativeSampler(BaseSampler):
  ...
  def __init__(
    ...
    self._hard_neg_sampler = self._g.neighbor_sampler(
      ['hard_neg_edge'], num_hard_sample, strategy='full'
    )
  ...
  def _get_impl(self, src_ids, dst_ids):
    ...
    hard_neg_features, hard_neg_indices = self._parse_sparse_nodes(
      sparse_nodes
    )
    ...
    for i, v in enumerate(hard_neg_features):
      if type(v) == list:
        results.append(
          np.asarray(neg_features[i] + v, order='C', dtype=object)
        )
      else:
  ...
  def get(self, src_ids, dst_ids):
    ...
    output_values = tf.py_func(
      self._get_impl, [src_ids, dst_ids], output_types
    )
    result_dict = {}
    for k, t, v in zip(
      self._attr_names, self._attr_tf_types, output_values[:-1]
    ):
      v.set_shape([None])
      result_dict[k] = v
...
class HardNegativeSamplerV2(BaseSampler):
  ...
  def _get_impl(self, src_ids, dst_ids):
    src_ids = np.array(src_ids, dtype=np.int64)
    src_ids_padded = np.pad(
      src_ids, (0, self._batch_size - len(src_ids)), 'edge'
    )
    dst_ids = np.array(dst_ids, dtype=np.int64)
    dst_ids = np.pad(dst_ids, (0, self._batch_size - len(dst_ids)), 'edge')
