...
def _remove_squeezable_dimensions(predictions, labels, weights):
    ...
    if (predictions_rank is not None) and (weights_rank is not None):
        ...
        if (weights_rank is not None) and (not weights_shape.dims[-1].is_compatible_with(1)):
            ...
            maybe_squeeze_weights = lambda: array_ops.squeeze(weights, [-1])  # noqa: E731
...
def _confusion_matrix_at_thresholds(labels, predictions, thresholds, weights=None, includes=None):
    ...
    if 'tp' in includes:
        ...
        is_true_positive = math_ops.cast(math_ops.logical_and(label_is_pos, pred_is_pos), dtypes.int64)
    ...
    if 'fn' in includes:
        ...
        is_false_negative = math_ops.cast(math_ops.logical_and(label_is_pos, pred_is_neg), dtypes.int64)
        if weights_tiled is not None:
    ...
    if 'tn' in includes:
        ...
        is_true_negative = math_ops.cast(math_ops.logical_and(label_is_neg, pred_is_neg), dtypes.int64)
        if weights_tiled is not None:
    ...
    if 'fp' in includes:
        ...
        is_false_positive = math_ops.cast(math_ops.logical_and(label_is_neg, pred_is_pos), dtypes.int64)
        if weights_tiled is not None:
...
def _aggregate_variable(v, collections):
    f = lambda distribution, value: distribution.extended.read_var(value)  # noqa: E731
    ...
    with variable_scope.variable_scope(name, 'auc', (labels, predictions, weights)):
        ...
        def interpolate_pr_auc(tp, fp, fn):
            ...
            prec_slope = math_ops.div_no_nan(
                dtp, math_ops.maximum(p[: num_thresholds - 1] - p[1:], 0), name='prec_slope'
            )
            intercept = tp[1:] - math_ops.multiply(prec_slope, p[1:])
            safe_p_ratio = array_ops.where(
                math_ops.logical_and(p[: num_thresholds - 1] > 0, p[1:] > 0),
                math_ops.div_no_nan(p[: num_thresholds - 1], math_ops.maximum(p[1:], 0), name='recall_relative_ratio'),
...
def mean_tensor(values, weights=None, metrics_collections=None, updates_collections=None, name=None):
    ...
    with variable_scope.variable_scope(name, 'mean', (values, weights)):
        ...
        compute_mean = lambda _, t, c: math_ops.div_no_nan(t, math_ops.maximum(c, 0), name='value')  # noqa: E731
...
def _num_relevant(labels, k):
    ...
    with ops.name_scope(None, 'num_relevant', (labels,)) as scope:
        ...
        num_labels = math_ops.reduce_sum(
            array_ops.where_v2(
                math_ops.greater_equal(labels, 0), array_ops.ones_like(labels), array_ops.zeros_like(labels)
            ),
            axis=-1,
        )
...
def _sparse_average_precision_at_top_k(labels, predictions_idx):
    ...
    with ops.name_scope(None, 'average_precision', (predictions_idx, labels)) as scope:
        ...
        relevant_precision_per_k = math_ops.multiply(
            precision_per_k, math_ops.cast(relevant_per_k, dtypes.float64), name='relevant_precision_per_k'
        )
...
def _clean_out_of_range_indices(labels, num_classes):
    ...
    def _clean_labels_out_of_range():
        ...
        if _labels_is_sparse():
            return type(labels)(
                indices=labels.indices, values=_clean_out_of_range(labels.values), dense_shape=labels.dense_shape
            )
        else:
    ...
    return control_flow_ops.cond(
        math_ops.greater_equal(max_labels, num_classes), _clean_labels_out_of_range, lambda: labels
    )
