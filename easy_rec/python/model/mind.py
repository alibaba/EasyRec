...
class MIND(MatchModel):
  def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
    super(MIND, self).__init__(model_config, feature_configs, features, labels, is_training)
    assert self._model_config.WhichOneof('model') == 'mind', 'invalid model config: %s' % self._model_config.WhichOneof(
      'model'
    )
    self._model_config = self._model_config.mind

    self._hist_seq_features, _, _ = self._input_layer(self._feature_dict, 'hist', is_combine=False)
  ...
  def build_predict_graph(self):
    capsule_layer = CapsuleLayer(self._model_config.capsule_config, self._is_training)

    if self._model_config.time_id_fea:
      time_id_fea = [x[0] for x in self._hist_seq_features if self._model_config.time_id_fea in x[0].name]
      logging.info('time_id_fea is set(%s), find num: %d' % (self._model_config.time_id_fea, len(time_id_fea)))
    else:
      time_id_fea = []
    time_id_fea = time_id_fea[0] if len(time_id_fea) > 0 else None

    if time_id_fea is not None:
      hist_seq_feas = [x[0] for x in self._hist_seq_features if self._model_config.time_id_fea not in x[0].name]
    else:
    ...
    if self._model_config.user_seq_combine == MINDConfig.SUM:
      ...
      for i in range(1, len(hist_embed_dims)):
        assert hist_embed_dims[i] == hist_embed_dims[0], (
          'all hist seq must have the same embedding shape, but: %s' % str(hist_embed_dims)
        )
      hist_seq_feas = tf.add_n(hist_seq_feas) / len(hist_seq_feas)
    else:
      hist_seq_feas = tf.concat(hist_seq_feas, axis=2)

    if self._model_config.HasField('pre_capsule_dnn') and len(self._model_config.pre_capsule_dnn.hidden_units) > 0:
    ...
    if time_id_fea is not None:
      assert time_id_fea.get_shape()[-1] == 1, 'time_id must have only embedding_size of 1'
    ...
    high_capsules, num_high_capsules = capsule_layer(hist_seq_feas, hist_seq_len)
    ...
    user_dnn = dnn.DNN(self.user_dnn, self._l2_reg, 'user_dnn', self._is_training)
    ...
    user_features_tile = tf.tile(user_features[:, None, :], [1, tf.shape(high_capsules)[1], 1])
    ...
    concat_dnn = dnn.DNN(self.concat_dnn, self._l2_reg, 'concat_dnn', self._is_training)
    ...
    item_dnn = dnn.DNN(self.item_dnn, self._l2_reg, 'item_dnn', self._is_training)
    ...
    tf.summary.histogram('interest_item_simi/pre_scale', tf.reduce_max(simi, axis=1))
    ...
    tf.summary.histogram('interest_item_simi/scaled', tf.reduce_max(simi, axis=1))
    # simi = tf.Print(simi, [tf.reduce_max(simi, axis=1), tf.reduce_min(simi, axis=1)], message='simi_max')
    simi_mask = tf.sequence_mask(num_high_capsules, self._model_config.capsule_config.max_k)
    ...
    tf.summary.histogram('interest_item_simi/softmax', tf.reduce_max(simi, axis=1))
    ...
    if self._model_config.scale_simi:
      sim_w = tf.get_variable('sim_w', dtype=tf.float32, shape=(1), initializer=tf.ones_initializer())
  ...
  def _build_interest_simi(self):
    ...
    high_capsule_mask = tf.sequence_mask(user_emb_num, self._model_config.capsule_config.max_k)
    ...
    interest_div = tf.maximum(tf.to_float(user_emb_num * (user_emb_num - 1)), 1.0)
  ...
  def build_metric_graph(self, eval_config):
    ...
    if hard_neg_indices is not None:
      ...
      simple_item_emb, hard_neg_item_emb = tf.split(item_tower_emb, [-1, noclk_size], axis=0)
    else:
    ...
    if hard_neg_indices is not None:
      ...
      hard_neg_sim = tf.scatter_nd(hard_neg_indices, hard_neg_sim, hard_neg_shape)
      ...
      metric_dict['hard_neg_acc'] = metrics.accuracy(hard_lbls, tf.argmax(hard_logits, axis=1))
  ...
  def get_outputs(self):
    if self._loss_type == LossType.CLASSIFICATION:
      ...
      self._prediction_dict['logits'] = tf.squeeze(self._prediction_dict['logits'], axis=-1)
      self._prediction_dict['probs'] = tf.nn.sigmoid(self._prediction_dict['logits'])
      return [
        'logits',
        'probs',
        'user_emb',
