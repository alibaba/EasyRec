...
class MetaGraphEditor:
  def __init__(
    ...
    if saved_model_dir:
      ...
      meta_graph.import_scoped_meta_graph_with_return_elements(meta_graph_def, clear_devices=True)
  ...
  def find_lookup_inputs(self):
    ...
    for fea in shapes.keys():
      logging.info('Lookup Input[%s]: indices=%s values=%s shapes=%s' % (fea, indices[fea], values[fea], shapes[fea]))
    ...
    if not self._embed_name_to_ids:
      ...
      self._embed_name_to_ids = {t: tid for tid, t in enumerate(embed_name_uniq)}
    self._embed_ids = [int(self._embed_name_to_ids[x]) for x in self._embed_names]

    self._is_cache_from_redis = [proto_util.is_cache_from_redis(x, self._redis_cache_names) for x in self._embed_names]
  ...
  def add_oss_lookup_op(
    ...
    for embed_id, embed_combiner, embed_is_kv, embed_dim in zip(
      self._embed_ids, self._embed_combiners, self._embed_is_kv, self._embed_dims
    ):
    ...
    if self._incr_update_params is not None:
      ...
      for x in ops.get_collection(constant.DENSE_UPDATE_VARIABLES):
        ...
        dense_update_input = tf.placeholder(tf.float32, x.get_shape(), name=dense_input_name)
  ...
  def clear_save_restore(self):
    ...
    if self._restore_tensor_node:
      ...
      logging.info(
        'update self._restore_tensor_node: string_val keep_num = %d drop_num = %d' % (keep_node_num, len(drop_ids))
      )
    ...
    for tid, node in enumerate(self._all_graph_nodes):
      ...
      if node.op == 'Assign' and 'save/Assign' in node.name and 'embedding_weights' in node.input[0]:
        ...
        if tmp_input2 != node.input[1]:
          if self._verbose:
            logging.info("update save/Assign[%s]'s input from %s to %s" % (node.name, node.input[1], tmp_input2))
