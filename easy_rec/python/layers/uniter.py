...
from easy_rec.python.layers import dnn, multihead_cross_attention
...
class Uniter(object):
  ...
  def __init__(self, model_config, feature_configs, features, uniter_config, input_layer):
    ...
    if input_layer.has_group('text'):
      self._txt_seq_features, _, _ = input_layer(features, 'text', is_combine=False)
      tower_num += 1
    ...
    for fea_group in model_config.feature_groups:
      if fea_group.group_name == 'general':
        ...
        assert self._general_feature_num == len(
        ...
        assert self._img_feature_num == len(img_feature_names), 'there are duplicate features in `image` feature group'
        ...
        assert self._txt_feature_num == len(txt_feature_names), 'there are duplicate features in `text` feature group'
    ...
    if self._txt_feature_num > 0 and uniter_config.use_position_embeddings:
      assert (
        uniter_config.max_position_embeddings > 0
      ), 'model config `max_position_embeddings` must be greater than 0. '
      assert uniter_config.max_position_embeddings >= max_seq_len, (
        'model config `max_position_embeddings` must be greater than'
        ' or equal to the maximum of all feature config '
        '`max_seq_len`, which is %d'
      ) % max_seq_len
    ...
    self._general_emb_size = general_emb_dim_list[0] if general_emb_dim_list else 0
  ...
  def text_embeddings(self, token_type_id):
    ...
    if self._general_features is not None:
      ...
      if self._general_emb_size != hidden_size:
        ...
        general_features = tf.reshape(general_features, shape=[-1, self._general_emb_size])
        general_features = tf.layers.dense(general_features, hidden_size, name='txt_projection')
      general_features = tf.reshape(general_features, shape=[-1, self._general_feature_num, hidden_size])
    ...
    if self._txt_seq_features is not None:
      ...
      for i, (seq_fea, seq_len) in enumerate(self._txt_seq_features):
        ...
        if emb_size != hidden_size:
          ...
          seq_fea = tf.layers.dense(seq_fea, hidden_size, name='txt_seq_projection_%d' % i)
          seq_fea = tf.reshape(seq_fea, shape=[-1, max_seq_len, hidden_size])

        seq_fea = multihead_cross_attention.embedding_postprocessor(
          seq_fea,
          use_token_type=self._use_token_type,
          token_type_ids=tf.ones(shape=tf.stack([batch_size, max_seq_len]), dtype=tf.int32) * (i + token_type_id),
          token_type_vocab_size=self._token_type_vocab_size,
          reuse_token_type=tf.AUTO_REUSE,
  ...
  def image_embeddings(self):
    ...
    if self._img_emb_size != hidden_size:
      # Run a linear projection of `hidden_size`
      image_features = tf.reshape(image_features, shape=[-1, self._img_emb_size])
    ...
    img_fea = multihead_cross_attention.embedding_postprocessor(
      image_features,
      use_token_type=self._use_token_type,
      token_type_ids=tf.zeros(shape=tf.stack([batch_size, self._img_feature_num]), dtype=tf.int32),
      token_type_vocab_size=self._token_type_vocab_size,
      reuse_token_type=tf.AUTO_REUSE,
      use_position_embeddings=self._model_config.use_position_embeddings,
      max_position_embeddings=self._model_config.max_position_embeddings,
  ...
  def __call__(self, is_training, *args, **kwargs):
    ...
    if batch_size is not None:
      ...
      if img_fea is not None:
        ...
        mask = tf.ones(shape=tf.stack([batch_size, self._img_feature_num]), dtype=tf.int32)
        masks.append(mask)
    ...
    if self._other_features is not None:
      if self._model_config.HasField('other_feature_dnn'):
        ...
        other_dnn_layer = dnn.DNN(
          self._model_config.other_feature_dnn,
          l2_reg,
          'other_dnn',
          is_training,
        )
