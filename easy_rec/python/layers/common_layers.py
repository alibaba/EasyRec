...
def highway(x, size=None, activation=None, num_layers=1, scope="highway", dropout=0.0, reuse=None):
  with tf.variable_scope(scope, reuse):
    ...
    for i in range(num_layers):
      T = tf.layers.dense(x, size, activation=tf.sigmoid, name="gate_%d" % i, reuse=reuse)
...
def text_cnn(x, filter_sizes=(3, 4, 5), num_filters=(128, 64, 64), scope_name="textcnn", reuse=False):
  ...
  for i in range(len(filter_sizes)):
    ...
    with tf.variable_scope(scope_name_i, reuse=reuse):
      ...
      conv = tf.layers.conv1d(x, filters=num_filter, kernel_size=filter_size, activation=tf.nn.relu,
                              name="conv_layer", reuse=reuse, kernel_initializer=initializer, padding='same')
