...
class SENETLayer(Layer):
  """SENETLayer used in FiBiNET. Input shape.
  ...
  def build(self, input_shape):
    ...
    if input_shape[1] % self.field_size != 0:
      raise ValueError(
          f'Embed input size must be divisible by field_size, got {input_shape[1]} and {self.field_size}'
      )
    ...
    self.W_1 = self.add_weight(
        shape=(self.field_size, reduction_size),
        initializer=glorot_normal(seed=self.seed),
        name='W1',
        trainable=True)
    self.W_2 = self.add_weight(
        shape=(reduction_size, self.field_size),
        initializer=glorot_normal(seed=self.seed),
        name='W2',
        trainable=True)
  ...
  def call(self, inputs, training=None, **kwargs):
    ...
    A_2 = tf.nn.l2_normalize(
        A_2, axis=1) if self.norm == 'l2' else tf.nn.softmax(
            A_2, axis=1)
