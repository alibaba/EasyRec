...
class RTPInputV2(Input):
    ...
    def __init__(self, data_config, feature_config, input_path, task_index=0, task_num=1):
        super(RTPInputV2, self).__init__(data_config, feature_config, input_path, task_index, task_num)
    ...
    def _parse_rtp(self, lines):
        ...
        def _parse_one_line_tf(line):
            ...
            for i, key in enumerate(self._input_fields):
                ...
                temp_vals[i] = tf.cond(
                    tf.reduce_any(msk), lambda: tf.reduce_join(val, separator=','), lambda: tf.constant(str(def_val))
                )
            return temp_vals

        fields = tf.map_fn(_parse_one_line_tf, lines, tf_types, parallel_iterations=64, name='parse_one_line_tf_map_fn')
