...
class MetaGraphEditor:
    def __init__(
        ...
        self._signature_def = meta_graph_def.signature_def[signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY]
    ...
    def _get_share_embed_name(self, x, embed_names):
        ...
        for embed_name in embed_names:
            ...
            if tmp_name == embed_name_sub:
                assert not sel_embed_name, 'confusions encountered: %s %s' % (x, ','.join(embed_names))
                sel_embed_name = embed_name
        assert sel_embed_name, '%s not find in shared_embeddings: %s' % (tmp_name, ','.join(embed_names))
    ...
    def _find_embed_combiners(self, norm_embed_names):
        ...
        combiner_map = {'SparseSegmentSum': 'sum', 'SparseSegmentMean': 'mean', 'SparseSegmentSqrtN': 'sqrtn'}
        for node in self._meta_graph_def.graph_def.node:
            if node.op in combiner_map:
                ...
                embed_combine_node_cts[norm_name] = embed_combine_node_cts.get(norm_name, 0) + 1
            ...
            if '_embedding_weights/SparseReshape' in node.name:
                if node.op == 'SparseReshape':
                    ...
                    for tmp_input in node.input:
                        tmp_shape = _get_output_shape(self._meta_graph_def.graph_def, tmp_input)
    ...
    def _find_embed_names_and_dims(self, norm_embed_names):
        ...
        embed_dims = {}
        for node in self._meta_graph_def.graph_def.node:
            if 'embedding_weights' in node.name and node.op in ['VariableV2', 'KvVarHandleOp']:
    ...
    def find_lookup_inputs(self):
        ...
        for fea in shapes.keys():
            logging.info(
                'Lookup Input[%s]: indices=%s values=%s shapes=%s' % (fea, indices[fea], values[fea], shapes[fea])
            )
        ...
        self._embed_names, self._embed_dims = self._find_embed_names_and_dims(values.keys())

        if not self._embed_name_to_ids:
            embed_name_uniq = list(set(self._embed_names))
            self._embed_name_to_ids = {t: str(tid) for tid, t in enumerate(embed_name_uniq)}
        ...
        return lookup_input_indices, lookup_input_values, lookup_input_shapes, lookup_input_weights

    def add_lookup_op(self, lookup_input_indices, lookup_input_values, lookup_input_shapes, lookup_input_weights):
        ...
        if self._verbose:
            ...
            with GFile(debug_path, 'w') as fout:
                fout.write(text_format.MessageToString(self._meta_graph_def.graph_def, as_utf8=True))
    ...
    def clear_meta_graph_embeding(self, meta_graph_def):
        ...
        def _clear_embedding_in_meta_collect(meta_graph_def, collect_name):
            ...
            for tmp_v in tmp_vals:
                meta_graph_def.collection_def[collect_name].bytes_list.value.append(tmp_v)
        ...
        kept_ops = [
            x
            for x in meta_graph_def.meta_info_def.stripped_op_list.op
            if x.name
            not in [
    ...
    def clear_meta_collect(self, meta_graph_def):
        ...
        for key in meta_graph_def.collection_def:
            ...
            if val.HasField('node_list'):
                if 'embedding_weights' in val.node_list.value[0] and 'easy_rec' not in val.node_list.value[0]:
    ...
    def remove_embedding_weights_and_update_lookup_outputs(self):
        ...
        logging.info('and replace the old embedding_lookup outputs with new lookup_op outputs')
            ...
            if _should_drop(node.name):
                ...
                for i in range(len(node.input)):
                    if _should_drop(node.input[i]):
                        input_name, _ = proto_util.get_norm_embed_name(node.input[i], self._verbose)
                        print('REPLACE:' + node.input[i] + '=>' + input_name)
                        input_name = self._lookup_outs[self._feature_names.index(input_name)].name
    ...
    def _drop_by_ids(self, tmp_obj, key, drop_ids):
        keep_vals = [x for i, x in enumerate(getattr(tmp_obj, key)) if i not in drop_ids]
    ...
    def clear_save_restore(self):
        ...
        if self._restore_tensor_node:
            drop_ids = []
            for tmp_id, tmp_name in enumerate(self._restore_tensor_node.attr['value'].tensor.string_val):
                if 'embedding_weights' in self.bytes2str(tmp_name):
                    drop_ids.append(tmp_id)

            self._drop_by_ids(self._restore_tensor_node.attr['value'].tensor, 'string_val', drop_ids)
            keep_node_num = len(self._restore_tensor_node.attr['value'].tensor.string_val)
            logging.info(
            ...
            self._restore_tensor_node.attr['_output_shapes'].list.shape[0].dim[0].size = keep_node_num
    ...
    def clear_save_assign(self):
        ...
        for tid, node in enumerate(self._all_graph_nodes):
            if not self._all_graph_node_flags[tid]:
                continue
            if node.op == 'Assign' and 'save/Assign' in node.name and 'embedding_weights' in node.input[0]:
                ...
                if '_class' in node.attr and 'embedding_weights' in node.attr['_class'].list.s[0]:
                    self._all_graph_node_flags[tid] = False
                ...
                tmp_id = [self.bytes2str(x) for x in self._restore_tensor_node.attr['value'].tensor.string_val].index(
                    node.input[0]
                )
                if tmp_id != 0:
                ...
                if tmp_input2 != node.input[1]:
                    if self._verbose:
                        logging.info(
                            "update save/Assign[%s]'s input from %s to %s" % (node.name, node.input[1], tmp_input2)
                        )
    ...
    def clear_save_v2(self):
        ...
        for node in self._all_graph_nodes:
            if node.name == 'save/SaveV2/shape_and_slices' and node.op == 'Const':
                ...
                node.attr['_output_shapes'].list.shape[0].dim[0].size -= len(save_drop_ids)
                node.attr['value'].tensor.tensor_shape.dim[0].size -= len(save_drop_ids)
                self._drop_by_ids(node.attr['value'].tensor, 'string_val', save_drop_ids)
            elif node.name == 'save/SaveV2/tensor_names':
                # tensor_names may not have the same order as save/SaveV2/shape_and_slices
                tmp_drop_ids = [
                    tmp_id
                    for tmp_id, tmp_val in enumerate(node.attr['value'].tensor.string_val)
                ...
                node.attr['_output_shapes'].list.shape[0].dim[0].size -= len(tmp_drop_ids)
        ...
        */embedding_weights/part_1/Initializer/truncated_normal [,/shape,/mean,/stddev,/TruncatedNormal,/mul]
        ...
        for tid, node in enumerate(self._all_graph_nodes):
            ...
            if 'embedding_weights' in node.name and 'Initializer' in node.name:
                ...
                self._all_graph_node_flags[tid] = False
            elif 'embedding_weights' in node.name and node.name.endswith('/read') and node.op == 'Identity':
                self._all_graph_node_flags[tid] = False
    ...
    def drop_dependent_nodes(self):
        drop_names = [
            tmp_node.name for tid, tmp_node in enumerate(self._all_graph_nodes) if not self._all_graph_node_flags[tid]
        ]
        while True:
            more_drop_names = []
            for tid, tmp_node in enumerate(self._all_graph_nodes):
                ...
                if len(tmp_node.input) > 0 and tmp_node.input[0] in drop_names:
                    logging.info('drop dependent node: %s depend on %s' % (tmp_node.name, tmp_node.input[0]))
                    self._all_graph_node_flags[tid] = False
    ...
    def edit_graph(self):
        # the main entrance
        lookup_input_indices, lookup_input_values, lookup_input_shapes, lookup_input_weights = self.find_lookup_inputs()

        # add lookup op to the graph
        self._meta_graph_def = self.add_lookup_op(
            lookup_input_indices, lookup_input_values, lookup_input_shapes, lookup_input_weights
        )
        ...
        self._meta_graph_def.graph_def.node.extend(
            [x for tid, x in enumerate(self._all_graph_nodes) if self._all_graph_node_flags[tid]]
        )
        ...
        if self._verbose:
            ...
            with GFile(debug_dump_path, 'w') as fout:
                fout.write(text_format.MessageToString(self._meta_graph_def, as_utf8=True))
