...
from easy_rec.python.protos.multi_tower_pb2 import (  # NOQA
    MultiTower as MultiTowerConfig,
)
...
class MultiTowerBST(RankModel):
    def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
        super(MultiTowerBST, self).__init__(model_config, feature_configs, features, labels, is_training)
        self._seq_input_layer = seq_input_layer.SeqInputLayer(feature_configs, model_config.seq_att_groups)
        assert (
            self._model_config.WhichOneof('model') == 'multi_tower'
    ...
    def multi_head_att_net(self, id_cols, head_count, emb_dim, seq_len, seq_size):
        ...
        for start_idx in range(0, emb_dim, part_cols_emd_dim):
            ...
            part_attention_net = self.attention_net(
                part_id_col, part_cols_emd_dim, seq_len, seq_size, name='multi_head_%d' % start_idx
            )
    ...
    def build_predict_graph(self):
        ...
        for tower_id in range(self._tower_num):
            ...
            tower_fea = tf.layers.batch_normalization(
                tower_fea, training=self._is_training, trainable=True, name='%s_fea_bn' % tower_name
            )
            tower_dnn = dnn.DNN(tower.dnn, self._l2_reg, '%s_dnn' % tower_name, self._is_training)
        ...
        for tower_id in range(self._bst_tower_num):
            ...
            tower_fea = self.bst(tower_fea, seq_size=tower_seq_len, head_count=tower_multi_head_size, name=tower_name)
            tower_fea_arr.append(tower_fea)
