...
from easy_rec.python.utils import config_util, estimator_utils, test_utils
...
class TrainEvalTest(tf.test.TestCase):
    ...
    def test_wide_and_deep_no_final(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/wide_and_deep_no_final_on_avazau_ctr.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    def test_adamw_optimizer(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/deepfm_combo_on_avazu_adamw_ctr.config', self._test_dir
        )
        self.assertTrue(self._success)

    def test_momentumw_optimizer(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/deepfm_combo_on_avazu_momentumw_ctr.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    def test_multi_tower_save_checkpoint_secs(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/multi_tower_save_secs_on_taobao.config', self._test_dir, total_steps=500
        )
        ckpts_times = []
        ...
        self.assertAllClose(ckpts_times[1:] - ckpts_times[:-1], [20] * (len(ckpts_times) - 1), atol=8)
        self.assertTrue(self._success)
    ...
    def test_latest_ckpt_v2(self):
        ...
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/taobao_fg.config', self._test_dir, post_check_func=_post_check_func
        )
        self.assertTrue(self._success)

    def test_fine_tune_ckpt(self):
        def _post_check_func(pipeline_config):
            pipeline_config.train_config.fine_tune_checkpoint = estimator_utils.latest_checkpoint(
                pipeline_config.model_dir
            )
    ...
    def test_multi_tower_multi_value_export(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/multi_tower_multi_value_export_on_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    def test_dssm_hard_neg_sampler(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/dssm_hard_neg_sampler_on_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)

    def test_dssm_hard_neg_sampler_v2(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/dssm_hard_neg_sampler_v2_on_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    def _test_kd(self, config0, config1):
        ...
        self._success = test_utils.test_single_train_eval(
            os.path.join(self._test_dir, 'kd_pipeline.config'), os.path.join(self._test_dir, 'kd')
        )
        self.assertTrue(self._success)
    ...
    def test_deepfm_with_sigmoid_l2_loss(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/deepfm_combo_on_avazu_sigmoid_l2.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    def test_deepfm_with_embedding_learning_rate(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/deepfm_combo_on_avazu_emblr_ctr.config', self._test_dir
        )
        self.assertTrue(self._success)

    def test_deepfm_with_eval_online(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/deepfm_combo_on_avazu_eval_online_ctr.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    def test_early_stop(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/multi_tower_early_stop_on_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    def test_dbmtl_variational_dropout_feature_num(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/dbmtl_variational_dropout_feature_num.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    def test_train_with_ps_worker_chief_redundant(self):
        self._success = test_utils.test_distributed_train_eval(
            'samples/model_config/multi_tower_on_taobao_chief_redundant.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    def test_batch_tfrecord_input(self):
        self._success = test_utils.test_distributed_train_eval(
            'samples/model_config/deepfm_on_criteo_batch_tfrecord.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    @unittest.skipIf(
        LooseVersion(tf.__version__) < LooseVersion('2.3.0'), 'MultiWorkerMirroredStrategy need tf version > 2.3'
    )
    def test_train_with_multi_worker_mirror(self):
        self._success = test_utils.test_distributed_train_eval(
            'samples/model_config/multi_tower_multi_worker_mirrored_strategy_on_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    def test_sequence_autoint(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/autoint_on_sequence_feature_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)

    def test_sequence_dbmtl(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/dbmtl_on_sequence_feature_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    def test_sequence_dssm(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/dssm_on_sequence_feature_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)

    def test_sequence_essm(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/essm_on_sequence_feature_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)

    def test_sequence_fm(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/fm_on_sequence_feature_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)

    def test_sequence_mmoe(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/mmoe_on_sequence_feature_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)
    ...
    def test_sequence_rocket_launching(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/rocket_launching_on_sequence_feature_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)

    def test_sequence_simple_multi_task(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/simple_multi_task_on_sequence_feature_taobao.config', self._test_dir
        )
        self.assertTrue(self._success)

    def test_sequence_wide_and_deep(self):
        self._success = test_utils.test_single_train_eval(
            'samples/model_config/wide_and_deep_on_sequence_feature_taobao.config', self._test_dir
        )
