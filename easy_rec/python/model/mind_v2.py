...
class MIND_V2(MatchModel):
  ...
  def __init__(self,
               ...
               labels=None,
               is_training=False):
    super(MIND_V2, self).__init__(model_config, feature_configs, features, labels,
                               is_training)
  ...
  def build_predict_graph(self):
    ...
    simi = tf.einsum('bhe,ne->bhn', user_interests, item_tower_emb)
    pos_simi = tf.diag_part(tf.reduce_max(simi[:, :, :batch_size], axis=1))
    tf.summary.histogram('interest_item_simi/pre_scale', pos_simi) 
    ...
    pos_simi = tf.diag_part(tf.reduce_max(simi[:, :, :batch_size], axis=1))
    tf.summary.histogram('interest_item_simi/scaled', pos_simi)
    ...
    max_thresh = (tf.cast(simi_mask[:, :, None], tf.float32) * 2 - 1) * 1e32
    simi = tf.minimum(simi, max_thresh)
    simi = tf.nn.softmax(simi, axis=1)
    pos_simi = tf.diag_part(tf.reduce_max(simi[:, :, :batch_size], axis=1))
    tf.summary.histogram('interest_item_simi/softmax', pos_simi)

    user_tower_emb = tf.einsum('bhe,bhn->bne', user_interests, simi)

    y_pred = tf.einsum('bne,ne->bn', user_tower_emb, item_tower_emb)
    ...
    sel_ids = tf.range(batch_size)
    sel_ids = tf.concat([sel_ids[:, None], sel_ids[:, None]], axis=1)
    self._prediction_dict['user_tower_emb'] = tf.gather_nd(user_tower_emb, sel_ids) 
  ...
  def build_loss_graph(self):
    loss_dict = super(MIND_V2, self).build_loss_graph()
