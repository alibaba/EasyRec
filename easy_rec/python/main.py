...
def _get_input_fn(data_config, feature_configs, data_path=None, export_config=None):
    ...
    input_obj = input_class(data_config, feature_configs, data_path, task_index=task_id, task_num=task_num)
...
def _create_eval_export_spec(pipeline_config, eval_data):
    ...
    if eval_config.num_examples > 0:
        eval_steps = int(math.ceil(float(eval_config.num_examples) / data_config.batch_size))
        logging.info('eval_steps = %d' % eval_steps)
    else:
        eval_steps = None
    # create eval input
    export_input_fn = _get_input_fn(data_config, feature_configs, None, export_config)
    if export_config.exporter_type == 'final':
        exporters = [FinalExporter(name='final', serving_input_receiver_fn=export_input_fn)]
    elif export_config.exporter_type == 'latest':
        exporters = [
            LatestExporter(
                name='latest', serving_input_receiver_fn=export_input_fn, exports_to_keep=export_config.exports_to_keep
            )
        ]
    ...
    eval_spec = tf.estimator.EvalSpec(
        name='val', input_fn=eval_input_fn, steps=eval_steps, throttle_secs=10, exporters=exporters
    )
    return eval_spec
    ...
    if not continue_train:
        if not gfile.IsDirectory(model_dir):
            ...
            assert len(gfile.Glob(model_dir + '/model.ckpt-*.meta')) == 0, (
                ...
                'delete dir %s or specify --continue_train[internal use only])' % (model_dir, model_dir)
            )
    else:
        if not gfile.IsDirectory(model_dir):
...
def _get_ckpt_path(pipeline_config, checkpoint_path):
    if checkpoint_path != '' and checkpoint_path is not None:
        ...
        logging.info(
            'checkpoint_path is not specified, '
            'will use latest checkpoint %s from %s' % (ckpt_path, pipeline_config.model_dir)
        )
    else:
...
def _train_and_evaluate_impl(pipeline_config, continue_train=False):
    ...
    if export_config.dump_embedding_shape:
        embed_shape_dir = os.path.join(pipeline_config.model_dir, 'embedding_shapes')
...
def evaluate(pipeline_config, eval_checkpoint_path='', eval_data_path=None, eval_result_filename='eval_result.txt'):
    ...
    if 'TF_CONFIG' in os.environ:
        ...
        if tf_config['task']['type'] == 'ps':
            ...
            server = server_lib.Server(cluster, job_name='ps', task_index=tf_config['task']['index'])
            server.join()
    ...
    if server_target:
        ...
        from tensorflow.python.training.device_setter import (
            replica_device_setter,  # NOQA
        )
        from tensorflow.python.training.monitored_session import (
            ChiefSessionCreator,  # NOQA
        )
        from tensorflow.python.training.monitored_session import (
            MonitoredSession,  # NOQA
        )
        ...
        session_config = ConfigProto(allow_soft_placement=True, log_device_placement=True)
        chief_sess_creator = ChiefSessionCreator(
            master=server_target, checkpoint_filename_with_path=ckpt_path, config=session_config
        )
...
def distribute_evaluate(
    pipeline_config, eval_checkpoint_path='', eval_data_path=None, eval_result_filename='eval_result.txt'
):
    """Evaluate a EasyRec model defined in pipeline_config_path.
    ...
    if 'TF_CONFIG' in os.environ:
        ...
        if tf_config['task']['type'] == 'ps':
            ...
            if 'ps' in tf_config['cluster']:
                ...
                cluster = tf.train.ClusterSpec(tf_config['cluster'])
                server = server_lib.Server(cluster, job_name=cur_job_name, task_index=cur_task_index)
                server_target = server.target
                print('server_target = %s' % server_target)
    ...
    if server_target:
        ...
        from tensorflow.python.training.monitored_session import (
            WorkerSessionCreator,  # NOQA
        )

        from easy_rec.python.utils.estimator_utils import (
            EvaluateExitBarrierHook,  # NOQA
        )

        cur_work_device = '/job:' + cur_job_name + '/task:' + str(cur_task_index)
        with device(replica_device_setter(worker_device=cur_work_device, cluster=cluster)):
            estimator_spec = estimator._distribute_eval_model_fn(input_feas, input_lbls, run_config)
        ...
        if cur_job_name == 'master':
            ...
            cur_scaffold = tf.train.Scaffold(saver=cur_saver, ready_for_local_init_op=model_ready_for_local_init_op)
            ...
            cur_sess_creator = WorkerSessionCreator(master=server_target, config=session_config)
        eval_metric_ops = estimator_spec.eval_metric_ops
        update_ops = [eval_metric_ops[x][1] for x in eval_metric_ops.keys()]
        metric_ops = {x: eval_metric_ops[x][0] for x in eval_metric_ops.keys()}
        update_op = tf.group(update_ops)
        ...
        with MonitoredSession(
            session_creator=cur_sess_creator, hooks=[cur_hooks], stop_grace_period_secs=cur_stop_grace_period_sesc
        ) as sess:
            while True:
                try:
                    count += 1
...
def export(export_dir, pipeline_config, checkpoint_path='', asset_files=None, verbose=False, **redis_params):
    ...
    export_config = pipeline_config.export_config
    data_config = pipeline_config.data_config
    serving_input_fn = _get_input_fn(data_config, feature_configs, None, export_config)

    if 'redis_url' in redis_params:
        return export_big_model(
            export_dir, pipeline_config, redis_params, serving_input_fn, estimator, checkpoint_path, verbose
        )
    ...
    if export_config.dump_embedding_shape:
        ...
        embed_files = gfile.Glob(os.path.join(pipeline_config.model_dir, 'embeddings', '*.pb.' + ckpt_ver))
        assets_extra = {}
    ...
    export_ts = [x for x in final_export_dir.split('/') if x != '' and x is not None]
