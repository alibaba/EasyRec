...
class CMBF(object):
  ...
  def __init__(
    self, model_config, feature_configs, features, cmbf_config, input_layer
  ):
    ...
    print(
      'txt_feature_num: {0}, img_feature_num: {1}, txt_seq_feature_num: {2}'.
      format(
        self._general_feature_num, self._img_feature_num,
        len(self._txt_seq_features) if self._txt_seq_features else 0
      )
    )
    print(
      'txt_embedding_size: {0}, img_embedding_size: {1}'.format(
    ...
    if self._txt_seq_features is not None:
      ...
      for i, (seq_fea, seq_len) in enumerate(self._txt_seq_features):
        ...
        seq_fea = multihead_cross_attention.embedding_postprocessor(
          ...
          token_type_ids=tf.
          ones(shape=tf.stack([batch_size, max_seq_len]), dtype=tf.int32) * i,
  ...
  def __call__(self, is_training, *args, **kwargs):
    ...
    if None not in [img_attention_fea, txt_attention_fea]:
      ...
      txt_embeddings = self.merge_text_embedding(
        txt_attention_fea, input_masks
      )
    ...
    if self._other_features is not None:
      if self._model_config.HasField('other_feature_dnn'):
        ...
        other_dnn_layer = dnn.DNN(
          self._model_config.other_feature_dnn, l2_reg, 'other_dnn',
          is_training
        )
