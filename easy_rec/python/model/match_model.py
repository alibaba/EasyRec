...
class MatchModel(EasyRecModel):
  def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
    super(MatchModel, self).__init__(model_config, feature_configs, features, labels, is_training)
  ...
  def _list_wise_sim(self, user_emb, item_emb):
    ...
    if hard_neg_indices is not None:
      ...
      simple_item_emb, hard_neg_item_emb = tf.split(item_emb, [-1, noclk_size], axis=0)
    else:
    ...
    if _mode == tf.estimator.ModeKeys.PREDICT:
      simple_user_item_sim = tf.reduce_sum(tf.multiply(user_emb, simple_item_emb), axis=1, keep_dims=True)
    else:
      simple_user_item_sim = tf.matmul(user_emb, tf.transpose(simple_item_emb))
    ...
    if hard_neg_indices is None:
      ...
      hard_neg_user_item_sim = tf.reduce_sum(tf.multiply(user_emb_expand, hard_neg_item_emb), axis=1)
      max_num_neg = tf.reduce_max(hard_neg_indices[:, 1]) + 1
      hard_neg_shape = tf.stack([tf.to_int64(batch_size), max_num_neg])
      hard_neg_sim = tf.scatter_nd(hard_neg_indices, hard_neg_user_item_sim, hard_neg_shape)
  ...
  def _point_wise_sim(self, user_emb, item_emb):
    user_item_sim = tf.reduce_sum(tf.multiply(user_emb, item_emb), axis=1, keep_dims=True)
  ...
  def build_predict_graph(self):
    if not self.has_backbone:
      raise NotImplementedError('method `build_predict_graph` must be implemented when you donot use backbone network')
  ...
  def _build_list_wise_loss_graph(self):
    if self._loss_type == LossType.SOFTMAX_CROSS_ENTROPY:
      ...
      self._loss_dict['cross_entropy_loss'] = -tf.reduce_mean(
        tf.log(hit_prob + 1e-12) * sample_weights
      ) / tf.reduce_mean(sample_weights)
      ...
      self._loss_dict['reg_pos_loss'] = tf.reduce_mean(reg_pos_loss * sample_weights) / tf.reduce_mean(sample_weights)

      # the AMM loss for DAT model
      if all(
        [
          k in self._prediction_dict.keys()
          for k in [
            'augmented_p_u',
            'augmented_p_i',
            'augmented_a_u',
        ...
        self._loss_dict['amm_loss_u'] = (
          self._model_config.amm_u_weight
          * tf.reduce_mean(
            tf.square(self._prediction_dict['augmented_a_u'] - self._prediction_dict['augmented_p_i'][:batch_size])
            * sample_weights
          )
          / tf.reduce_mean(sample_weights)
        )
        self._loss_dict['amm_loss_i'] = (
          self._model_config.amm_i_weight
          * tf.reduce_mean(
            tf.square(self._prediction_dict['augmented_a_i'][:batch_size] - self._prediction_dict['augmented_p_u'])
            * sample_weights
          )
          / tf.reduce_mean(sample_weights)
        )
  ...
  def _build_point_wise_loss_graph(self):
    ...
    # build kd loss
    kd_loss_dict = loss_builder.build_kd_loss(self.kd, self._prediction_dict, self._labels, self._feature_dict)
  ...
  def _build_list_wise_metric_graph(self, eval_config):
    ...
    for metric in eval_config.metrics_set:
      if metric.WhichOneof('metric') == 'recall_at_topk':
        metric_dict['recall@%d' % metric.recall_at_topk.topk] = metrics_tf.recall_at_k(
        ...
        metric_dict['recall_neg_sam@%d' % metric.recall_at_topk.topk] = metrics_tf.recall_at_k(
          labels_v2, logits_v2, metric.recall_at_topk.topk
        )

        metric_dict['recall_in_batch@%d' % metric.recall_at_topk.topk] = metrics_tf.recall_at_k(
  ...
  def _build_point_wise_metric_graph(self, eval_config):
    ...
    for metric in eval_config.metrics_set:
      if metric.WhichOneof('metric') == 'auc':
        ...
        metric_dict['auc'] = metrics_tf.auc(label, self._prediction_dict['probs'])
  ...
  def get_outputs(self):
    if not self.has_backbone:
      raise NotImplementedError('could not call get_outputs on abstract class MatchModel')
    if self._loss_type == LossType.CLASSIFICATION:
      return [
        'logits',
        'probs',
