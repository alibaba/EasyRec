...
class MIND(MatchModel):
  ...
  def build_predict_graph(self):
    capsule_layer = CapsuleLayer(
      self._model_config.capsule_config, self._is_training
    )
    ...
    if self._model_config.time_id_fea:
      ...
      logging.info(
        'time_id_fea is set(%s), find num: %d' %
    ...
    if self._model_config.HasField('pre_capsule_dnn') and \
      ...
      pre_dnn_layer = dnn.DNN(
        self._model_config.pre_capsule_dnn, self._l2_reg, 'pre_capsule_dnn',
        self._is_training
      )
    ...
    high_capsules, num_high_capsules = capsule_layer(
      hist_seq_feas, hist_seq_len
    )
    ...
    tf.summary.scalar(
      'high_capsules_norm', tf.reduce_mean(tf.norm(high_capsules, axis=-1))
    )
    tf.summary.scalar(
      'num_high_capsules', tf.reduce_mean(tf.to_float(num_high_capsules))
    )
    ...
    user_dnn = dnn.DNN(
      self.user_dnn, self._l2_reg, 'user_dnn', self._is_training
    )
    ...
    user_features_tile = tf.tile(
      user_features[:, None, :], [1, tf.shape(high_capsules)[1], 1]
    )
    ...
    concat_dnn = dnn.DNN(
      self.concat_dnn, self._l2_reg, 'concat_dnn', self._is_training
    )
    ...
    item_dnn = dnn.DNN(
      self.item_dnn, self._l2_reg, 'item_dnn', self._is_training
    )
    ...
    tf.summary.histogram(
      'interest_item_simi/pre_scale', tf.reduce_max(simi, axis=1)
    )
    ...
    tf.summary.histogram(
      'interest_item_simi/scaled', tf.reduce_max(simi, axis=1)
    )
    # simi = tf.Print(simi, [tf.reduce_max(simi, axis=1), tf.reduce_min(simi, axis=1)], message='simi_max')
    simi_mask = tf.sequence_mask(
      num_high_capsules, self._model_config.capsule_config.max_k
    )
    ...
    tf.summary.histogram(
      'interest_item_simi/softmax', tf.reduce_max(simi, axis=1)
    )
  ...
  def _build_interest_simi(self):
    ...
    sum_interest_simi = tf.reduce_sum(
      (interest_simi + 1) * multi_interest
    ) / 2.0
    ...
    sum_capsule_simi = tf.reduce_sum(
      (high_capsule_simi + 1) * multi_interest
    ) / 2.0
  ...
  def build_metric_graph(self, eval_config):
    ...
    pos_indices = tf.concat(
      [pos_indices[:, None], pos_indices[:, None]], axis=1
    )
    pos_item_sim = tf.gather_nd(
      simple_item_sim[:batch_size, :batch_size], pos_indices
    )
    ...
    pos_item_logits = tf.gather_nd(
      logits[:batch_size, :batch_size], pos_indices
    )
    logits_v2 = tf.concat(
      [pos_item_logits[:, None], logits[:, batch_size:]], axis=1
    )
    ...
    if hard_neg_indices is not None:
      ...
      hard_neg_sim = tf.einsum(
        'nhe,ne->nh', hard_neg_user_emb, hard_neg_item_emb
      )
      ...
      hard_neg_sim = tf.scatter_nd(
        hard_neg_indices, hard_neg_sim, hard_neg_shape
      )
