...
from easy_rec.python.layers import dnn, mmoe
...
class DBMTL(MultiTaskModel):
    def __init__(self, model_config, feature_configs, features, labels=None, is_training=False):
        super(DBMTL, self).__init__(model_config, feature_configs, features, labels, is_training)
        assert (
            self._model_config.WhichOneof('model') == 'dbmtl'
    ...
    def build_predict_graph(self):
        if self._model_config.HasField('bottom_dnn'):
            bottom_dnn = dnn.DNN(
                self._model_config.bottom_dnn, self._l2_reg, name='bottom_dnn', is_training=self._is_training
            )
        ...
        for i, task_tower_cfg in enumerate(self._model_config.task_towers):
            ...
            if task_tower_cfg.HasField('dnn'):
                tower_dnn = dnn.DNN(
                    task_tower_cfg.dnn, self._l2_reg, name=tower_name + '/dnn', is_training=self._is_training
                )
        ...
        for task_tower_cfg in self._model_config.task_towers:
            ...
            output_logits = tf.layers.dense(
                relation_fea, task_tower_cfg.num_class, kernel_regularizer=self._l2_reg, name=tower_name + '/output'
            )
