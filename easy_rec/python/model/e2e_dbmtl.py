...
from easy_rec.python.protos.dbmtl_pb2 import E2E_DBMTL as E2E_DBMTL_Config

if tf.__version__ >= '2.0':
  tf = tf.compat.v1


class E2E_DBMTL(MultiTaskModel):
    ...
    if 'img' in self._input_layer._feature_groups:
      ...
      for feature_config in feature_configs:
        if feature_config.feature_type == feature_config.ImgFeature:
          self.img_width = feature_config.width
          self.img_height = feature_config.height
          self.img_channel = feature_config.channel
          img_fea_flag = True
          break
  ...
  def highway_layer(self, x, size=None, num_layers=1, dropout=0.0, name=''):
    with tf.variable_scope(name, x):
      if size is None:
        size = x.shape.as_list()[-1]
      else:
        x = tf.layers.dense(x, size, activation=tf.nn.relu, name='projection')

      for i in range(num_layers):
        T = tf.layers.dense(
            inputs=x, units=size, activation=tf.sigmoid, name='gate_%d' % i)
        H = tf.layers.dense(
            inputs=x,
            units=size,
  ...
  def tune_img_emb(self, img_emb):
    with tf.device('/CPU:0'):
      img_emb = tf.layers.batch_normalization(
          img_emb,
  ...
  def img_net(self, img_feature):
    ...
    with tf.device('/GPU:0'):
      ...
      img_emb = resnet_v1a_18(
          img_feature, num_classes=100, is_training=self._is_training)[0]
      img_emb = self.tune_img_emb(img_emb)
  ...
  def build_predict_graph(self):
    ...
    if 'img' in self._input_layer._feature_groups:
      # img_emb = tf.cond(tf.equal(tf.shape(self._img_features)[1], self.img_width*self.img_height*self.img_channel),
      #                   self.img_net,
      #                   self.img_direct)
    ...
    print(self._prediction_dict.keys())
