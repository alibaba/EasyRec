...
from easy_rec.python.layers.common_layers import gelu, highway
...
from easy_rec.python.protos.collaborative_metric_learning_pb2 import (  # NOQA
    CoMetricLearningI2I as MetricLearningI2IConfig,
)
...
class CoMetricLearningI2I(EasyRecModel):
    def __init__(
        ...
        assert (
            self._model_config.WhichOneof('model') == 'metric_learning'
        ...
        assert (
            self._model_config.WhichOneof('loss').lower() == LossType.Name(self._loss_type).lower()
    ...
    def build_predict_graph(self):
        for _id in range(self._highway_num):
            ...
            highway_fea = highway(highway_fea, highway_cfg.emb_size, activation=gelu, scope='highway_%s' % _id)
    ...
    def build_loss_graph(self):
        ...
        if self._loss_type == LossType.CIRCLE_LOSS:
            self._loss_dict['circle_loss'] = circle_loss(
                norm_emb, self.labels, self.session_ids, self.loss.margin, self.loss.gamma, embed_normed=emb_normed
            )
    ...
    def build_metric_graph(self, eval_config):
        ...
        if len(precision_at_k) > 0:
            metric_dict.update(
                metric_learning_average_precision_at_k(precision_at_k, emb, self.labels, self.session_ids)
            )
        return metric_dict
