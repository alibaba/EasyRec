...
def check_split(line, sep, requried_field_num, field_name=''):
  ...
  for one_line in line:
    ...
    if field_name:
      assert_info = (
        'sep[%s] maybe invalid. field_num=%d, required_num=%d, field: %s, value: %s, '
        'please check separator and data.' % (sep, field_num, requried_field_num, field_name, one_line)
      )
    else:
      assert_info = (
        'sep[%s] maybe invalid. field_num=%d, required_num=%d, current line is: %s, '
        'please check separator and data.' % (sep, field_num, requried_field_num, one_line)
      )
...
def check_string_to_number(field_vals, field_name):
  for val in field_vals:
    try:
      ...
      assert False, (
        'StringToNumber ERROR: cannot convert string_to_number, field: %s, value: %s. '
        'please check data.'
        % (
          field_name,
          val,
...
def check_sequence(pipeline_config_path, features):
  ...
  for seq_att_group in seq_att_groups:
    ...
    for seq_att_map in seq_att_maps:
      assert len(seq_att_map.key) == len(
