...
import easy_rec
from easy_rec.python.utils import estimator_utils, io_util, proto_util
...
def export_big_model(export_dir, pipeline_config, redis_params, serving_input_fn, estimator, checkpoint_path, verbose):
    ...
    if redis_params.get('redis_write_kv', ''):
        ...
        chief_sess_creator = ChiefSessionCreator(
            master=server.target if server else '', checkpoint_filename_with_path=checkpoint_path, config=session_config
        )
        with tf.train.MonitoredSession(
            session_creator=chief_sess_creator, hooks=None, stop_grace_period_secs=120
        ) as sess:
    ...
    tf.add_to_collection(
        tf.GraphKeys.ASSET_FILEPATHS, tf.constant(embed_name_to_id_file, dtype=tf.string, name='embed_name_to_ids.txt')
    )
