...
from tensorflow.python.ops import gradients as ops_gradients


from easy_rec.python.sok_adapter import sok_import_sucess
...
def optimize_loss(loss,
    ...
    with vs.variable_scope(name, 'OptimizeLoss', [loss, global_step]):
        ...
        # Compute gradients.
        sok_emb_update_ops = None
        if sok_import_sucess:
            sok_emb_vars, variables = sok.optimizers.utils.split_embedding_variable_from_others(variables)
            sok_emb_vars = ops.get_collection("SOK")[1].trainable_variables
            if len(sok_emb_vars) > 0:
                replica_emb_vars = []
                replica_emb_grads = []
                for emb_var in sok_emb_vars:
                    grads = ops_gradients.gradients(loss, [v for v in emb_var.values],
                                                 colocate_gradients_with_ops=False,
                                                 unconnected_gradients=tensorflow.UnconnectedGradients.NONE)
                    valid_grad = []
                    valid_grad_idx = []
                ...
                # SGD for now. How to do it later?
                #emb_opt = tensorflow.keras.optimizers.SGD(learning_rate=lr)
                #emb_opt = modify_apply_gradients(emb_opt)
                emb_opt = ops.get_collection("SOK")[2]
                with sok.OptimizerScope(replica_emb_vars):
