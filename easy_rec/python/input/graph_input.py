      ...
      try:
        ...
        if self._mode == tf.estimator.ModeKeys.TRAIN:
          start_nodes = GraphInput.graph.V(GraphInput.node_type).batch(
              self._data_config.batch_size).values().next()
        else:
          start_nodes = GraphInput.graph.V(GraphInput.node_type).batch(
              self._data_config.batch_size).values().next()
        # sample paths
        paths = GraphInput.graph.V(
            GraphInput.node_type, feed=start_nodes).repeat(
                lambda v: v.outV(GraphInput.edge_type).sample(1).by('random'),
                self._walk_len -
                1).emit(lambda x:
                        [x[i].ids.reshape([-1]) for i in range(self._walk_len)])
        # build positive pairs
        src_ids, dst_ids = gl.gen_pair(paths, self._window_size,
                                       self._window_size)
        ...
        # negative sample
        neg_nodes = GraphInput.graph.V(
            GraphInput.node_type,
            feed=src_nodes).outNeg(GraphInput.edge_type).sample(
                self._negative_num).by('random').emit(lambda x: x[1])
