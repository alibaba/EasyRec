...
class RankModel(EasyRecModel):
  ...
  def _output_to_prediction_impl(self, output, loss_type, num_class=1, suffix='', **kwargs):
    prediction_dict = {}
    binary_loss_type = {
      LossType.F1_REWEIGHTED_LOSS,
      LossType.PAIR_WISE_LOSS,
      LossType.BINARY_FOCAL_LOSS,
      LossType.PAIRWISE_FOCAL_LOSS,
      LossType.LISTWISE_RANK_LOSS,
      LossType.PAIRWISE_HINGE_LOSS,
      LossType.PAIRWISE_LOGISTIC_LOSS,
      LossType.BINARY_CROSS_ENTROPY_LOSS,
  ...
  def _add_to_prediction_dict(self, output):
    if len(self._losses) == 0:
      prediction_dict = self._output_to_prediction_impl(output, loss_type=self._loss_type, num_class=self._num_class)
  ...
  def build_rtp_output_dict(self):
    ...
    try:
      ...
      if len(op.outputs) != 1:
        raise ValueError(
          ('failed to build RTP rank_predict output: op {}[{}] has output ' + 'size {}, however 1 is expected.').format(
      ...
      if loss_types & binary_loss_set:
        ...
        logging.warning('failed to build RTP rank_predict: unsupported loss type {}'.format(loss_types))
  ...
  def _build_loss_impl(
    ...
    if loss_type in {LossType.CLASSIFICATION, LossType.BINARY_CROSS_ENTROPY_LOSS}:
    ...
    if metric.WhichOneof('metric') == 'auc':
      ...
      if num_class == 1 or loss_type & {LossType.JRC_LOSS, LossType.ZILN_LOSS}:
        ...
        if isinstance(uids, tf.sparse.SparseTensor):
          uids = tf.sparse_to_dense(uids.indices, uids.dense_shape, uids.values, default_value='')
      ...
      if num_class == 1 or loss_type & {LossType.JRC_LOSS, LossType.ZILN_LOSS}:
        label = tf.to_int64(self._labels[label_name])
        metric_dict['max_f1' + suffix] = metrics_lib.max_f1(label, self._prediction_dict['logits' + suffix])
      elif num_class == 2:
        label = tf.to_int64(self._labels[label_name])
        metric_dict['max_f1' + suffix] = metrics_lib.max_f1(label, self._prediction_dict['logits' + suffix][:, 1])
      else:
        raise ValueError('Wrong class number')
      ...
      metric_dict['recall_at_topk' + suffix] = metrics_tf.recall_at_k(
        label,
        self._prediction_dict['logits' + suffix],
      ...
      if loss_type & {
        ...
        metric_dict['mean_absolute_error' + suffix] = metrics_tf.mean_absolute_error(
      ...
      if loss_type & {
        ...
        metric_dict['mean_squared_error' + suffix] = metrics_tf.mean_squared_error(
          label, self._prediction_dict['y' + suffix]
        )
      ...
      if loss_type & {
        ...
        metric_dict['root_mean_squared_error' + suffix] = metrics_tf.root_mean_squared_error(
          label, self._prediction_dict['y' + suffix]
        )
      elif loss_type & {LossType.CLASSIFICATION} and num_class == 1:
        metric_dict['root_mean_squared_error' + suffix] = metrics_tf.root_mean_squared_error(
      ...
      assert num_class > 1
      label = tf.to_int64(self._labels[label_name])
      metric_dict['accuracy' + suffix] = metrics_tf.accuracy(label, self._prediction_dict['y' + suffix])
    return metric_dict
