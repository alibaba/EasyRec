...
class HiveParquetInput(Input):
  ...
  def __init__(
    ...
    input_hdfs_path = hive_util.get_table_location(
  ...
  def _parquet_read(self):
    for input_path in self._input_hdfs_path:
      ...
      for start_idx in range(
        0, total_records_num, self._data_config.batch_size
      ):
        end_idx = min(
          total_records_num, start_idx + self._data_config.batch_size
        )
  ...
  def _build(self, mode, params):
    ...
    if len(self._all_hdfs_path) >= 2 * self._task_num:
      file_shard = True
      self._input_hdfs_path = self._file_shard(
        self._all_hdfs_path, self._task_num, self._task_index
      )
    else:
      file_shard = False
    ...
    dataset = dataset.map(
      self._parse_csv, num_parallel_calls=self._data_config.num_parallel_calls
    )
