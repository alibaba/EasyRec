...
def export_big_model(export_dir, pipeline_config, redis_params,
                     serving_input_fn, estimator, checkpoint_path, tf_config,
                     verbose):
  ...
  with gfile.GFile(embed_name_to_id_file, 'w') as fout:
...
def export_big_model_to_oss(export_dir, pipeline_config, oss_params,
  ...
  for key in oss_params:
    logging.info('oss_params: %s=%s' % (key, oss_params[key]))

  write_kv_lib_path = os.path.join(easy_rec.ops_dir, 'libembed_op.so')
  logging.info('write_kv_lib_path=%s' % write_kv_lib_path)
  kv_module = tf.load_op_library(write_kv_lib_path)
  ...
  with ops.Graph().as_default():
    ...
    embedding_vars = {}
    for x in global_variables():
      logging.info('global var: %s %s[device=%s]' %
                   (x.name, str(type(x)), x.device))
      if 'EmbeddingVariable' in str(type(x)):
        norm_name, part_id = proto_util.get_norm_embed_name(x.name)
        tmp_export = x.export()
        if x.device not in embedding_vars:
  ...
  with ops.Graph().as_default():
    ...
    if 'incr_update' in oss_params:
      ...
      asset_file_path = os.path.join(estimator.model_dir, asset_file)
      with gfile.GFile(asset_file_path, 'w') as fout:
        ...
        # there is no need to update dense from old steps
        incr_update_json['dense_step'] = global_step
        incr_update_json['sparse_step'] = max(global_step - 100, 0)
