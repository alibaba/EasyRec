...
class OdpsRTPInput(Input):
  ...
  def _parse_table(self, *fields):
    ...
    print(
      'field_delim = %s, feature_num = %d' %
      (self._data_config.separator, feature_num)
    )
    logging.info(
      'field_delim = %s, input_field_name = %d' %
    ...
    for i in range(feature_num):
      field = string_to_number(
        tmp_fields[:, i], record_types[i], record_defaults[i], i
      )
      fields.append(field)
    ...
    print(
      'effective field num = %d, input_num = %d' % (len(fields), len(inputs))
    )
    return inputs
  ...
  def _build(self, mode, params):
    ...
    assert len(
      self._input_path
    ) > 0, 'match no files with %s' % self._input_path
    ...
    if selected_cols:
      ...
      print(
        'selected_cols: %s; defaults num: %d' %
    ...
    if self._data_config.pai_worker_queue and \
      ...
      logging.info(
        'pai_worker_slice_num = %d' % self._data_config.pai_worker_slice_num
      )
