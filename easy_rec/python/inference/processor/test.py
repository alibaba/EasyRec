...
if __name__ == '__main__':
  ...
  if not os.path.exists(PROCESSOR_ENTRY_LIB):
    if not os.path.exists('processor/' + PROCESSOR_FILE):
      subprocess.check_output('wget %s -O processor/%s' % (PROCESSOR_URL, PROCESSOR_FILE), shell=True)
    subprocess.check_output('cd processor && tar -zvxf %s' % PROCESSOR_FILE, shell=True)
    assert os.path.exists(PROCESSOR_ENTRY_LIB), 'invalid processor path: %s' % PROCESSOR_ENTRY_LIB
  ...
  assert args.input_path is not None and os.path.exists(args.input_path), '%s does not exist' % args.input_path
  ...
  input_fields = [[] for x in data_config.input_fields if x.input_name not in data_config.label_fields]
  ...
  while sparse_step.value < last_step or dense_step.value < last_step:
    tf_predictor.saved_model_step(ctypes.c_void_p(handle), ctypes.byref(sparse_step), ctypes.byref(dense_step))
    time.sleep(1)
    if time.time() - start_ts > 300:
      logging.warning('could not reach last_step, sparse_step=%d dense_step=%d' % (sparse_step.value, dense_step.value))
      break
