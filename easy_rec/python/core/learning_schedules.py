...
def exponential_decay_with_burnin(
    ...
    return tf.maximum(
        tf.where(
            tf.less(tf.cast(global_step, tf.int32), tf.constant(burnin_steps)), burnin_rate, post_burnin_learning_rate
        ),
        min_learning_rate,
        name='learning_rate',
    )
...
def cosine_decay_with_warmup(
    global_step, learning_rate_base, total_steps, warmup_learning_rate=0.0, warmup_steps=0, hold_base_rate_steps=0
):
...
def manual_stepping(global_step, boundaries, rates, warmup=False):
    ...
    rate_index = tf.reduce_max(
        tf.where(tf.greater_equal(global_step, boundaries), list(range(num_boundaries)), [0] * num_boundaries)
    )
