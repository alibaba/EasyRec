...
class RTPInput(Input):
    ...
    def __init__(self, data_config, feature_config, input_path, task_index=0, task_num=1):
        super(RTPInput, self).__init__(data_config, feature_config, input_path, task_index, task_num)
        logging.info('input_fields: %s label_fields: %s' % (','.join(self._input_fields), ','.join(self._label_fields)))
        ...
        self._selected_cols = [int(x) for x in self._data_config.selected_cols.split(',')]
        self._num_cols = -1
    ...
    def _parse_csv(self, line):
        ...
        lbl_id = 0
        for x, t, v in zip(self._input_fields, self._input_field_types, self._input_field_defaults):
            if x not in self._label_fields:
                continue
            record_defaults[self._selected_cols[lbl_id]] = self.get_type_defaults(t, v)
        ...
        record_types = [t for x, t in zip(self._input_fields, self._input_field_types) if x not in self._label_fields]
        # assume that the last field is the generated feature column
        print('field_delim = %s' % self._data_config.separator)
        fields = tf.string_split(fields[:, self._feature_col_id], self._data_config.separator, skip_empty=False)
    ...
    def _build(self, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            if self._data_config.chief_redundant:
                dataset = dataset.shard(max(self._task_num - 1, 1), max(self._task_index - 1, 0))
            else:
                dataset = dataset.shard(self._task_num, self._task_index)
            if self._data_config.shuffle:
                dataset = dataset.shuffle(
                    self._data_config.shuffle_buffer_size, seed=2020, reshuffle_each_iteration=True
                )
        ...
        dataset = dataset.map(self._parse_csv, num_parallel_calls=self._data_config.num_parallel_calls)

        # preprocess is necessary to transform data
        # so that they could be feed into FeatureColumns
        dataset = dataset.map(map_func=self._preprocess, num_parallel_calls=self._data_config.num_parallel_calls)
