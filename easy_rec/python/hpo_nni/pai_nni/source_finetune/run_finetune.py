...
from easy_rec.python.hpo_nni.pai_nni.code.utils import set_value, parse_config


def get_params():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        '--odps_config', type=str, help='odps_config.ini', default='../config/odps_config.ini')
    parser.add_argument(
        '--oss_config', type=str, help='excel config path', default='../config/.ossutilconfig')
    parser.add_argument(
        '--easyrec_cmd_config', type=str, help='pai config path', default='../config/easyrec_cmd_config_finetune')
    parser.add_argument(
        '--exp_dir', type=str, help='exp dir', default='../exp')
    parser.add_argument(
        '--metric_config', type=str, help='metric config path', default='../config/metric_config')
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    try:
        ...
        o = create_odps(access_id=odps_config['access_id'], access_key=odps_config['access_key'],
                        project=odps_config['project_name'], endpoint=odps_config['end_point'])
        ...
        while (datestart <= dateend):
            ...
            if cnt > 0:
                pre_edit['train_config.fine_tune_checkpoint'] = os.path.join(
                    pre_edit['train_config.fine_tune_checkpoint'], experment_id + '_' + trial_id)
            edit_json = json.dumps(pre_edit)
            easyrec_cmd_config['-Dedit_config_json'] = edit_json
            print(easyrec_cmd_config['-Dedit_config_json'])
            easyrec_cmd_config['-Dtrain_tables'] = easyrec_cmd_config['-Dtrain_tables'].replace('{bizdate}',
                                                                                                datestart.strftime(
                                                                                                    '%Y%m%d'))

            next_day = datestart + datetime.timedelta(days=1)
            easyrec_cmd_config['-Deval_tables'] = easyrec_cmd_config['-Deval_tables'].replace('{eval_ymd}',
                                                                                              next_day.strftime(
                                                                                                  '%Y%m%d'))

            pre_day = datestart - datetime.timedelta(days=1)
            easyrec_cmd_config['-Dedit_config_json'] = easyrec_cmd_config['-Dedit_config_json'].replace('{predate}',
                                                                                                        pre_day.strftime(
                                                                                                            '%Y%m%d'))

            easyrec_cmd_config['-Dmodel_dir'] = easyrec_cmd_config['-Dmodel_dir'].replace('{bizdate}',
                                                                                          datestart.strftime('%Y%m%d'))

            easyrec_cmd_config['-Dmodel_dir'] = os.path.join(easyrec_cmd_config['-Dmodel_dir'],
            ...
            dst_filepath = os.path.join(args.exp_dir, experment_id + '_' + trial_id, datestart.strftime('%Y%m%d'))
            metric_dict = parse_config(args.metric_config)
            print("filepath:", filepath)
            print("dst_file_path:", dst_filepath)
            print("metric dict:", metric_dict)
            best_res, best_event = get_result(filepath, dst_filepath, metric_dict, trial_id, oss_config=args.oss_config,
                                              nni_report=False)
            if best_res:
